...
## MINOR (ILP32-native readiness) — native `__Package` accessor hardcodes the 16-byte managed-header offset (LP64-only) — ✅ DONE & LANDED (main `7ac189fc`)
...
## aarch64 inline refcount fast-path is LP64-only — ✅ DONE & LANDED (main `94f0268f`)
...
`undefined: iota`; iota resolved only once a grouped block had 2+ members
(`const ( A int = iota; B )` → 0, 1). §9.1 `decl.const.iota` says iota is
recognized inside a grouped const block (any group), and Go binds iota in a
single-member group → X=0; the 2+-member requirement was an arbitrary impl quirk.

Root cause: `parseConstDecl` (`pkg/binate/parser/parse_decl.bn`) collapsed a
single-member parenthesized const (`len(decls)==1`) back to a bare `DECL_CONST` —
which is the *non-grouped* form, where iota is undefined. Fix: a parenthesized
const block is ALWAYS `DECL_GROUP` regardless of member count (the grouped form is
what binds iota), distinct from a non-grouped `const X = iota` (still undefined).

Verified builder-comp / -int / -comp: `const ( X int = iota )`→0, `1<<iota`→1;
plain single-member group and non-grouped const unaffected; full builder-comp
2479/0 (no regression); hygiene 15/15. Tests: conformance
`spec/09-declarations-and-scope/006_const_iota_single_member_group` + parser unit
`TestParseSingleMemberConstGroup`; 005's stale "2+-member" comment corrected.

Discovered in passing (filed active in [claude-todo.md](claude-todo.md), pinned
`007_const_group_func_local.xfail.all`): func-local grouped const is entirely
unsupported — statement-level `DECL_GROUP` is unrouted in both `check_stmt` and
IR-gen `gen_stmt`, so members are never defined.
...
There is an open follow-up (delete the now-stale xfail `055_escape_unicode_divergence_xfail`) tracked **active** in [`claude-todo.md`](claude-todo.md) — not here.
...
leg compiles the fixture with the pinned BUILDER (old scheme) directly but linked
`--runtime` at the checkout `runtime/binate_runtime.c`, which after the flip
defines `bootstrap.Write` only under the new-scheme name → `undefined reference to
bn_pkg__bootstrap__Write`. Fix: link the BUILDER's OWN bundled runtime
(`fetch-builder.sh --lib`/runtime) so the BUILDER-scheme compile meets a
BUILDER-scheme runtime — robust to every future mangle flip, and preserves the
leg's intent (exercise the released BUILDER through split -I/-L). Surfaced during
the bnc-0.0.10 release CI triage.
...
- **(h-shr) signedness-aware const-fold — the `/`,`%`,`>>` + relational + grouped + imported family — ✅ DONE & LANDED.** `evalConstIntValue`/`evalConstExpr` folded the signedness-DEPENDENT ops as SIGNED host ints, so an unsigned operand ≥ 2^63 mis-folded (`(2^63)/(2^61)` → -4 not 4). Fixed in lockstep across every fold path via an ExprType value stamp the checker writes and IR-gen reads:
  ...
  - (family complete: (single consts `05d08117`; h-cmp `0625521f`, h-inline-shift `865e2e79`, h-group `beffb741`, h-bni `cf549e2f`). Both adjacent residuals are now ✅ DONE & LANDED: the runtime-count negative-shift variant (`63ecc3b8`, BUG-BASH LANE 2) and the grouped-signedness-const-followed-by-bare-repeat residual (`c5381592`, BUG-BASH LANE 2).
  ...
  - **(runtime-count residual)** ✅ DONE & LANDED `63ecc3b8` (BUG-BASH LANE 2). The RUNTIME-count variant `(0 - 16) >> n` (n a runtime var) lowered to `lshr` because the shift's result type was the negative untyped-const operand's untyped-int type (`Signed=false`). Fix (`gen_binary.bn`): recover the operand's genuine value sign from the checker's folded-literal `LitSign` (not the two's-complement bit pattern, which can't tell a negative const from a positive `≥ 2^63` one) and retag the shift result type to signed int → `ashr`; a positive untyped const keeps `lshr`. Un-xfailed `859_runtime_count_signed_shift` (builder-comp / -int / native_aa64) + an IR-gen unit test on the OP_SHR signed result type.

Remaining open residual (tracked elsewhere): the proper IR-gen transitive-`.bni`-const fix, under the CRITICAL entry.  The other former residuals are now ✅ DONE & LANDED — iota-grouped `.bni` consts (`22065010`), the `parseCharLiteral`/`parseCharLit` decoder dedup (`be3b9661`, into `types.ParseCharLit`), and the multi-byte/empty char-literal leniency (`8644e540`, now rejected per `lex.literal.char.one`; un-xfailed 056/057).  A `\u` spec/impl drift surfaced during this work — tracked in the active todo.
...
## ~~CRITICAL (REGRESSION from `2e783acd`, gap (b)) — a TRANSITIVE `.bni` const silently miscompiles~~ — ✅ FULLY RESOLVED (proper fix landed `505d7069`, 2026-06-20)

**✅ PROPER FIX LANDED `505d7069` (2026-06-20).** The proper fix needed NO IR-gen transitive-const resolution: there is ONE shared checker (`mod.Checker`, set before `RegisterImports`) with every package scope loaded, so `defineBniConst` folds `a.VAL`=7 (`evalConstIntValue`'s `EXPR_SELECTOR` arm via `resolveQualifiedSym`) and STAMPS it on the initializer AST node.  IR-gen already reads that stamp FIRST (`importConstStampVal`, in all three const paths) off the SAME loader-parsed `.bni` AST, before its broken `evalConstExpr` fallback — so checker and IR-gen agree via the stamp, no alias re-resolution.  The fix is simply dropping the conservative `exprRefsSelector` guard added by `8dd35667` (now dead code, removed).  Both `[b.SZ]int` = `[7]` AND `x << b.SZ` = 128 are correct everywhere.  conformance `806` converted from a reject test to a positive 3-package test (array dim + index + shift count); green on builder-comp / -int / -int-int / native-aa64 / gen2 (builder-comp-comp).  Lockstep unit tests drive the REAL registration order (not a pre-seeded const table): checker `TestLoadPackageInterfaceTransitiveConstFolds` + IR-gen `TestRegisterImportTransitiveConstViaStamp`.  Full builder-comp suite 1694/0; hygiene 15/15.  (A related pre-existing `genSelector` silent-0 fallback was surfaced during this work and filed as a separate MAJOR — see claude-todo.md.)

--- Original diagnosis (conservative guard era) below ---
...
## ~~CRITICAL (type-checker / wrong-code) — imports are PACKAGE-scoped, not file-scoped~~ — ✅ FULLY RESOLVED (all wrong-code facets landed; last residual `76027b58`, 2026-06-20)

**✅ FULLY RESOLVED.** Every wrong-code facet of the package-scoped-imports CRITICAL is fixed and
landed: visibility leak + same-alias miscompile (A/B/C/E, `cf0d1cad`); import-alias-vs-decl
redeclaration (D, `973e82f7`); import-cycle detection (H, `a4d8a907`); generic-instantiation path
(`d2a9ff20` + `fcb0dcbc`, separate entry); and the last KNOWN RESIDUAL — a cross-file same-alias
package-level `var x = dep.Foo()` (the combined `__init` body lowered under a merged overlay) — fixed
by a per-statement init-overlay in IR-gen (`76027b58`, conformance `871`, green on
builder-comp / -int / -comp / native-aa64).  (G) was verified a NON-BUG.  Two non-miscompile
follow-ups (the checker's missing unused-import handling — F-checker, tracked under the unused-entity
checks; and an incompatible-signature escalation coverage want) are carried forward to
claude-todo.md.  Original diagnosis below.

--- Original diagnosis ---


**✅ FIX (facets A, B, C, E) — LANDED `cf0d1cad` (2026-06-18).** Direction 1 / option A: imports are now FILE-scoped in BOTH layers. Checker (`checkPackageImpl`) resolves each decl under a per-file import scope (built from `loader.Package.Files` + `pkg.BNI`; `Scope.DelegateDefine` keeps package symbols package-wide). IR-gen (`GeneratePackage`) overlays each decl's file's imports per-decl onto the post-RegisterImports base (`.bni`-prepended decls fall back to the merged set via `declImportFile`). 12 in-tree files that relied on the leak got their missing imports added (incl. the compiler's own `check_expr.bn`→buf, `codegen/emit_*`→mangle, `cmd/bnc/main.bn`→types). Conformance 830 (same-alias → 1,2, was 1,1), 831 (cross-file leak → clean reject), 832 (implicit same-last-segment → 100,200). All green: builder-comp 1495/0, gen2 self-host 1495/0, unit 45/0, repros also pass VM + native-aarch64. See `plan-file-scoped-imports.md`. **KNOWN RESIDUAL:** a same-alias collision in a package-level `var x = dep.Foo()` across files (package-init combines initializers into one body, lowered under the merged overlay) — narrow; tracked in the plan doc. **Facets D (import-alias-vs-decl redeclaration, `973e82f7`) and H (import-cycle detection, `a4d8a907`) — LANDED. NOTE: A/B/C are fixed for NON-GENERIC decls only; the GENERIC-instantiation path still resolves bodies at the use-site (separate CRITICAL entry at the top of this file) — found by adversarial review, NOT covered here.**

**Symptom A (visibility leak — REPRODUCED, builder-comp gen1, bnc-0.0.9).** A package `pkg/xf` with two impl files: `a.bn` does `import "pkg/p1"` and uses `p1.V()`; `b.bn` has NO import of `pkg/p1` yet calls `p1.V()`. The package compiles with NO error and runs (prints `7,7`). Correct behavior: `b.bn` should fail with `undefined: p1` — imports are per-file in every language we model on.

**Symptom B (silent MISCOMPILATION — REPRODUCED, same setup).** `pkg/xf/a.bn`: `import dep "pkg/p1"; func A() int { return dep.V() }` (p1.V==1). `pkg/xf/b.bn`: `import dep "pkg/p2"; func B() int { return dep.V() }` (p2.V==2). Same alias `dep`, DIFFERENT packages, in two files of one package. Expected output `1,2`; ACTUAL output `1,1` — `B()` silently calls `p1.V` instead of `p2.V`. The two files' identically-aliased imports collapse into one package-wide binding, so `dep.V()` in `b.bn` resolves to the wrong package. This is wrong-code generation (no diagnostic), the worst class.

**Root cause.** `checkPackageImpl` (`pkg/binate/types/checker.bn:135`) creates ONE package scope (`pushScope` at :146), then `registerImports(c, merged)` (:163) dumps EVERY file's imports into that single scope, then `collectDecls`/`checkDecls` run over `merged.Decls` (all files concatenated) in that one scope. There is no per-file import scope anywhere. A selector `pkg.X` resolves `pkg` against the single package scope regardless of which file the reference came from (decls carry `Pos.File`, but resolution ignores it). Compounding: `definePkg`→`Scope.Define` (`scope.bn`) silently OVERWRITES a same-named symbol, and `registerImports` (checker.bn:420) has NO duplicate-alias check (it only errors on `unknown package`); `MergeFiles` (`loader/loader_merge.bn`) dedups imports by `(alias,path)`, so same-alias-different-path both survive into `merged.Imports` and both get `definePkg`'d (one wins).

**Discovery.** Surfaced while planning the unused-import `(a)` gap fix + the `(b)-(e)` unused-entity checks (see below): the unused-import lint rule pretends imports are per-file, but the CHECKER registers them package-wide on the merged AST. User flagged package-scoping as a likely bug; two repros (above) confirmed both facets.

**Impact.** Symptom B is a silent miscompilation landmine: any package whose files reuse an import alias for different packages mis-binds. Symptom A erodes import hygiene (a file silently depends on a sibling's import). Believed latent in the current tree (no production package is known to trigger it; full suite green) — but UNSWEPT; a repo-wide check for files-using-unimported-qualifiers / cross-file alias reuse should be part of the fix.

**Proposed fix (options — user to choose).**
- **(1) Proper — file-scoped imports.** Mirror Go's `fileScope → packageScope → universe` chain: keep the shared package decl scope, but layer a per-file import scope and check each file's decls/bodies under ITS file's imports (group `merged.Decls` by `Pos.File`, or retain per-file ASTs — `loader.Package` currently keeps only `Merged`, no `Files`). Resolution of a qualifier then sees only the referencing file's imports. Fixes both facets at the root.
- **(2) Pragmatic — validation pass.** Keep merged checking but ADD: (a) error on duplicate import alias across files binding different paths; (b) error when a package-qualifier reference doesn't resolve to an import in the referencing decl's OWN file. Turns both facets into clean errors (no miscompilation) without restructuring resolution; less invasive, but doesn't make imports truly file-scoped (it bolts the rule on).

**Tests to add (Bug Discovery Protocol).** Multi-package conformance `.error` tests for both facets (use-unimported-qualifier → must error; same-alias-different-package → must error or resolve correctly), xfail until fixed. This is the foundation the unused-import per-file lint rule and the planned `(b)-(e)` checks assume.

**NEW facets — adversarial bug hunt 2026-06-17 (all build-VERIFIED on gen1 `bnc-0.0.9`, IR inspected, unless noted). These materially change the FIX, so the proper fix must address ALL of them in lockstep:**

- **(B) The collapse reaches qualified TYPES → wrong struct symbol + wrong field GEP → MEMORY-LAYOUT CORRUPTION (CRITICAL, the highest-stakes shape, NOT previously characterized).** Same setup as Symptom B but with types: `pkg/ta` has `type T struct { f int }` (`f`@0), `pkg/tb` has `type T struct { p0 int; p1 int; f int }` (`f`@2); `a.bn`: `import dep "pkg/ta"; func UseA() int { var v @dep.T = dep.Make(); return v.f }`; `b.bn`: `import dep "pkg/tb"; func UseB() int { var v @dep.T = dep.Make(); return v.f }`. Expected `111,999`; ACTUAL `111,111` — `UseB` calls `ta.Make` and GEPs `f` at index 0 into `%bn_pkg__ta__T` where the real value (`tb.T`) is at index 2. With mismatched layouts this reads/writes the wrong offset (memory-safety hazard, not just a wrong value). Root: `@dep.T`'s struct name + `dep.Make`'s symbol both expand `dep`→first-write-wins (`ir/gen_import.bn:147`, `ir/gen.bn:158-164`, `buildQualName` `ir/gen_util_literals.bn:36`; type-qualified path `ir/gen_selector.bn:308`).

- **(C) IMPLICIT same-last-segment trigger — no explicit alias needed (CRITICAL, the most LIKELY real-world trigger).** `a.bn`: `import "pkg/alpha/widget"; func A() int { return widget.Code() }`; `b.bn`: `import "pkg/beta/widget"; func B() int { return widget.Code() }`. Expected `100,200`; ACTUAL `100,100`. Implicit imports carry `Alias=""`, so `hasImport` dedups on `("",path)` and both survive; the collision is manufactured when `pushFileImports` sets `alias = lastPathSegment = "widget"` for both (`ir/gen_import.bn:78-81`) and `RecordImportPath` keeps the first (`:147`); checker derives the same implicit alias (`checker.bn:438-440`). NOTE: this defeats the landed same-last-segment fix (`e201f448`/`2122648`, which keys cross-MODULE IR resolution on full path) *within one module*, because IR-gen overlays the merged file's implicit aliases together.

- **(opposite-winner split — load-bearing for the fix) The checker binds the alias LAST-write-wins (`Scope.Define` overwrites, `scope.bn:27-36`) while IR-gen binds it FIRST-write-wins (`RecordImportPath` early-return, `ir/gen_import.bn:144-153`).** They pick OPPOSITE winners. Consequence: a checker-only validation pass (fix option 2) is INSUFFICIENT — any program that slips the checker still mis-mangles in IR-gen. Both layers must be fixed together. **(E)** build-verifies the split: `a.bn`/`dep`→`pkg/p1` using `dep.Foo()` (Foo only in p1), `b.bn`/`dep`→`pkg/p2` using `dep.Bar()` (Bar only in p2) → spurious `undefined: Foo` (checker bound `dep`→p2 last-wins) while IR-gen would have mangled to p1.

- **(D) Import alias vs package-level decl of the same name: NO "redeclared" diagnostic (MAJOR, NEW — ✅ BUILD-VERIFIED 2026-06-17 on gen1 `bnc-0.0.9`).** `import p "pkg/p1"` + `func p() int {...}` in the same package: `registerImports` defines `SYM_PKG` (`checker.bn:166`) BEFORE `collectDecls` (`:169`), both via the overwriting `Scope.Define`, and `checkDuplicateDecls` (`check_decl.bn:26`) compares decls-to-decls and never sees the `SYM_PKG`. Result: no redeclaration error; `p` ends bound to the func, so `p.V()` mis-resolves to field/method handling. **Repro:** `import p "pkg/p1"` (p1 has `func V()`) + `func p() int { return 5 }` + `main { println(p()) }` → compiles clean, prints `5` (p = the func; the import is silently shadowed, no diagnostic). NOT fixed by per-file imports for the same-file direction — needs an explicit import-alias-vs-decl conflict check.

- **(F-checker) The checker has ZERO unused-import handling (MAJOR, NEW).** The only check is the opt-in `bnlint` rule (`lint/unused_import.bn`), whose per-file attribution rests on the false premise that imports are file-scoped — so it has a false-POSITIVE corner (Symptom-A leak: package genuinely uses an import via a sibling file, but the lint attributes use per-file and reports "unused") and a false-NEGATIVE corner (`walkExprRefs` records `e.X.Name` for any bare-ident selector base without checking it's an alias, so a local var shadowing an alias masks a genuinely-unused import). Entangled with the scoping decision and the planned `(a)-(e)` work below.

- **(G) Cross-file top-level redeclaration of a non-`.bni` decl — ✅ VERIFIED NON-BUG (2026-06-17, code-read).** `checkDuplicateDecls` (`check_decl.bn:26`, called at `:180` with the MERGED decl list) compares each decl against all earlier ones, so two `func F(){}` in different `.bn` files of one package ARE caught ("redeclared in this block"). It skips `DECL_TYPE` (types go through the separate `checkTypeRedeclaration`) and `.bni`→`.bn` forward+body (correct). The earlier "within same file" worry was a misread — it runs on the package-merged list. Not a bug.

- **(H) Import cycles: detection machinery is DEAD CODE → cycles surface as confusing cascading errors, no clean diagnostic (MAJOR, NEW — ✅ BUILD-VERIFIED 2026-06-17 on gen1 `bnc-0.0.9`).** The loader HAS a `loadingStack`/`isLoading` cycle check (`loader.bn:105-167`), but it is non-functional: `loadPackage` calls `popLoading(path)` (`:390`) BEFORE recursing into the package's dependencies (`:393-399`), so by the time any dep is loaded the parent is already off the stack and `isLoading` never fires. The `GetPackage()!=nil` dedup guard (`:395`) prevents infinite recursion, so a cycle isn't a hang — but no `import cycle` error is ever emitted. **Repro:** `pkg/ca/ca.bn` imports `pkg/cb` and `pkg/cb/cb.bn` imports `pkg/ca` (mutual) → NO `import cycle` diagnostic; instead the topo-order breaks down and the checker emits misleading cascades (`unknown package pkg/ca` in cb.bn, then `undefined: ca`, `cannot call non-function`). Fix: make cycle detection real — either keep `path` on the loading stack across the dep recursion (move `popLoading` after the loop) and emit a clean `import cycle: A -> B -> A` chain, or add an on-stack set to `computeOrder`'s `visit` DFS (`:431`, which today only has a `visited`/done set, no cycle guard). Add a conformance `.error` test. Whether intra-package cycles (which Binate's merged-package model may make legal) differ from inter-package cycles needs a spec decision.
...
## Embeddable interpreter API (Inc 1 + Inc 2 extern cleanup + loader de-rooting) — ✅ CORE DONE & LANDED (2026-06-20)

The `@interp.Interp` embeddable whole-program API and the extern-registration cleanup behind it. Landed:

- **Inc 1** — `5bdd76b6` (machinery → `pkg/binate/interp`, errors as values), `596fb872` (the `@Interp` facade: `New` / `AddBniPath` / `AddImplPath` / `LoadProgram` / `RunMain` / `RunFunc`); cmd/bni's `runProgram` collapsed onto it.
- **Inc 2 extern cleanup** — Layer 1 (`71748fa4`): the standard-set POLICY moved out of `pkg/binate/vm` into the host (`pkg/binate/interp`); the VM keeps only the registration MECHANISM (`RegisterPackageFunctions/Globals/Vtables` + `RegisterVmTrampolines`); the double `pkg/bootstrap` registration eliminated; vm's own tests get a local `registerTestExterns` (can't import interp — cycle); repl routes through interp. Layer 2 (`c843eab7`): `New(stackSize, pkgs @[]@reflect.Package)` takes the swappable library inject-set, defaulting to the newly-exported `StandardPackages()`; engine substrate (rt/reflect/bootstrap/trampolines) stays fixed. Review (b)-fix (`14bf3f43`): `@Interp.isCompiled` derives the lowering skip from the actual inject-set (so "what is injected" and "what is lowered" can't diverge), plus a `RegisterPureCExterns` unit test and doc-staleness fixes.
...
## Moved 2026-06-21 (mangling migration + reflect fix/rename session)
...
## MINOR (test-coverage) — gaps from the 2026-06-20 adversarial review of the session's generic/reloc work — ✅ DONE & LANDED (`dc04f824`; named-axis injectivity guard delivered by mangling 4a `a0a0ea80`)
