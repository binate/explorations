...
Remaining open residuals tracked in the active todo: the `>>` const-fold signedness-unification (h-shr — `bignum` would fold `>>` logically but the const-value-validation + IR-gen folds are arithmetic, so `>>` stays on the host-int fold for now), iota-grouped `.bni` consts, `parseCharLiteral`/`parseCharLit` dedup, multi-byte char-literal leniency, and the proper IR-gen transitive-`.bni`-const fix.
