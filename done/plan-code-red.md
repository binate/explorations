...
## 7. Test matrices (a family, one per recurring class)

Coverage is the third leg of the thesis. The systematic coverage is a **family of matrices**, one per recurring defect class — each a Cartesian product over its *own* axes, with its *own* **correctness assertion** (a refcount-balance check can't see a wrong *value*; a value-correctness check can't see a leak). All live under `conformance/matrix/<class>/…`, coordinate-addressed (the path is the test's identity → stable, collision-free, idempotently regenerable by a per-class generator), discovered recursively by the runner. One-off / long-tail bugs that aren't a systematic product go in `conformance/regressions/` instead (Section 8). Every cell runs in all relevant modes (§7.3).

**Realized so far:** the **refcount matrix** (§7.1, the dominant gap) is built — `conformance/matrix/refcount/<form>/<shape>/<type>`, generated by `conformance/gen-matrix.py` (Python now; intended to port to Binate/dogfood later), ~76 cells. It has already confirmed **three latent memory-corruption defects** that ad-hoc tests missed: for-range value-bind over-release (double-free), single short-var managed-struct under-copy (double-free), and the `@func` call-result discard leak. The other class matrices (§7.1b) are planned, reusing the same infrastructure.

### 7.1 The refcount matrix — `{form × shape × type}` for managed values (Class 1)
...
### 7.1b The other class matrices (realized)

Same infrastructure (coordinate-addressed subtree + per-class generator + `.xfail` tracking), different axes + assertion, each a sibling under `conformance/matrix/`. All three are now built:

- **`scalar/`** — sub-word & 64-on-32 **value correctness** (Class 5). Axes `op × width × signedness`; operands chosen to expose dirty-upper-bit / sign defects (`(a*b) >>u n`, `cast(uint64, a*b)`, unsigned compare/div, int↔float round-trip). Assertion: the computed value equals a target-independent reference. **Realized** (`gen-scalar-matrix.py`); systematically covers the **confirmed dirty-upper-bits + unsigned-int→float** class (§3.8 / §8) — invisible to refcount-balance. Faults on the host VM + natives already; arm32 lanes via CI.
- **`abi/`** — aggregate & multi-return passing (Class 4). Axes: categories `multi-return / struct-return / struct-param` plus the call-shape axis `iface-param / funcval-param` (the same struct by value through an indirect call). Assertion: every field's bytes/values survive. **Realized** (`gen-abi-matrix.py`); found the x64 sub-word multi-return drop, aa64 non-8-multiple tail-drop, and — via the call-shape axis — a **CRITICAL LLVM SIGSEGV** passing a >16-byte struct by value through an indirect (iface-method / func-value) call (direct calls are fine). The remaining call shapes are closed: `handle` is not user-expressible (`OP_CALL_HANDLE` is compiler-internal dtor/free dispatch; the func-value shape already covers indirect aggregate passing), and `__c_call` is realized as `conformance/regressions/c-call/` (found a CONFIRMED native variadic-float ABI bug — printf reads 0; libc-self-contained cells).
- **`const/`** — compile-time constant materialization (the named-constant invariant, §5; not taxonomy Class 6, which is `@Iface` lifecycle). Axes `read-form × type × value`, where read-form = var-init / named-const / const-group / local-const / call-arg / field / return. Assertion: the constant materializes to the exact bit pattern of its type (printed as target-stable 16-bit lanes). **Realized** (`gen-const-matrix.py`); the read-form axis found a **CRITICAL** front-end gap (an untyped float literal is not narrowed to `float32` at call-arg / composite-field / return — silent wrong value, all backends) and a **MAJOR** local-const-materializes-0; re-confirms the DEFERRED float32-const (539/844) and float64-tie (538/777) VM/native divergences across read-forms. The companion **const-expr-folding** axis is realized as `conformance/regressions/const-expr/` — integer arith/bitwise/paren/const-of-const fold, but non-int const-*expressions* (binary float, bool comparison) and const-as-array-dimension drop to int 0 (CONFIRMED, all backends; the literal cases were Phase-A-fixed). An open language question remains (bare const-group member = plain iota vs Go-style repeat-expression).
...
**DECIDED + realized:** option 2 (a dedicated, coordinate-addressed subtree) won, for the *stability* reason — a path-as-identity name never collides and never needs renumbering (the recurring pain), and it makes the per-class generator idempotent (same coordinates → same path; adding a dimension value creates new paths, touches nothing existing). Layout is **class-level**: `conformance/matrix/<class>/<axes…>` (§7 intro). The runner got one additive change — recursively discover `*.bn` under `conformance/matrix/` and `conformance/regressions/`, naming each test by its path relative to `conformance/`, so `.expected`/`.xfail.<mode>` resolve as siblings. The generator owns `.bn` + `.expected` and never touches `.xfail.*`. The discussion that led here is preserved below.

The matrix is large, systematic, and *generated by a product of axes* — it must not be sprinkled into the flat `conformance/NNN_*` numbering as ad-hoc one-offs, or coverage becomes unauditable (a current failure mode: nobody can see which cells exist). The systematic suite should be its own organized body of tests with a checked-in coverage manifest. Three structural options that were on the table:
...
> `done/plan-cr-p2.md` (index) and `plan-cr-p2-{1-frontend,2-refcount,3-abi,4-scalar}.md`.**
