...
**Part B (type-assertion / RTTI)** landed as `conformance/matrix/type-assert/` (22
cells; core in `f068c851e`, generic-instantiation value target in `f149af5f9`;
generator `conformance/gen-type-assert-matrix.py`).
...
records are in `claude-todo-done.md`).  What remains: Part B's later waves
(type-switch narrowing grid, struct recovery from `@I`); Part A's cross-package
dispatch variants and `copy` / `destroy-populated` ops.
