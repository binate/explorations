...
Binate is a systems programming language built around a core idea: code should be able to run compiled _and_ interpreted, with full interoperability between the two modes. A compiled binary can call into an embedded interpreter, and interpreted code can call compiled code — no special glue required.
...
- **Self-hosting** — the long-term goal is for the compiler and interpreter to be written in Binate itself.
