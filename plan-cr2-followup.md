...
❌ **REVERTED — MISDIAGNOSIS** (landed `e508c841`, reverted `71bf2b2a`, 2026-06-09). The premise is WRONG: extern + Binate body is a *legitimate* pattern — VM **trampolines** (e.g. `pkg/binate/vm.TrampolineScalar`, vm.bn:71) are intentionally BOTH a registered extern AND a Binate body (the body is the implementation; the extern registration exposes it to bytecode). The hard-error guard false-positives: when the inner VM lowers `cmd/bni` itself — which only happens in `builder-comp-int-int` — it hits `TrampolineScalar` and exits, **breaking the entire int-int lane for every test**. (The "no-false-positive, builder-comp-int 1263/0" check missed it: single-VM lowers the TEST module, not `cmd/bni`; and int-int was dead at the time so the regression was invisible.) There is no "silent shadowing bug" to fix here. Do NOT re-implement without first establishing that an *accidental* (non-trampoline) extern+body collision is a real, occurring failure mode.
