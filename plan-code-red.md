...
- **`const/`** — compile-time constant materialization (Class 6). Axes `read-form × type × value`, where read-form = var-init / named-const / const-group / local-const / call-arg / field / return. Assertion: the constant materializes to the exact bit pattern of its type (printed as target-stable 16-bit lanes). **Realized** (`gen-const-matrix.py`); the read-form axis found a **CRITICAL** front-end gap (an untyped float literal is not narrowed to `float32` at call-arg / composite-field / return — silent wrong value, all backends) and a **MAJOR** local-const-materializes-0; re-confirms the DEFERRED float32-const (539/844) and float64-tie (538/777) VM/native divergences across read-forms. A const-expr-folding axis remains (a probe confirmed `const X float64 = 0.1+0.2` → invalid IR).
