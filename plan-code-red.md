...
### 7.1b The other class matrices (planned)

Same infrastructure (coordinate-addressed subtree + per-class generator + `.xfail` tracking), different axes + assertion, each a sibling under `conformance/matrix/`:

- **`scalar/`** — sub-word & 64-on-32 **value correctness** (Class 5). Axes `op × width × signedness`; operands chosen to expose dirty-upper-bit / sign defects (`(a*b) >>u n`, `cast(uint64, a*b)`, unsigned compare/div, int↔float round-trip). Assertion: the computed value equals a target-independent reference. This is where the **confirmed-but-untracked dirty-upper-bits + unsigned-int↔float** class (§3.8 / §8) gets systematic coverage — structurally invisible to refcount-balance. Likely needs the missing native + 32-bit-VM lanes to bite fully, but several cells fault on the host VM already.
- **`abi/`** — aggregate & multi-return passing (Class 4). Axes `aggregate-shape × {param, return} × call-shape (direct / iface-method / func-value / handle / c-call)`. Assertion: every field's bytes/values survive. Targets the x64 multi-return n=2 drop, aa64 tail-drop, iface-method byval.
- **`const/`** — constant materialization. Axes `const-type × scope (pkg / group / local) × read-form`. Assertion: the materialized value is correct. Targets local-const-group, float32-const, large-const-on-ILP32.
