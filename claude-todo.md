...
- **Scope**: `-int` modes (bni build) CONFIRMED broken. gen2 (`builder-comp-comp`/-comp-comp/-comp-comp-comp) appeared to still build (UNCONFIRMED — possible cache reuse). Native modes unaffected (separate `pkg/binate/native` backend). `builder-comp` (single LLVM stage, simple cells) works.
...
### `readonly` / `const` type modifier is broken for managed values — Symptom 1 (silent field read) FIXED (binate `27c1ee8b`); Symptom 2 (method dispatch compile error) still OPEN
