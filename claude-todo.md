...
- **STILL OPEN — do not lose these**:
  1. **Struct/array equality implementation** — ✅ IMPLEMENTED (920a; commit on `bugbash-lane2`, pending land approval). A struct/array is comparable iff every transitive field/element is (`aggregateComparable`, `types_query.bn`); one transitively containing a slice/iface/func is rejected ("this struct/array is not comparable"), all-comparable aggregates lower field/element-wise (`gen_eq_aggregate.bn`: materialize both operands to stack scratch, walk fields via `EmitGetFieldPtr` / array elements via byte-offset `EmitGetElemPtr`, one `OP_EQ` per scalar/pointer leaf conjoined with `OP_LAND`, `OP_NOT` for `!=`; recurses for nested aggregates). No refcount traffic (a `@T` field compares by identity). Verified on LLVM + VM + native aa64 + native x64. Tests: un-xfailed `spec/13-expressions/015`, new `049` (nested struct + array-of-struct), `050`/`051` (reject struct-with-slice / array-of-slice), `052` (managed-`@T` field, refcount-balanced), `07-types/044` (repurposed to positive named-array), `12-generics/067` (generic + comparable struct compose) / `068` (generic + struct-with-slice reject); matrix `eq-reject/{struct,array}` regenerated; `gen_eq_aggregate_test.bn` units. Adversarially reviewed — no defects in the lowering. Also removed the now-stale `772_generic_eq_managed_slice.xfail.all` (920b made the direct `eq[@[]int]` case a clean XPASS reject). **Exposed one MAJOR follow-up gap → item 4 below.**
  ...
  4. 🏷[BUG-BASH 2026-06-27 → LANE 2] **Generic AGGREGATE-FIELD comparability NOT re-checked at instantiation (MAJOR — newly reachable since item 1; surfaced to user 2026-06-29 for fix-now-vs-defer).** A generic struct `Box[T] struct { v T }` (or a `[N]T` field) compared with `==` inside a generic function type-checks — the field `v: T` is opaque/comparable at the generic definition — but the 920b instantiation re-check (item 2) only re-validates a DIRECT type-param operand, NOT a type-param buried in a generic aggregate. So `cmp[@[]int]` over `Box[T]` reaches IR-gen: an invalid aggregate `icmp` on the LLVM backends (a loud clang failure) and a **SILENT first-word-only managed-slice compare on the bytecode VM** (wrong result — observed `cmp[@[]int]` printing `false` instead of a clean rejection). Before item 1, the generic body's `a == b` on `Box[T]` was rejected outright (struct `==` unimplemented), so this was UNREACHABLE; enabling struct/array `==` exposes it. **Pinned:** `spec/12-generics/069_err_generic_aggregate_field_slice.xfail.all` (ideal behavior = the clean "this struct is not comparable" rejection; currently fails on LLVM via clang `icmp requires integer operands` + VM via wrong `false`). **Proper fix:** at instantiation, re-run `checkEqOperands` / `aggregateComparable` on the SUBSTITUTED operand TYPE of each body comparison — i.e. record the compared OPERAND TYPES (not just bare type-params) in `recordTypeParamComparison` and re-validate their substituted forms in `instantiateGenericFunc`; OR a post-substitution checker pass over the instantiated body's comparisons. The array-field variant (`struct { items [N]T }`) has the same gap.
