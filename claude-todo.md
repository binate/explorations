...
## CRITICAL (front-end / silent miscompilation) — user `func _Package()` collides with the compiler-synthesized per-package reflect accessor (`IsReservedIdentifier` misses the single-underscore namespace) (2026-06-26) — 🔴 OPEN

**Symptom.** A user declaration named exactly `_Package` (a free function, and
likely a global/struct too) mangles to the *same* linker symbol as the
compiler-synthesized package-introspection accessor the reflect feature emits for
EVERY package. **Reproduced** (adversarial review, full bnc build): compiling
`package "main"; func _Package() int { return 42 }` emits TWO defines of
`@bn_F1_4_main1_8__Package` — the user's `i64` func and the synthesized `ptr`
accessor — with exit 0 and **no diagnostic**. Duplicate symbol / wrong-code.

**Root cause.** `mangle.IsReservedIdentifier` (mangle.bn:115-120) reserves only
`__`-prefixed names and names containing `__bn_inst__`. The accessor is emitted
via the *same* public mangler a user func uses (`mangle.FuncName(pkg,
"_Package")` — codegen emit_pkg_descriptor.bn:28, native x64_pkg_descriptor.bn:24
→ symFor → FuncName; IR name set in ir/gen_import.bn:45). The mangler is purely
structural (no source-vs-synthesized distinction), so a user `_Package` decl
mangles identically. The sole front-end guard (`checkReservedDeclNames`,
check_decl.bn) delegates entirely to `IsReservedIdentifier`. **Pre-existing**
(predates the reserved-name work cc1171e7), but cc1171e7 *codified the wrong
rationale*: mangle.bn:113 claims single-underscore `_Package` names "are emitted,
not parsed, so they never reach this check" (false — a user SOURCE `_Package`
decl does reach the check), and mangle_test.bn:276 asserts
`IsReservedIdentifier("_Package")` is false, and spec lex.ident.reserved says a
single leading underscore is never reserved.

**Proposed fix (user's call).** (1) Narrow: `IsReservedIdentifier` also returns
true for an exact `_Package` (audit for any OTHER single-underscore synthesized
exported names routed through the public manglers first); or (2) Cleaner: rename
the synthesized accessor into the already-reserved `__` namespace (e.g.
`__Package`) — touches ir/gen_import + both backends + the call-side `_Package`
resolution in check_expr*.bn. Either way: flip mangle_test.bn:276, correct the
mangle.bn:112-114 rationale + spec lex.ident.reserved, and add a conformance
negative test beside 893. Found by the 2026-06-26 adversarial review of the
mangling/reserved-name/reflect work.

## MAJOR (front-end coverage gap) — REPL top-level `type __T` bypasses the reserved-identifier check (2026-06-26) — 🔴 OPEN

`checkReservedDeclNames` runs only in `collectDeclsBody`. The REPL entry
`CheckDeclInScope` (checker.bn:352) special-cases `DECL_TYPE` (preRegisterTypeNames
+ checkDeclTypeTentative, then RETURNS at ~388) WITHOUT calling collectDecls — so a
`type __T …` typed at the REPL prompt is NOT rejected (every other REPL decl kind
falls through to collectDecls and IS checked). Fix: call `checkReservedDeclNames(c,
single)` in CheckDeclInScope before the DECL_TYPE branch (covers all kinds once).
Add a REPL unit test. Found by the 2026-06-26 adversarial review.

## MAJOR/MINOR (sweep incompleteness) — the old-mangling-scheme comment sweep (3c1e3da6) skipped several file types; concrete old-folded symbols survive (2026-06-26) — 🔴 OPEN

The sweep grepped only `*.bn` under `pkg/ cmd/ impls/` (and excluded `*_test.bn`),
so old-scheme illustrations survive in file types it never covered — the *same*
enumeration-breadth lesson (now sharpened in CLAUDE.md), this time on file-type
coverage. Confirmed survivors:
- **.bni (MAJOR):** `reflect.bni:71` has a FULLY-SPELLED concrete old symbol
  `__ivt.bn_pkg__std__strings__Builder__pkg__std__io__Writer`; `common.bni:262`
  similar. → `__ivt.bn_V…`.
- **conformance/**/main.bn (MINOR):** 867 (`bn_pkg__genlib__Pick__bn_inst__int` →
  `bn_I…`), 785 (`bn_pkg__aa__thing___Package` → `bn_F/bn_S…`), 408 (`@bn_F…`).
- **\*_test.bn (MINOR/NIT):** emit_copy_ssa_load_test.bn:33 (comment contradicts its
  own `%bn_S…` assertion), aarch64_dispatch_test.bn:37 (`bn_main__main` →
  `bn_F1_4_main1_4_main`), emit_impls_test.bn:9, asm_test.bn:73, three c_call test
  comments (`bn_<pkg>__` → `bn_<kind>…`).
- **.py (MINOR):** conformance/gen-globals-matrix.py:93-94 (`%bn_main__S` → `%bn_S…`).
- **.md (OBSERVATION):** ifaces/core/README.md:11 forward-references a future BUILDER
  with the old `bn_pkg__builtins__rt__…` prefix.
Also an ACCURACY defect introduced BY the sweep: ir_ops_flow.bn:86 now reads
`…not a Binate \`bn_F...\` (a mangled func symbol)).` — a doubled `))` typo, 88
cols. Found by the 2026-06-26 adversarial review.

## MINOR (test coverage) — `swapIfaceSuffix` iface-PACKAGE-mismatch return path (vm_exec_iface.bn:385) is untested (2026-06-26) — 🔴 OPEN

The two defensive source-match returns (line 385 = iface PACKAGE mismatch, line
386 = iface NAME mismatch) — only 386 is exercised (TestSwapIfaceSuffixSourceMismatch
matches the package and mismatches the name). Add a test with a matched iface NAME
but mismatched iface PACKAGE (e.g. `pkg/fmt.Writer|pkg/io.Reader` against a
...
### MAJOR PROJECT — unify module-level static data into one IR representation (`ir.DataGlobal`) + one per-backend emitter — FILED 2026-06-10 (🟢 PHASE 1 + INC 2 + INC 3a + INC 3b + INC 4a/4b/4c LANDED, phased migration in progress)
...
- **Not blocking**: the `ir.DataGlobal` string migration (Inc 4c/4d) completes without it; the `.ms` header just stays in `data` until this lands.
