...
- **🟠 MAJOR — `func f() RS { return <managed-slice> }` over a named RAW-slice fails to COMPILE (found 2026-06-12 audit; NOT fixed; user chose fix-as-follow-up)**: with `type RS *[]int`, returning a `@[]int` value where the declared result is `RS` produces invalid LLVM `ret %BnManagedSlice` against result type `%BnSlice` → the LLVM verifier rejects it (a LOUD compile error on checker-accepted code, not silent corruption — hence major, not critical). Root cause: the `@[]T→*[]T` conversion in `genReturnStmt` is gated on the UNPEELED declared-result kind — `retTyp.Kind == types.TYP_SLICE` (`gen_return.bn:93`, single-value path) and `resTyp.Kind == types.TYP_SLICE` (`gen_return.bn:60`, `return f(...)` multi-return arm) — both false for a named raw-slice result, so `EmitManagedToRaw` is skipped. **Fix**: peel (`peelTransparent`) the declared result type before those two `== TYP_SLICE` tests, mirroring the line-124 peel landed in `672d884d`. Repro 992 (verified): `func view(s @[]int) RS { return s }` + `var rs RS = view(m)` → COMPILE_ERROR. **Audit context — 4 sibling sites were FALSE POSITIVES** (the unpeeled `.Kind` check is redundant, downstream `emitStoreManagedSlot`/store paths already peel; all compile+run correctly): `gen_stmt.bn:301` (`var rs RS = mslice`), `gen_composite.bn:108` (named-raw-slice struct field init), `gen_control.bn:133` + `:211` (plain assignment, named source or target). Their unpeeled checks could be tidied for consistency but are not bugs. Add a conformance test for the return case (currently no coverage for named raw-slice returns).
- **Minor follow-up (checker coverage)** 🟡: `checkCompositeLit` (`pkg/binate/types/check_expr_composite.bn:39`) only validates ELEMENTS for a named-over-struct wrapper, not a named array/slice — so `Row{1, "x", 3}` is not type-error-flagged (harmless for codegen — the IR fix lowers whatever the checker accepts — but a missing diagnostic). Extend the checker's `TYP_NAMED` arm to array/slice underlyings.
