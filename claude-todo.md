...
### bnc codegen: byval-spill alloca emitted at call site leaks per loop iteration — `*-int*` unit-test modes overflow on long-running loops (e.g. bni VM dispatch)
- **Symptom**: under every `builder-comp-int*` / `builder-comp-comp-int` unit-test mode, the bni VM segfaults mid-test (`EXC_BAD_ACCESS` on macOS, SIGSEGV on Linux) for any package whose tests drive long interpreted runs.  Test runner prints `=== RUN <Test>` then silently exits (the test process crashes; the runner reports the package FAILED).  Affected packages (sampled from CI run `26795575927`): `pkg/binate/types`, `pkg/binate/parser`, `pkg/binate/ir`, `pkg/binate/codegen`, `pkg/binate/lint`, `pkg/binate/native/common`, `pkg/binate/native/aarch64`, `pkg/binate/native/x64`, `pkg/binate/asm/{macho,parse,aarch64,elf}`, `pkg/binate/buf`, `pkg/binate/debug`, `pkg/binate/lexer`, `pkg/binate/vm`, `pkg/bignum`, `pkg/bootstrap`, `pkg/builtins/{lang,testing}`, `cmd/bnc`, `cmd/bnas`.  Native-compiled modes (`builder-comp`, `-comp-comp`, `-comp-comp-comp`, `_native_*`) all pass.
- **Root cause (refined 2026-06-02)**: NOT extern-callback recursion as initially suspected.  bnc's `writeByvalArgPreamble` (in `pkg/binate/codegen/emit_util.bn`) emits a per-byval-arg `alloca <T>` + `store` at the CALL SITE basic block, not at the function entry block.  Inside a long-running loop (e.g. bni's `execLoop`), each iteration that takes a branch with a byval-spill emits a fresh host-stack alloca that **only releases when the enclosing function returns**.  After ~165K iterations on the 8 MiB default stack with a 48-byte `BCInstr` byval spill, host stack is exhausted and the next call's prologue faults.
  - lldb backtrace at crash shows only ~5 frames deep (NOT recursive); SP sits ~8 MB below FP — single execLoop frame consuming the full stack budget through accumulated per-iteration leaks.
  - Disassembly of `pkg/binate/vm/vm_exec.bn::execLoop` shows 13 distinct `mov w15, #0x30; ...; mov sp, sp - x15; ...; str x?, [sp + N]` patterns (= 48-byte alloca + 6 i64 stores = BCInstr).  IR confirms 13 byval-spill allocas with names like `%v91.bv3`, `%v107.bv1` etc. — one per call site that passes `instr` by value to a helper (`execStringOp`, `execFuncRefOp`, `execMemoryOp`, `execArithOp`, `execCmpOp`, `execUnaryOp`, `execOp64`, `execNumericCast`, ...).  Each is in its own basic block inside the for-loop, not in the entry block.
- **Pre-existing**: the `Unit tests` CI workflow has been failing for at least 982 runs (since 2026-05-18).  The team already hand-hoisted ONE leaking allocation (`var callArgs @[]int = make_slice(int, 64)` in `execLoop`) — see the comment at `vm_exec.bn:24-36`.  This bug class is broader: every call site passing an aggregate by value inside a long-running loop has the same shape.  Cleanest fix is in bnc codegen, not in the source (one-line workarounds per call site would compound the maintenance burden).
- **Discovery**: 2026-06-02.  `pkg/binate/types/TestLoadPackageInterfaceTypeDecl` reproduced locally; lldb backtrace:
  ```
  ...
  ```
  Fault address sits below the macOS-arm64 8 MiB stack region (`0x16f800000`–`0x16fffffff`).  Backtrace shows ONLY ~5 frames; the host stack between the (top) execLoop frame's FP and SP is ~8 MB, far more than execLoop's own 2304-byte stack frame.  The mismatch is the smoking gun for "accumulated leaks within one execLoop invocation," not recursion.
- **Why MAJOR (not CRITICAL)**: this is **pre-existing** — the `Unit tests` CI workflow has been failing for at least 982 runs (since 2026-05-18 visible in the run list).  Bytecode-VM CI lanes have never been green on the current bni surface.  Doesn't block native CI (`builder-comp`/`-comp-comp`/`-comp-comp-comp` are green); doesn't block conformance (single-test binaries finish before accumulating enough leak to overflow).  Production codepath unaffected — compilation goes through native, not the VM.
- **Verification**: `ulimit -s 32768` (4× default) makes the first ~3 type tests pass; `ulimit -s 65520` (8×, macOS hard cap) gets ~40 more before a deeper test still crashes.  Linear with stack budget — exactly the shape expected for a "leak Y bytes per iteration; iterations × Y exhausts S bytes of stack" pattern.
- **Band-aid landed**: `ulimit -s 65520` in the three bni-using runner scripts (`builder-comp-int.sh`, `builder-comp-int-int.sh`, `builder-comp-comp-int.sh`).  Capped at 64 MiB (an earlier `unlimited` attempt turned crashes into multi-minute hangs on Linux CI).  Makes more tests pass on both platforms; the deepest tests still crash but quickly.  Remove this band-aid once the codegen fix lands.
- **Proper fix**: fix `pkg/binate/codegen/emit_util.bn::writeByvalArgPreamble` to emit the byval-spill alloca in the function's ENTRY block, not at the call site.  LLVM's standard idiom is "allocas in entry block"; bnc emitting them at the call-site basic block is the codegen bug.  See [`plan-codegen-byval-spill-hoist.md`](plan-codegen-byval-spill-hoist.md).
