...
- **Tests covering it**: pkg/binate/version's tests pin the string case end-to-end through both in-package and cross-package reads.  No coverage for bool / float / composite / pointer cases yet — would benefit from a focused gen_const-style unit-test suite + a `pkg/binate/ir/gen_const_nonint_test.bn` covering each value-bearing op.
- **Status**: partial fix landed (string only) in binate `7b0f77a3` (commit `a5acfc45` on the worktree side).  Follow-up plan: see plan-const-nonint.md (forthcoming) for bool/float (scalar non-int — easy mechanical extension) and the composite-vs-restriction design call for arrays/structs/slices/pointers.
