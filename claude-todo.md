...
### `SignedMinForWidth` shift count not cast to int64 → `mismatched types int64 and int` breaks ALL 32-bit-int compilation — REGRESSION from `efeb0f94` — 🔴 OPEN (bnc-0.0.8 release blocker)
- **Symptom**: `pkg/binate/types/types_query.bn:168` is `var shifted int64 = cast(int64, 1) << (width - 1)`. In 32-bit-int target modes the shift count `(width - 1)` is `int` (32-bit) while the shifted operand is `int64`, so the checker rejects it: `mismatched types int64 and int`. Because `types_query.bn` sits in nearly every package's transitive dependency, the single error cascades — arm32 unit + conformance fail to compile (`FAIL: pkg/binate/{lint,ir,codegen,types,vm,native/*,repl}, cmd/{bnc,bni,bnlint}`). Compiles fine in 64-bit-int host modes (where `int`'s width == `int64`), which is why every `-comp*` mode stayed green and the break was invisible to the green legs.
- **Baseline / regression proof**: `builder-comp_arm32_baremetal` Unit was **green at bnc-0.0.7** (commit `ee06ec87`, job `success`); it is **red at `ac738936`**. The offending line landed in `efeb0f94` (2026-06-05, the integer divide/remainder fault-guard work), after the 0.0.7 tag (2026-06-04) → in-window regression.
- **Severity**: MAJOR — breaks all 32-bit-int compilation (the primary target class) on a previously-green gate mode. Loud compile error, not a silent miscompile (hence MAJOR not CRITICAL). **Blocks cutting bnc-0.0.8** per release-process.md's "modes where bnc-0.0.<prev> was green" gate rule.
- **Root cause**: missing cast on the shift count. Binate's established convention is that a shift count carries the shifted operand's type — every other shift site casts it (`cast(uint64, 1) << cast(uint64, size)`, `sign << cast(uint32, 31)`; `cast(int64, 1) << 63` works because `63` is an untyped literal that adapts). This is the only shift whose count is a plain `int` variable. NOT a type-checker / semantics issue.
- **Fix (NOT a semantics change)**: `var shifted int64 = cast(int64, 1) << cast(int64, width - 1)`. After landing, rebuild arm32 unit/conformance — a second latent 32-bit-int error could surface behind this first one (compilation stops at the first error), so iterate until `arm32_baremetal` unit goes green.
- **Coverage gap**: `SignedMinForWidth`'s tests ran only in 64-bit-int host mode, so the 32-bit-int break was invisible to the green legs — the recurring "tests only exercise host-int" trap. The arm32 unit run covers it once it compiles; a targeted 32-bit-int check is wanted.
