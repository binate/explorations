...
- **Verified**: `conformance/498` (non-variadic), `500` (single vararg), and `526` (multi-vararg `printf("%d %d %d\n", 11, 22, 33)`) are all green on builder-comp (LLVM) + native aa64 + native x64; native aa64 full sweep 455/0/1.  The earlier "KNOWN BUG: 498 and 500 can't both be green simultaneously" was a transient artifact of the convention mismatch and no longer holds.
