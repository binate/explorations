...
### REGRESSION on main: `b0402d04` (codegen: discover struct types reachable only through globals) makes gen1 emit an invalid GEP for loader's `Package` struct → the compiled-interpreter (`bni`) build fails → all `-int` conformance modes broken — CONFIRMED + BISECTED 2026-06-08
