...
### `71ff7489` (length-0 slices → nil-equivalent rep) regressed the bytecode VM — `110_cross_pkg_type_alias` fails on `builder-comp-int` (a default CI mode) — CONFIRMED (bisect) 2026-06-08
...
- **Fix direction (owner's call)**: either (a) update the VM's length-0/nil-equiv slice handling (`pkg/binate/vm` — `len` / `make_slice` / const-nil lowering / rt slice helpers) to match `71ff7489`'s rep, or (b) if `71ff7489` is VM-incompatible, adjust there. NOT yet attempted — concurrent commit; whether the gap is the IR change or the VM is the owner's call. Do NOT paper over with an xfail (would hide the regression).
