...
- **Next**: `ir.bni`'s over-export trim is exhausted (the rest is genuinely-public API — big type defs + the IR-gen entry points). Next is structural decomposition: the largest clean target is pulling the IR-**generation** subsystem (the `gen_*` files, ~344 `.bni` lines: `GenModule`/`GeneratePackage`/…) into its own package importing the `ir` data model (acyclic: generator → data model). Remaining over-1000 `.bni`s after `ir`: `vm.bni` (1265), `types.bni` (1109).
