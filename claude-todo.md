...
- **Next — Phase 2**: move the emit API (75 `@Block.Emit*` + construction primitives NewInstr/MakeArgs/AllocValueID/NewVoidInstr/addInstr) out of `ir` into new `pkg/binate/irbuild` as free functions (`irbuild.EmitX(b, …)`); re-point irgen (925 sites) + iropt; move `verify.bn` + emit-def tests to `irbuild`. This is the big `ir.bni` reduction (~330 lines → ~880). Original entry:
