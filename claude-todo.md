...
- **Discovery**: 2026-06-10, release-prep for `bnc-0.0.8`, while removing lang's sibling `pkg/binate/buf` violation (binate `4e581c3e`). With `buf` gone, `bootstrap` is lang's remaining questionable dependency.
...
### `readonly`-wrapped aggregate `OP_CONST_NIL` lowered to a null address → silent miscompile / SIGSEGV on clean code — CONFIRMED (adversarial review + empirical trigger) 2026-06-09 — MAJOR
- **Symptom**: an `OP_CONST_NIL` whose `.Typ` is `readonly <aggregate>` (outer `TYP_READONLY` over a slice / managed-slice / func-value / iface-value) is mis-classified as a NON-aggregate and lowered to a scalar null (`BC_LOAD_IMM 0` on the VM, `Movz rd,0` on native) instead of the address of a zeroed image. A by-address consumer (`len()`, `present()`, a call argument) then dereferences null. **Empirically triggered**: `func lenOf(b readonly @[]char) int { return len(b) }; lenOf("")` COMPILES CLEANLY but SIGSEGVs (exit 139) on native; misbehaves on the VM (`builder-comp-int`). Well-formed program, silent miscompile — cross-backend.
- **Root cause**: the const-nil aggregate classifiers peel only `TYP_NAMED`, never `TYP_READONLY`/`TYP_ALIAS` — VM `isAggregateLoadTyp` (`pkg/binate/vm/lower_instr_helpers.bn` via `vmUnwrapNamed`) and the native const-nil region predicate. So the planner skips reserving the zeroed frame region and the lowering falls through to the scalar-0 arm. This is the readonly variant of the bug binate `c997cf2e` (R2-D2) fixed for bare aggregates; `c6fe0914` (R2-D4) fixed `common.IsAggregateTyp` for the by-value PARAM path but NOT the const-nil-region path.
- **Why readonly leaks to the const-nil type (the two IR-gen sites that don't peel)**: (1) `EmitStringToChars` (`pkg/binate/ir/ir_ops.bn` ~256-273) resolves readonly only into a local for the dispatch decision but passes the still-readonly `sliceTyp` to `EmitRodataMSlice`/`EmitRodataSlice`, which call `EmitConstNil(sliceTyp)` for an empty literal. (2) `gen_return.bn` (~77-79) assigns the declared (readonly) result type to a `TYP_NIL` const-nil with no Kind guard. (The var-decl `gen_stmt.bn` and call-arg `gen_call.bn` paths DO Kind-guard, so they're safe.)
- **Discovery**: 2026-06-09 adversarial review of R2-D2/R2-D4 (the user asked "has your work been reviewed adversarially yet?"). 4 lenses, find→verify; this was the only confirmed-real finding (2 lenses, deduped).
- **Severity**: MAJOR — silent miscompile + SIGSEGV on a cleanly-compiling, type-valid program, on BOTH native and VM. Narrow trigger (readonly-wrapped aggregate nil-equivalent: empty `readonly @[]char` string, `readonly @func()`/`@Iface` return) but real, and exactly the class R2-D2 was meant to close.
- **Fix direction (owner's call)**: ROOT — peel `readonly`/alias before `EmitConstNil` at the two leaking sites (`EmitStringToChars`, `gen_return.bn`), so the const-nil carries the representation type and every backend's existing aggregate-const-nil path fires. DEFENSE-IN-DEPTH (recommended in addition) — make the const-nil aggregate classifiers robust: VM `isAggregateLoadTyp` and the native const-nil region predicate peel `readonly`+alias+named (mirror `common.peelTransparent` / `types.IsByvalParam`). Add a conformance cell (`readonly @[]char` param + `""`, plus a `readonly @func()` return) green on every mode; it currently SIGSEGVs native + misbehaves VM.
