...
### Bytecode VM mishandles `@Iface` (interface) values that live in / pass through transient-SP aggregate slots — TWO manifestations
- **Manifestation A — call result passed directly as an arg → "call through nil interface value"**: `consume(makeFoo(i))` where `makeFoo` returns `@Foo` and `consume(t @Foo)` calls `t.foo()` → in `-int` the VM panics `vm: call through nil interface value`, though the value is a present, non-nil `@Foo`.  Bisected (all `-int`): `var f @Foo = makeFoo(i); f.foo()` (bind then dispatch) PASSES; `consume(makeFoo(i))` (pass the call result DIRECTLY as an arg) FAILS; inline `var f @Foo = h; f.foo()` PASSES; `consume(makePtr(i))` (a `@Holder` ptr boxed at the arg) PASSES.  So: **a 2-word interface-value *call result* passed directly into another call's arg slot** — the VM materializes its vtable word from the wrong stack location.  Independent of any managed field in the receiver (reproduces with a plain `int`-field Holder).  This is the `handle(errors.New(...))` / `log(wrap(err))` shape, so it blocks `pkg/std/errors` in `-int`.
- **Manifestation B — struct dtor recursing into an `@Iface`-field dtor with a managed-field receiver → hard crash**: dropping a struct (`Wrap { F @Foo }`) whose `@Foo` boxes a receiver (`Holder { Inner @Inner }`) that has its own managed field SIGSEGVs the VM in the nested struct-dtor → `OP_IFACE_DTOR` → receiver-dtor chain.  `Holder` with a plain `int` field PASSES (`996`); a directly-boxed receiver-with-managed-field PASSES (`473`); only the **struct-FIELD `@Iface` whose receiver has a managed field** crashes.  This is a wrapped-error struct holding an `@Error` cause — also relevant to `pkg/std/errors`.
- **Pre-existing / NOT a codegen regression**: both emit correct IR (LLVM + native run them correctly, refcount-balanced); the bytecode VM (`pkg/vm`) mislowers interface values sitting in transient-SP aggregate slots (call-arg copy-back; nested-dtor field load).  No *existing* `-int` test regressed when the `@Iface` refcount fix landed (only the pre-existing 520 fails).  Likely the same root cause as **520_iface_dtor_callee_sole_ref** (also `-int`, same panic; 520 has no xfail and is a standing `-int` red).
