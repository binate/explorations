...
- **Confirmed**: `conformance/635_float32_arith` — expects `3\n3`; xfailed on the 3 VM modes + 3 native lanes, passes on the LLVM modes. Discovered during the Plan-4 adversarial review while probing float32 RETURNS (which are CORRECT — see `conformance/634_native_float32_return`).
