...
### MAJOR PROJECT — unify module-level static data into one IR representation (`ir.DataGlobal`) + one per-backend emitter — FILED 2026-06-10 (🟢 PHASE 1 LANDED, phased migration in progress)
- **🟢 PHASE 1 LANDED (binate `1ae1b52b`, 2026-06-21)**: `ir.DataGlobal` + `emitDataGlobal` (both backends) + `ir.BuildPackageDescriptor`; the `_Package` descriptor NODE+name now flow through `DataGlobal` (the hand-rolled node layout is gone from both backends). Also unified the native node/name to weak_odr/local, **closing the strong-symbol hardening item below**. Living plan + remaining increments (Inc 2: info-node tables + arrays → retire the per-kind/interim emitters; then vtables; strings; globals) in [`plan-codegen-data-global.md`](plan-codegen-data-global.md). The bullets below are the original design notes (still accurate for the unlanded phases).
