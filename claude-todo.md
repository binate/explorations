...
- **In-cone migration progress (2026-06-12):** DONE — `lexer` `97b3af36`, `parser` `80bf8687`, `loader` `1bc59c7b`, `buildcfg`+`asm/parse` (`9e5a6a3d`), `asm/macho`+`asm/elf` (`9d500c1d`), `cmd/bnc` `69679440`. (`native/common`, `debug` already CharBuf-free.) **REMAINING in-cone:** `mangle`, `ir`, `types`, `codegen`, `native/{x64,aarch64}` — plus stray test-file builds in `token`/`stringutils`.
  - **`mangle` is the dependency ROOT — do it FIRST:** `mangle.{FuncName,GlobalName,StructName}` RETURN `buf.CharBuf`, held as `var x buf.CharBuf = mangle.FuncName(...)` + `x.Bytes()` by ~35 caller sites in `native/x64`, `native/aarch64`, `codegen`, `ir`. Change those three to return `@[]char` (build internally with `buf.Builder`, `return buf.CopyStr(b.String())`); then every caller drops `.Bytes()` and retypes `buf.CharBuf`→`@[]char`. ATOMIC cross-package cascade (mangle + all callers in one commit) — map it with an analysis-fan-out before editing. Unblocks `native/x64`+`native/aarch64`+`codegen`.
