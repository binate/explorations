...
- **Latent follow-up** (same class, NOT yet fixed): `.ap` (function-value-call aggregate args) and `.rb` (retbufs) call-site allocas are also unhoisted; no package currently triggers them.  Tracked in [`plan-codegen-byval-spill-hoist.md`](plan-codegen-byval-spill-hoist.md).
