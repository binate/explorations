...
- **(G, CANDIDATE — needs a build) Cross-file top-level redeclaration of a non-`.bni` decl may not be diagnosed.** `func F(){...}` in `a.bn` AND `b.bn` of one package — `checkDuplicateDecls` is documented as within-file + skips `.bni`/`.bn` matches, so the cross-`.bn`-file case may slip (should be `F redeclared`). Untracked; confirm with a build.

- **(H, CANDIDATE — needs a build) Import cycles: no detection / no spec rule / no test.** The loader topo-orders `.bni` consts (implying a cycle is a problem) but nothing detects/rejects an import cycle. Untracked; confirm whether a cycle hangs, miscompiles, or goes undiagnosed.
...
## (planning) unused-entity checks — fix the unused-import `(a)` cross-file gap + add `(b)` unused locals / `(c)` unused private funcs / `(d)` unused private globals / `(e)` unused private types — 🟡 PLAN PENDING

bnlint today has exactly one "unused" rule (`unused-import`, `pkg/binate/lint/unused_import.bn`); the type checker has no usage tracking at all. Requested: plan the `(a)` gap fix and the `(b)-(e)` additions. **Blocked/entangled by the CRITICAL import-scoping bug above** — the unused-import rule's per-file attribution rests on file-scoped imports the checker doesn't actually enforce, so `(a)`'s semantics (per-file vs per-package) and the import-scoping fix must be decided together. Plan doc `explorations/plan-unused-checks.md` to be written once the import-scoping direction is chosen. Design recon (per-rule approach, edge cases, reference-vs-reachability for c/d/e, BUILDER-compat, shared-infra factoring) already gathered.
