...
## CRITICAL (REGRESSION from `2e783acd`, gap (b)) — a TRANSITIVE `.bni` const (`const X = otherpkg.Y`) silently miscompiles to a garbage value when the top-level importer does not also import `otherpkg` (2026-06-16) — 🔴 OPEN
...
**Fix options:**
- **(conservative, immediate)** — make `defineBniConst` NOT fold a `.bni` const whose initializer contains an `EXPR_SELECTOR` (leave it value-less, restoring the pre-`2e783acd` clean rejection). Preserves gap (b)'s actual win (self-contained `.bni` consts like `cast(int8,0)-cast(int8,3)` still fold; only transitive-selector `.bni` consts revert). Small, safe; stops the silent miscompile.
- **(proper)** — fix IR-gen's `RegisterImport` to fold transitive `.bni` consts consistently with the checker (resolve `pkg/a`'s consts when loading `pkg/b`, regardless of whether the top-level importer references `pkg/a`). Then `[b.SZ]int` = `[7]` and `x << b.SZ` is correct. Bigger (IR-gen transitive-const loading).
...
- **(c) bit_cast sign-reinterpret — MAJOR (design decision).** `const N int8 = bit_cast(int8, cast(uint8, 253))` (== -3 as int8); `x << N` → silent **0**. Passthrough folds it to the positive SOURCE value 253, so the gate accepts; IR-gen materializes the reinterpreted -3 → overshift → 0. (Plain narrowing `cast(int8, 200)` is rejected upstream with "use bit_cast", so bit_cast is the realistic remaining vector.) The committed `TestCheckRejectBitCastHiddenNegativeShiftCount` gives FALSE confidence — it uses an already-negative operand (no reinterpretation). Fix requires truncate+sign-extend to the target width in BOTH `evalConstIntValue` AND `evalConstExpr` in lockstep — a language-semantics-adjacent call (passthrough vs precise-fold); **needs user decision.**
- **(d) doc comment overstates parity — MINOR.** `evalConstIntValue`'s new comment says it "MIRRORS IR-gen's evalConstExpr exactly" — false while char-lit/selector (and sizeof/alignof/iota) remain unhandled. Reword (or make true by doing (a)+(b)).
...
- **(f) negated sizeof/alignof const shift count — MAJOR (untracked silent miscompile; pre-existing).** `const N int = 0 - cast(int, sizeof(int)); x << N` → silent **0** (REPRODUCED). `evalConstIntValue` folds neither `sizeof` nor `alignof` (IR-gen's `evalConstExpr` does), so a NEGATED sizeof/alignof const count folds to nothing on the checker side and the gate doesn't reject; IR-gen materializes the negative and overshifts → 0. Note: gap (d)'s comment ("sizeof/alignof are always non-negative, so no negative-shift risk") is FALSE — a negated sizeof IS negative. Fix: extend `evalConstIntValue`'s `EXPR_BUILTIN` arm to fold `sizeof`/`alignof` (resolve the TypeRef → `SizeOf`/`AlignOf`), mirroring `evalConstExpr`; side benefit `[sizeof(T)]int` dims fold. Same mechanical pattern as (a)/(b). Add a `.error` test.
- **(g) malformed char-literal escape in array-dim position — MINOR (diagnostic regression from `81d2655b`, gap (a)).** `['\q']int` → silently accepted as `[113]int` (REPRODUCED); before (a) char dims were rejected outright. Array dims resolve via `resolveTypeExpr → evalConstInt → evalConstIntValue → parseCharLiteral`, a path that never calls `checkExpr`, so `checkCharLitEscapes` never runs — a malformed escape (`\q`, `\x` w/o digits, surrogate `\ud800`) folds without validation. NO miscompile (checker ≡ IR-gen agree on the value), just a missing diagnostic. Fix: validate char-literal escapes on the array-dim subexpression (or have `parseCharLiteral` signal invalid). Char-literal-only (string lits don't fold to int in a dim). Also: extend `TestParseCharLiteral` with `\r \' \"`; add an escape-bearing e2e test; consider factoring `parseCharLiteral`/`parseCharLit` into ONE shared decoder (currently duplicated across `types`/`ir` with no tie test — drift risk). Pre-existing adjacent: a raw multi-byte char literal (`'é'`) is accepted as its first UTF-8 byte (front-end leniency, not from these commits).
