...
## MAJOR — native funcval/closure shim marshalling uses `ArgWords`, not the CallConv classifier — x64 false-rejects, aa64 SILENTLY MISCOMPILES (IN PROGRESS, being fixed as part of package-introspection B0)
...
- **Stage B — IN PROGRESS**: replace the loud `SetError` over-budget guards (`x64_funcvalue.bn` / `aarch64_funcvalue.bn`) with a real genuine-overflow **stack-spill** for the NON-closure funcval shims, so funcs whose effective user-arg words exceed the GP reg file (found in `ir`/`types`) build instead of failing. Design (mirrors `emitClosureShimStackSpill_*`):
  - When `nUserWords > userBudget`: `SUB rsp/sp, frameDelta` (16-aligned; x64 frameDelta ≡ 8 mod 16 to re-align across the dispatch caller's return-addr push), marshal user args with spill, `CALL/BL` underlying (NOT tail-jump — the frame must be torn down before RET), post-process the return, `ADD`, `RET`.
  - Outgoing placement via the CallConv classifier on `outArgTypes` = (sret-dummy-ptr if x64 sret, since `SretInGpArgReg`) ++ user params — handles reg / stack / AAPCS SPLIT. Float-scalar params peel to FP regs (NSRN); GP/effective words go to `CallArgRegStart`/`CallArgStackOff`.
  - Incoming reads: user word w (srcPrefix p: 1 scalar/float, 2 pack/sret) is dispatch word g=p+w → reg `argReg(g)` if g<NumGpArgRegs, else caller-stack at x64 `[rsp+frameDelta+8+(g-6)*8]` / aa64 `[sp+frameDelta+(g-8)*8]`.
  - Per-shape framing: scalar/void & sret → flat `SUB` frame + CALL/BL (x64 sret keeps retbuf in RDI; aa64 sret moves retbuf→X8 first); pack → frame also saves retbuf above the outgoing-args area, restores + stores RAX/RDX (x64) / X0.. (aa64) through it; float-scalar return → frame + CALL + `fmov`/`movq` FP→GP.
  - **File split FIRST** (both `*_funcvalue.bn` are at/over the 500 soft cap): carve the shim-body emission into new `*_funcvalue_shim.bn` before adding spill code.
- **Closure-shim cousins — FOLLOW-UP (not Stage B; user owns)**: the closure shims (`emitClosureShimFast_*` / `emitClosureShimStackSpill_*` / the closure-aggregate shims) (1) still count USER words via raw `ArgWords` (the indirect-large divergence, per line 10), and (2) don't marshal float-scalar USER args GP→FP at all (the non-closure shim does). Both are latent miscompiles for closures with managed-slice/iface or float params. B0's force-emit only emits NON-closure triples (top-level exported funcs aren't closures), so these don't block B0 — fix as a follow-up once Stage B lands.
...
### `build-bnc.sh --debug` (-O0 -g) emits invalid LLVM for `pkg/binate/native` — clang verifier rejects a dangling `, !dbg !DILocation` + "inlinable function call must have a !dbg location" — 🔴 OPEN (blocks all debuggable bnc builds)
- **Symptom**: `scripts/build-bnc.sh -o <path> --debug` fails compiling `pkg__binate__native.ll` with `error: expected instruction opcode` on a line that is JUST a metadata attachment — `  , !dbg !DILocation(line: 64, scope: !22)` — i.e. a `!dbg` attached to no instruction (dangling). Preceded by many warnings `inlinable function call in a function with debug info must have a !dbg location` on the `pkg/binate/native/x64` sret helper calls (`symPrefixed`, `globalSymFor`, `symFor`, `stringLabel` — all `call void @… (ptr sret(%BnManagedSlice) …)`). The release (`-O2`, no `-g`) build is fine; only `--debug` trips it. Blocks producing a debuggable (`-O0 -g`) bnc entirely (the failing module is in bnc's own tree).
- **Root cause (HYPOTHESIS — needs confirmation)**: the debug-info emitter (`pkg/binate/codegen/emit_debug*.bn`) appends a `, !dbg !DILocation(...)` attachment to an sret-call emission that produced no instruction text (or split the call across lines so the `!dbg` landed on its own line), and separately omits the required `!dbg` on inlinable sret calls. Both point at the sret-call debug-attachment path. Discovered 2026-06-11 building a `--debug` bnc to disassemble the 526 native-aa64 multi-return bug (switched to the release build to repro; this is unrelated to 526).
- **Severity**: MAJOR — a hard compile error (not silent) that blocks all debuggable bnc builds, hampering native-backend debugging. Not a correctness/runtime issue. **To repro + root-cause**: `scripts/build-bnc.sh -o /tmp/dbg --debug`, then read the preserved `.ll` (path printed on failure) around the reported line + the `symPrefixed`/`globalSymFor` call sites; compare the `--debug` vs release emission of an sret `call void`.
- **Test (TODO when fixing)**: a codegen unit test that an sret `call void` under debug-info emission carries a `!dbg` attachment on the call instruction itself (not a dangling line); ideally a smoke that `--debug` builds `pkg/binate/native`.
...
### Named-distinct type transparency — ✅ RATIFIED 2026-06-11 (adopt Go's model); IMPLEMENTATION PENDING
...
- **IMPLEMENTATION (pending)** — the checker (`pkg/binate/types`) must peel `TYP_NAMED` (+ `TYP_READONLY`) to the concrete underlying at every site that currently tests the raw `Kind`: the `len`/`present`/`same` builtin-arg checks (`check_builtin.bn` — `len` at :73), index (`EXPR_INDEX`), slice (`EXPR_SLICE`), the field-access selector (`check_selector`), binary/relational/arith operators, and the assignability predicate (identical-underlying + ≥1-unnamed). Comparison stays rejected for slice underlyings (incl. nil). Empirically (2026-06-11) ALL of index / assign-from-unnamed / `len` are rejected today for `type Buf @[]int`, so this is a coordinated sweep, not a single peel. Un-xfail `conformance/regressions/len-named-managed-slice` (all modes) + the `conformance/matrix/globals` named-slice/named-array cells when done; add field-access + assignability + comparison-rejection coverage.
...
- **RESIDUAL GAPS (loud follow-ups, NOT silently deferred):** (1) **x87 cross-package — 🔴 OPEN, Option A chosen (native x64 x87), to implement fresh.** A multi-return with >2 FLOAT fields crossing native↔LLVM diverges on x64: LLVM x86_64 returns the 3rd/4th float in x87 ST0/ST1 (empirically dumped: `{f64,f64,f64}`→XMM0,XMM1,ST0; `{f64×4}`→XMM0,XMM1,ST0,ST1; `{i64,f64,f64}`→RAX,XMM0,XMM1, no x87), while native x64 srets (`fpCount>2`). Pinned by `conformance/698_cross_pkg_mr_float3` (xfailed native x64; green aa64/LLVM/VM). **aa64 is unaffected** (D0..D7). **Option B (force-sret for >2-float multi-returns in pkg/binate/codegen) was ATTEMPTED 2026-06-11 and REVERTED**: it fixed the direct-call path (698 green on x64) but a codegen sret attribute affects ALL call paths, breaking the LLVM **func-value + iface-dispatch multi-return shims** (`abi/{funcval,iface}-multi-return*/f64/{3,4,5}` red on builder-comp) — a blast radius into the closure-shim worker's lane. User chose **Option A** (more localized): native x64 learns x87, no codegen change, aa64 untouched. **Option A plan (scoped, ready):** (a) empirically pin LLVM's exact x87 caller+callee convention for 3/4-float multi-returns via a Rosetta RUN (clang `-O0` asm alone is ambiguous — juggles XMM2/ST0); (b) add `fldl [mem]` (DD /0) + `fstpl [mem]` (DD /3) to the x64 assembler (`pkg/binate/asm/x64` — NONE exist today); (c) x87-aware `emitMultiReturnPack` (`x64_return.bn`) + `collectMultiReturnTuple` (`x64_call.bn`): floats 1–2→XMM0/XMM1, floats 3–4→ST0/ST1 (respect the x87 LIFO stack order); (d) x64 sret threshold → register up to 3 GP + 2 SSE + 2 x87 (sret at 5+ floats), leaving shared `MultiReturnTupleNeedsSret`/aa64 untouched (likely an x64-specific override); (e) un-xfail 698, verify x64-darwin + Rosetta + abi MR matrix. Still rare (>2-float MR crossing native↔LLVM); x87 is legacy/fiddly. (2) **aa64 SAME threshold bug — ✅ RESOLVED (`d206635d` 2026-06-11).** aa64 native used the 16-byte rule and sret'd any 17..64-byte tuple while LLVM register-returns up to 8 GP (X0..X7) + 8 FP (D0..D7); `MultiReturnTupleNeedsSret` now uses the per-target register-count rule (aa64 8/8). This was hit in practice by 526's `(int64, @errors.Error)` (3 GP words) — see the dedicated 526 entry. Float-HFA ≥3-component cross-pkg on aa64 is now register-returned too (D0,D1,D2..), though a dedicated cross-pkg FP-≥3 cell isn't added (the x64 sibling of that shape is the open x87 gap (1), so such a cell would need an x64 xfail). (3) **aggregate FIELDS inside a multi-return** — LLVM flattens; keep current behavior / sret, don't regress.
