...
**Proposed fix.** Record each generic decl's defining-file import scope at collection time (stash alongside `GenericTypeDecls`/`GenericTypeDeclPkgs`) and install THAT scope (not the use-site `c.Scope`) when resolving the instantiated body in `populateInstantiatedStruct`/`populateInstantiatedInterface`, while keeping the type-ARGS resolved in the consumer's context. IR-gen `ensureInstantiated*` must overlay the defining file's full import map for the body (mirror `overlayFileImports`/`declImportFile`) so both layers agree. Then `837` flips to passing.

## CRITICAL (type-checker / wrong-code) — imports are PACKAGE-scoped, not file-scoped: a file can use a package only a SIBLING file imported, and same-alias-different-package across files silently binds to the WRONG package — incl. qualified TYPES → wrong field offset (MEMORY-LAYOUT CORRUPTION) and an IMPLICIT same-last-segment trigger; checker & IR-gen pick OPPOSITE winners (2026-06-17) — ✅ facets A/B/C/E FIXED for NON-GENERIC decls (`cf0d1cad`); D `973e82f7`, H `a4d8a907` LANDED; 🔴 GENERIC instantiation still resolves bodies at the use-site (separate CRITICAL entry above)
