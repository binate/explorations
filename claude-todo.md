...
- **Next**: `ir.bni` (1231) is now below `vm.bni` (1274), so the max — and the ratchet target — is **`vm.bni`**. Cap is at 1274. Shrink `vm.bni`, reset the cap, repeat; then `types.bni` (1109). (`ir.bni` can still be pushed further toward its ~1050 floor later, but it's no longer the binding constraint.)
