...
- **Why the fix must live in IR (not in each backend separately)**: (1) **compiler/interpreter interop** — the dual-mode design has compiled and interpreted code calling each other via function pointers; both must produce identical refcount behavior or shared data will be double-freed/leaked at the boundary. Three independent implementations (compiler, interp, VM) is exactly the recipe for subtle interop divergence. (2) **multi-backend plan** (see `ir-backend-cleanup-plan.md`) — additional compiler backends are coming (32-bit ARM next); each would otherwise need its own correct copy/dtor support. Putting it in IR means one source of truth that all backends consume as ordinary CALLs. **Plan**: see `plan-vm-copy-dtor-via-ir.md`.
- **Next**: execute `plan-vm-copy-dtor-via-ir.md` — diagnose VM gap, ensure IR emits copy/dtor calls uniformly, and let backends consume them as ordinary CALLs.
