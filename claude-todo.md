...
- **(h-shr) `>>` const-fold is signedness-inconsistent across the three fold paths — 🔴 OPEN** (pre-existing; surfaced while doing h-bitwise). `bignum` would fold `>>` *logically*, but the parallel const-value-validation path and IR-gen's `evalConstExpr` fold `>>` via a host signed int (*arithmetic*), so they disagree for an unsigned operand ≥ 2^63. Folding `>>` in bignum here was therefore dropped: it would make `cast(uint8, U>>60)` (U=2^63) accepted as a `var` init but rejected as a `const` decl. Observed murk: `const X uint64 = U>>60` folds to `8` (logical) while the const-value path for `cast(uint8, U>>60)` folds to `-8` (arithmetic) and rejects, and runtime unsigned `>>` is logical. Proper fix: unify the three `>>` folds on *logical-for-unsigned / arithmetic-for-signed* (threading operand signedness into `evalConstIntValue` + `evalConstExpr`), then add `bignum.Shr` + fold it. Needs investigation — the existing host-int `>>` fold's per-context inconsistency should be mapped first. No xfail yet (no clear single reproduction; `>>` of a non-negative operand only shrinks it, so it is not a ≥2^63 miscompile vector — the value-wrongness is the concern).
