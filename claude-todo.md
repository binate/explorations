...
- **B0 step 3 (the Functions table) — IN PROGRESS**:
  ...
  - **3b-ii populate the table — NEXT (the large piece)**: per `.bni`-exported non-extern func (same set as `addExportedFuncsToSeen`), emit a static-managed `FunctionInfo` node (header + 8-word payload) + a `*[]@FunctionInfo` pointer array, wire `Functions.{data,len}`. Per node: `Pkg` → this package's node payload; `Name` → fully-qualified-name rodata; `Value` → `&@__handle.<mangled>` (the force-emitted handle); `ResultSize` = `types.SizeOf(result)` (0 void); `ParamSlots` = Σ per-param slot count (`argSlots` is VM-side — replicate the `(SizeOf+7)/8`-style sum in shared/codegen); `Sig` → `SigString` rodata. Mutually-referential globals (Package↔array↔nodes). Must land lockstep in the LLVM emitter AND the shared-native emitter (+ per-arch wiring to pass the func metadata list); `emitPackageDescriptor` needs the module (`m`) passed in (today it only has `modulePkgName`). Tests: conformance iterating `<pkg>._Package().Functions` (names/ResultSize) for a dep (LLVM emitter) + main (native emitter); unit tests per emitter. This is an ABI-critical 3-site lockstep emission — being done as a focused unit, not rushed.
...
### Named-distinct type transparency — ✅ RATIFIED 2026-06-11; INCREMENT 1 LANDED (slices/pointers/arrays + assignability); field-access pending
...
- **INCREMENT 2 — pending: field-access transparency** — the `check_selector` path (`pkg/binate/types`) peels `readonly`/alias but NOT named-distinct, so `type H @Box; h.field` is rejected (455's original case). Make it peel `TYP_NAMED` to a concrete struct / pointer-to-struct underlying (read+write, incl. auto-deref for a pointer underlying); opaque types stay rejected; methods NOT inherited.
- **REMAINING after inc 2**: verify/peel `present` + `same` for named types (user-flagged — IsSlice/IsPointer now peel, so `present` on a named slice/pointer likely already works via those; confirm + add coverage); the `conformance/matrix/globals` named-slice/named-array cells; the named fixed-array PARSER bug (`type Row [3]int` unparseable — separate entry below) so named array types can even be constructed.
