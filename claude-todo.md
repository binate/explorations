...
- **(h-bitwise) const-BITWISE operand ≥ 2^63 fit-check — ✅ CLOSED for `&` `|` `^` `<<`.** `checkCastConstFits` now folds const operands exactly in `bignum.Num` (`foldConstNum`): arithmetic landed in `c699cd78` (`822` reject / `825` accept guards), and non-negative `&`/`|`/`^`/`<<` landed in `3f57dc3a` (`826` `A<<1` is now a normal reject test, `827` `M|1` guards the in-range accept). A negative bitwise operand routes to the host-int fold (width/sign-dependent), symmetric with IR-gen. **Right shift (`>>`) is intentionally NOT bignum-folded — see the `>>` fold-unification item below.**
