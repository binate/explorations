...
- **Cap now 1231.** Max `.bni` is `ir.bni` (1231) again; then `types.bni` (1109), `vm.bni` (1103). **Next: push `ir` again** toward its ~1050 floor (extract more of the still-shared construction/emit helpers that only `irgen` needs, and/or remaining satellite-ish types), then `types.bni`.
