...
**Discovery.** Building the interface-path coverage test (review w5ck1416h item B2) for the qualified-method-type overlay; the test couldn't reach the overlay because the upcast fails first. `853` uses a qualified result (`dep.Tag`) so it ALSO covers the generic-interface qualified-method-type overlay (the interface counterpart of `841`/`850`/`852`) once this gap is fixed.

**Proposed fix (unknown depth — needs investigation).** At the consumer, an upcast to an instantiated cross-pkg generic interface whose impl lives in the interface's package must resolve to that package's emitted vtable (mangled for the (recv, iface-instantiation) pair). Likely needs the consumer's IR-gen to register/declare the library-side impl's vtable for the instantiation (parallel to how `RegisterImports`/`RegisterAllInterfaces` surface imported impls), plus correct iface-value construction for both raw (`*`) and managed (`@`) forms (the managed path's stray `extractvalue` is a separate symptom of the same missing wiring).
