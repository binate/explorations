...
- **Follow-up (tracked)**: `addDbgAnnotations`'s instruction-line test is `first char == ' '`; it correctly handles every current emitter (full `--debug` build is clean) but would mis-annotate a hypothetical whitespace-only line WITH a trailing newline (`  \n`).  Harden it to require a non-space char (≈3 lines + a direct helper test) so it's correct-by-construction.
