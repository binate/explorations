...
## CRITICAL (IR-gen / silent miscompile, ALL modes) — a GENERIC struct/interface's qualified by-value field/method TYPE binds to the CONSUMER's same-named import when the consumer imports a DIFFERENT package under the SAME short alias the generic uses → wrong layout; checker and IR-gen DISAGREE (2026-06-19) — 🔴 OPEN

**Symptom (REPRODUCED + independently confirmed, builder-comp + builder-comp-int).** `main` imports `pkg/other` AS `dep`; `pkg/genlib` imports `pkg/dep` AS `dep` and declares `type Box[T] struct { tag T; p dep.Pair }` (genlib's `dep.Pair` = `{x,y}` = 16B). `main` does `println(cast(int, sizeof(genlib.Box[int])))` → prints **32** (consumer's `pkg/other.Pair` = `{pad,x,y}` = 24B, + tag 8 = 32) instead of **24** (genlib's `pkg/dep.Pair`, + tag 8 = 24). The instantiated `Box[int]`'s `p` field binds to the WRONG package. Conformance `846_generic_cross_pkg_alias_collision` (`.xfail.all` — fails native AND VM, since the defect is in the shared IR-gen layout).
...
**Proposed fix (IR-gen only).** At the three overlay sites derive a CLEAN base before overlaying — strip short-alias entries from the snapshot (keep only full-path entries) — OR make the defining-file overlay LAST-WINS (rebind, not skip) for the generic's own imports. Then verify checker-vs-IR layout agreement (a `pkg/binate/ir/gen_generic_test.bn` unit test asserting the instantiated field's @Type package path is the generic's, not the consumer's) and flip `846` to passing.

---

## CRITICAL (checker + IR-gen / wrong-code) — a GENERIC decl's body resolves its package-qualified references against the INSTANTIATION SITE's scope, not the generic's defining-file imports → dangling symbol / wrong-package layout corruption (2026-06-18) — ✅ qualified-TYPE facet FIXED (`d2a9ff20`, 2026-06-19); 🔴 transitive-extern func-call (NATIVE-only) still OPEN — part (c)
...
**🔴 REMAINING — part (c): transitive-extern func-call, NATIVE-codegen ONLY.** A generic body that CALLS a function from a package the consumer does not import (`dep.V()`) needs that callee declared (LLVM `declare`) as an extern in the consumer's module; no transitive-extern declaration is emitted, so `main.ll` references an undeclared `@bn_pkg__dep__V` (LLVM "use of undefined value"). The qualifier now resolves to the CORRECT package (`@bn_pkg__dep__V`, not the old wrong `@bn_dep__V`) — only the declare is missing. **The VM modes already PASS** (the VM resolves cross-package calls via its runtime registry, no declare needed), so `837_generic_cross_pkg_import_body` is now a NATIVE-only xfail (8 `.xfail.<native-mode>` markers, no `.xfail.all`; mirrors `386`'s native-only pattern). Fix: when monomorphizing a generic body into a consumer module, emit a `declare` (extern Func) for any called function whose package the consumer doesn't already import — the linker resolves it against that package's compiled object (all packages link together).
...
**Fix status.** The proposed type-facet fix (record the defining-file scope/file and overlay it during instantiated-body resolution, type-ARGS still in the consumer's context) is IMPLEMENTED and LANDED — see the **✅ FIX LANDED** block above. The only remaining piece is the transitive-extern declaration for cross-package func CALLS in a generic body (part c, **🔴 REMAINING** block above); `837` flips to passing in native once that lands (it already passes in the VM).

## CRITICAL (type-checker / wrong-code) — imports are PACKAGE-scoped, not file-scoped: a file can use a package only a SIBLING file imported, and same-alias-different-package across files silently binds to the WRONG package — incl. qualified TYPES → wrong field offset (MEMORY-LAYOUT CORRUPTION) and an IMPLICIT same-last-segment trigger; checker & IR-gen pick OPPOSITE winners (2026-06-17) — ✅ facets A/B/C/E FIXED for NON-GENERIC decls (`cf0d1cad`); D `973e82f7`, H `a4d8a907` LANDED; GENERIC instantiation: qualified-TYPE facet FIXED (`d2a9ff20`), transitive-extern func-call still OPEN (separate CRITICAL entry above)
...
- **(h-shr / signedness-aware const-fold) — SINGLE consts ✅ DONE & LANDED `05d08117`; family residuals 🔴 OPEN.** Root cause (worse than first thought — a SILENT MISCOMPILE, not a false-reject): `evalConstIntValue` (checker) and `evalConstExpr` (IR-gen) fold the signedness-DEPENDENT ops `/`,`%`,`>>` as SIGNED host-int ops, so an unsigned operand ≥ 2^63 (stored as a negative host int) gives the wrong value — `const Q uint64 = (2^63)/(2^61)` emitted `-4` not `4`, silently (both folds agree on the wrong value). Hit typed AND untyped operands. **Fixed for SINGLE (non-grouped) consts** in lockstep: `bignum.Shr` (logical); `foldConstNum` re-enables `>>`; `constIntFor` prefers the bignum fold (`foldConstIntValue`); `checkConstDecl` stamps the value on the initializer's ExprType (`attachConstLitVal`); IR-gen's `genConst` reads the stamp (it can't re-fold — no checker scope at emit). Signed operands unchanged. Conformance `829`. Open residuals (same family, separate fold paths, all pre-existing; an adversarial review of the fix surfaced them):
  - **(h-group)** grouped consts (`const ( … )`). Deliberately EXCLUDED from the fix (new `InConstGroup` flag) — `genConstGroup` doesn't read the stamp and a shared-node iota repeat makes it unreliable, so applying the fix there would DESYNC checker (correct) from IR-gen (signed) → a grouped const used as an array dim would go from safe-reject to memory-corruption miscompile. Excluded, grouped consts stay host-int-folded on both sides (consistent, safe). Fix: make `genConstGroup` read a per-member-stable stamp (handle the bare-member shared-node restamp). xfail conformance `844_const_group_unsigned_fold_residual`.
  - **(h-bni)** imported `.bni` consts. `defineBniConst` (checker) folds via the old `evalConstIntValue`; `gen_import.bn` `registerImportConst`/`…Group` via `evalConstExpr` — neither routed through the signedness-aware fold. A cross-package `const Q uint64 = U/D` mis-folds on both sides. xfail conformance `843_import_unsigned_fold_residual`.
  - **(h-cmp)** relational compares of unsigned operands ≥ 2^63 (`const C bool = U > D` → folds signed → wrong). Separate path (`evalConstBool` / `evalConstExpr` LT/GT). xfail conformance `840_const_unsigned_compare_fold`.
  - **(h-inline-shift)** a const-folded inline negative-literal shift in value position (`var y int = (0-16) >> 2`) emits a LOGICAL shift (`(2^64-16)>>2`) instead of arithmetic `-4` — `genBinary`/type-inference, distinct from the const-decl path. Silent. xfail conformance `845_inline_signed_shift_residual`.
...
## MAJOR (native x64 codegen) — C-extern calls emit a non-PIC relocation → PIE link failure on `builder-comp_native_x64-comp_native_x64` (2026-06-18) — 🔴 OPEN (pre-existing; surfaced while diagnosing the chronically-red conformance CI)
...
## MAJOR (codegen) — a local of a NAMED ARRAY TYPE is not zero-initialized → reads of unset elements return STACK GARBAGE (2026-06-19) — 🔴 OPEN (pre-existing; surfaced during the array-dim-stamp review, NOT caused by it)

`type Arr [8]int; var a Arr; a[0] = 1; println(a[3])` prints stack garbage instead of `0`; the anonymous-type form (`var a [8]int`) zero-inits correctly. Memory-unsafe (uninitialized stack reads). Reproduces identically pre- and post the array-dim-stamp fix, so it is pre-existing and unrelated. Root cause: `emitAlloc` (`pkg/binate/codegen/emit_helpers.bn:66`) gates the fieldwise zero-fill on `instr.TypeArg.Kind == TYP_STRUCT || == TYP_ARRAY` **unpeeled** — for a named array type the Kind is `TYP_NAMED`, so it returns early and emits no zero-stores (the anon array, Kind `TYP_ARRAY`, gets 8 `store i64 0`). The sibling helpers `emitZeroRec` (`emit_copy.bn`) and `needsStructCopy` (`gen_util_refcount.bn`) DO peel `TYP_NAMED`; `emitAlloc` is the lone unpeeled site. Existing test `723_named_array_type` sets every element before reading, so it never exercises default zero-init. Fix: peel `TYP_NAMED`/alias before the kind check in `emitAlloc` (mirror `emitZeroRec`); also verify the bytecode-VM lowering zero-inits a named-array local (the `gen_stmt.bn` uninitialized-var path relies on dead-load elimination that does not hold for partial reads). Being fixed now as a separate commit.
