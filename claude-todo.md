...
- **(h) out-of-range `cast` not fit-checked for a const-ident operand / nested cast / array-dim position — MAJOR — ✅ FIXED in worktree (option S: cast does NOT launder constants), pending land.** `const M int = 257; cast(uint8, M)`, `cast(int8, cast(int, 200))`, and `[cast(int8, 200)]int` all silently produced wrong values; now all rejected. The false comments from (c)/(f) are corrected. Decision recorded in `claude-notes.md` (cast of a constant is still a constant; mask for truncation, bit_cast for same-size reinterpret). Implementation: new `checkCastConstFits` (folds the operand via `evalConstIntValue`, so nested casts don't escape) wired into `checkBuiltinCall`'s CAST arm (value position) and a new `validateDimCasts` walk in `evalConstInt` (array-dim position). Fallout fixed (tiny blast radius): the 4 `STATIC_MANAGED_REFCOUNT` sites in `pkg/binate/native/common/*` → `bit_cast`; `vm_exec64_test.bn` `cast(uint64, cast(int64,-1))` → `bit_cast`; conformance `650` rewritten to mask / bit_cast. Tests: conformance `816`/`817`/`818` + checker unit tests. Verified builder-comp + gen2 (conformance + unit).
  - **Residual sub-gap (h-precision) — OPEN.** The fit-check folds a const-ident / arithmetic operand through a host `int` (`evalConstIntValue` → `makeUntypedIntWithLit`), so a const whose magnitude is **≥ 2^63** is mis-judged: `const M uint64 = 0x8000000000000000; cast(int64, M)` is NOT rejected (the wrapped host int −2^63 "fits" int64). The DIRECT/untyped-literal path stays exact. Proper fix: carry the exact bignum value on the const symbol (`Symbol.ConstVal` is a host int today) instead of folding through host int. Acceptable interim: documented limitation. Also: external repos (`examples/`) not audited for laundering uses (they'll error on compile if any exist).
