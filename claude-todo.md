...
## MAJOR (IR-gen / latent silent-0) — `genSelector` reads an UNRESOLVED package-qualified const as 0 instead of erroring (2026-06-20) — 🟡 OPEN
...
## MAJOR (hygiene / lint coverage) — the ENTIRE compiler tree (`pkg/binate/*`, 22 packages) is NOT a bnlint target (2026-06-20) — 🟡 OPEN
`scripts/hygiene/lint.sh` discovers lint targets with a ONE-LEVEL glob `for d in "$BINATE_DIR"/pkg/*/` that requires direct `.bn` files in `pkg/<name>/`.  But the package reorg moved the compiler from `pkg/parser`, `pkg/types`, … to `pkg/binate/parser`, `pkg/binate/types`, … — so `pkg/*/` now matches only `pkg/binate/` (which has NO direct `.bn`; its files live one level deeper) and that dir is skipped.  **Net: ZERO of the 22 `pkg/binate/*` packages — the entire self-hosted compiler (parser, lexer, types, ir, codegen, vm, native, asm, loader, …) — is ever passed to bnlint.**  Confirmed by replicating the target loop: `pkg/`+`cmd/` discovery yields only `cmd/bnc` (cmd/bni/bnas/bnlint are LINT_SKIP'd).  bnlint's rules (`unused_import`, `readonly_uninit`, `func_value_escape`, `recursive_closure_capture`) therefore never run on the compiler — only its TYPECHECK happens transitively as `cmd/bnc`'s deps.  (The OTHER hygiene checks — file-length/line-length/bn-doc/bni-doc/naming — use a recursive `find "$BINATE_DIR/pkg" … -name '*.bn'`, so THEY do cover `pkg/binate/*`; only the bnlint-rules check has the gap.)  Surfaced by the adversarial review of the `__c_call` void work (the `pkg/binate/vm`/`pkg/binate/repl` LINT_SKIP entries I added are no-ops for the same reason — they're never targets; the real gate for rt's void `__c_call` is `cmd/bni`).  **Fix**: make lint target-discovery recursive over `pkg/` (find every dir containing a non-test `.bn`, or every distinct `package "..."` clause under `pkg/`, mirroring the impls/ifaces loop) so all 22 compiler packages are linted — then audit/clean up the now-meaningful `LINT_SKIP` (vm/repl/interp would need real skips while the void `__c_call` outruns the BUILDER bnlint).  Expect this to surface real `unused_import` (and possibly other) findings across the compiler that must be addressed (or the rule tuned).  Likely a regression introduced silently by the `pkg/` → `pkg/binate/` reorg.
