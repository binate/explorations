...
- **Severity**: CRITICAL — interface methods returning the idiomatic `(value, @Error)` tuple (the language's core errors-as-values shape) cannot be dispatched AT ALL; hard compile failure. Blocks `pkg/std/io` being usable for its entire purpose (polymorphic Reader/Writer dispatch).
- **HUGE TESTING MISS** — this is a core-feature × core-feature intersection ({interface dispatch} × {multi-value return}) that NO conformance or unit test ever exercised, despite multi-return being THE idiomatic pattern (errors-as-values: `func f() (T, @Error)`) and an interface method returning `(T, @Error)` being extremely natural. Same shape as the global-of-aggregate gap (an empty matrix intersection). The existing iface tests all use single-return methods; the multi-return matrix cells all use free functions, never interface methods. PREVENTIVE: a conformance + unit sweep of iface dispatch × {1, 2, 3 results} × {scalar / @Iface / managed-slice / struct components} — the `(int,int)` and `(int,@Error)` cells fail pre-fix, pass post-fix. (Broader pattern this session: feature-COMBINATION coverage is the recurring blind spot — cf. the global-of-aggregate intersection.)
