...
3. 🏷[BUG-BASH 2026-06-27 → LANE 1] **MINOR (type-checker / opaque encapsulation) — opaque-type field access LEAKS under co-compilation — ✅ DECIDED (2026-06-28): real gap, fix in-tree opacity.** `b.v` on an `@bx.Box` (Box forward-declared in bx.bni, full body in bx.bn) compiles instead of "cannot access field on this type" (type.opaque.field-rejection). **FINDING (2026-06-28):** opacity IS enforced in real SEPARATE compilation — compiling main against ONLY the opaque bx.bni (no bx.bn in reach) REJECTS `b.v` (verified). It LEAKS only when bx.bn is co-compiled in the SAME invocation (222's setup + whole-program builds): the consumer's checker resolves Box's FULL .bn layout instead of the opaque .bni surface. Inconsistent with symbol-visibility, which IS enforced in-tree (031). **DECISION (designer): real gap — fix it.** The checker must keep an imported type opaque per its .bni surface even when the .bn body is co-compiled (a two-view model: .bni surface for type-checking the CONSUMER, full .bn layout only for CODEGEN). Then un-xfail `222_err_opaque_field_access`. Non-trivial checker change (track per-import opaque-surface vs full-layout). Pinned: `222_err_opaque_field_access` (xfail.all).
...
- **STILL OPEN — do not lose these**:
  1. **Struct/array equality implementation** — ✅ DONE & LANDED (920a; main `f99f4a4e`). A struct/array is comparable iff every transitive field/element is (`aggregateComparable`, `types_query.bn`); one transitively containing a slice/iface/func is rejected ("this struct/array is not comparable"), all-comparable aggregates lower field/element-wise (`gen_eq_aggregate.bn`: materialize both operands to stack scratch, walk fields via `EmitGetFieldPtr` / array elements via byte-offset `EmitGetElemPtr`, one `OP_EQ` per scalar/pointer leaf conjoined with `OP_LAND`, `OP_NOT` for `!=`; recurses for nested aggregates). No refcount traffic (a `@T` field compares by identity). Verified on LLVM + VM + native aa64 + native x64. Tests: un-xfailed `spec/13-expressions/015`, new `053` (nested struct + array-of-struct), `054`/`051` (reject struct-with-slice / array-of-slice), `052` (managed-`@T` field, refcount-balanced), `07-types/044` (repurposed to positive named-array), `12-generics/067` (generic + comparable struct compose) / `068` (generic + struct-with-slice reject); matrix `eq-reject/{struct,array}` regenerated; `gen_eq_aggregate_test.bn` units. Adversarially reviewed — no defects in the lowering. Also removed the now-stale `772_generic_eq_managed_slice.xfail.all` (920b made the direct `eq[@[]int]` case a clean XPASS reject). **Exposed one MAJOR follow-up gap → item 4 below.**
  2. **Generic path (functions)** — ✅ DONE & LANDED (main `6b748a24`). Implemented the instantiation-time re-check as an INFERRED comparable constraint: `recordTypeParamComparison` (`check_expr_binop.bn`) flags the type-param (`TpUsedInEq` / `TpUsedInRel`) when the once-checked-opaque body compares it, and `instantiateGenericFunc` (`check_generic.bn`) re-runs `checkEqOperands` / `relationalOperandOK` on the concrete type-arg — so `eq[*[]int]` (slice `==`) / `less[*int]` (pointer `<`) are rejected at the instantiation instead of reaching IR-gen as an invalid `icmp`. Never false-rejects a comparable arg. Tests `spec/12-generics/064` (slice `==`), `065` (pointer `<`), `066` (int positive); 123 comparison/generic conformance + types unit green. **Two documented limitations remain (sub-follow-ups, not regressions):** (a) ORDER-DEPENDENT — a forward-ref instantiation checked before the generic's body is type-checked falls back to the loud IR-gen error (never a miscompile, never a false reject; a fully order-independent version needs a checker sub-pass or an explicit `comparable` constraint); (b) covers generic FUNCTIONS, not yet the rarer generic-TYPE-method comparison sub-case (`checkInstantiationConstraints` iterates AST type-params, not the `@Type` the flag lives on, and the method-body-check timing differs).
  3. **Sentinel comparison (`err == io.EOF`)** — ✅ RESOLVED (not open): interface-value `==` is permanently disallowed by design, and the sanctioned mechanism is `same(...)` (identity) — decided, with `pkg/std/errors` fully implemented. The earlier "under discussion / needs `identical`/`errors.Is`" framing was stale; there is no pending question here.
  4. 🏷[BUG-BASH 2026-06-27 → LANE 2] **Generic AGGREGATE-FIELD comparability re-checked at instantiation** — ✅ FIXED (commit `3a969b2f` on `bugbash-lane2`, pending land approval). A generic struct `Box[T] struct { v T }` (or a `[N]T` field) compared with `==` inside a generic function type-checks (field `v: T` opaque at the definition), but the 920b instantiation re-check only re-validated a DIRECT type-param operand — so `cmp[@[]int]` over `Box[T]` reached IR-gen: an invalid aggregate `icmp` on LLVM (loud clang failure) and a **SILENT first-word-only managed-slice compare on the VM** (printed `false`). Newly reachable since item 1 (before struct `==` was implemented, the body's `a == b` on `Box[T]` was rejected outright). **Fix:** `recordTypeParamComparison` now recurses through an aggregate operand's BY-VALUE fields / elements, flagging a type parameter reached by value so the existing instantiation re-check (`checkEqOperands` on the concrete type-arg) catches it; the recursion STOPS at pointers/slices (a type-param behind `@T`/`*T` compares by identity, so `cmp[@[]int]` over `Box[T]{ p @T }` is NOT false-rejected). Now rejected with "slices cannot be compared" (naming the offending type arg). Tests `spec/12-generics/069` (struct-field reject, was xfail → clean-rejection error test), `072` (array-field reject), `070` (comparable instantiation not false-rejected, lowered + run), `071` (behind-`@T` not false-rejected); green on LLVM + VM + native aa64 + native x64. Adversarially reviewed (no false/missed rejections, terminates, flag lands on the function's type-param instance). **Residual (pre-existing, NOT worsened):** generic-TYPE methods (vs free functions) still aren't covered by the re-check — see item 2's limitation (b).
