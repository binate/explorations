...
- **Two distinct issues, do not conflate**:
  1. **Spec gap (language-design decision, user-owned)**: what `==`/`!=` *should mean* for slices, func values, interface values, and structs is UNSPECIFIED. claude-notes.md:898 only positively specifies pointers ("address equality"); it does not address aggregates. (The `Comparable`/`Equatable` material is NOT about this — `Comparable` is a generics-constraint-only "fake" interface; `a == b` under `[T Comparable]` monomorphizes to a comparison on the concrete (usually non-interface) value. It says nothing about `==` on interface-/aggregate-typed operands.) Per type the spec could choose: identity/bitwise (compare the words), structural/fieldwise (structs — Go-style), or disallow (clean reject). This is the user's call. **Direction (user, 2026-06-07): DISALLOW for interface values (and slices, and func values).** This is consistent with the already-made decisions that `slice == nil` is disallowed (a nil slice acts like an empty slice but is not the same — silent footgun) and that interface values use `present(iv)` rather than `iv == nil` (typed-nil is an even bigger footgun). That precedent extends to `==`/`!=` *between* two interface values: disallow, with `present()` as the sanctioned emptiness test (see the `present()`-coverage TODO below). Structs remain open (fieldwise vs disallow). The one thing the interface-value disallow leaves unresolved is sentinel detection (`err == io.EOF`) — a separate question from emptiness that `present()` does NOT answer.
  2. **Codegen/checker bug (independent of the spec choice)**: regardless of what the spec decides, the CURRENT behavior — checker accepts `==` on every aggregate, codegen blindly emits a scalar `icmp` → invalid IR / hard compile failure — is wrong. It must end as either a clean checker rejection or a correct lowering, not invalid IR.
- **Root cause**: the `==`/`!=` type-check rule permits non-scalar operands, and codegen's equality lowering assumes a single-word integer/pointer operand (emits `icmp`). Same "aggregate treated as a scalar in codegen" family as the iface/func-value global-init bug.
- **Fix direction**: per the direction above, the checker REJECTS `==`/`!=` on slices, func values, and interface values with a clear diagnostic (point users at `present()` / the relevant helper); rejecting at the checker also fixes the codegen bug for those types (they never reach the bad `icmp` lowering). Structs remain gated on the open struct decision (fieldwise lowering vs reject) — do NOT reject structs unilaterally, that prejudges the still-open piece.
- **Consequence for io**: disallowing `==` on interface values means `err == io.EOF` is NOT the detection mechanism. `present(err)` answers "is there an error", not "is it THE EOF error" — sentinel identity needs its own mechanism (an identity-comparison carve-out, an `errors.Is`-style helper, or an `io.IsEOF(e) bool`). Open; resolve before the first real `Reader` lands. See the io.EOF TODO.
- **Discovery**: 2026-06-07, verifying whether `==` works for interface values (it does not — and the surface is all aggregates, not just interfaces). Original filing wrongly asserted "the spec says reject"; corrected — it's a spec gap, per user.
- **Tests (TODO with fix)**: a checker negative test for the disallowed types (slices, func values, interface values); a runtime equality conformance cell only if structs end up allowed (fieldwise).
