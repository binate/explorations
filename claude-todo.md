...
- **Symptom**: `var RP *Node = &VS` / `RP = &VS` where `VS` is a value-struct global (and the array analogue `AP = &AR`) stores **null** into the pointer instead of the global's address. Reading/writing through it then crashes (null deref) or reads 0. Reproduced **identically on all four host-runnable backends** (builder-comp / LLVM, builder-comp-int / VM, native aa64, native x64-darwin) — so it is a **front-end / IR-gen** bug, not per-backend codegen.
- **Trigger matrix** (each probed): NULL for `&structGlobal` / `&arrayGlobal` (addr-of whose RESULT is a pointer-to-aggregate) when the store DESTINATION is a pointer, via either the assignment arm or the global-var initializer (`__init` lowers `var RP *T = &VS` as `RP = &VS`). CORRECT for: `&scalarGlobal` and `&arr[i]` (result is `*scalar`) into any pointer; `&structGlobal` bound to a LOCAL via decl-init (true alias, write-through works); `RP = tmp` where `tmp` is a local holding `&VS`; and `x = VS` / `y = AR` (whole-struct/array copy FROM a global — these depend on the load and must keep working).
- **Root cause (confirmed, source-read + 4-agent workflow + behavioral matrix)**: `&VS` lowers (`pkg/binate/ir/gen_expr.bn` `genUnary` → `lookupVar`, `gen_func.bn`) to an **IsGlobalRef pseudo** carrying `Op=OP_ALLOC`, `Typ=*Node`, `TypeArg=Node` (the struct pointee). The ident/deref/selector assignment arms at `pkg/binate/ir/gen_control.bn:144,174,186` gate their load-before-store on **`isStructOrArrayAlloc(rhs)`** (`gen_refcount_pred.bn:139`), which matches `Op==OP_ALLOC` + a struct/array `TypeArg` but **ignores the destination type** — so for `RP = &VS` it emits `EmitLoad(IsGlobalRef, Node)`, loading the struct VALUE (0) out of the address and storing THAT, instead of storing the address. This is the **dest-type-blind sibling** of the `isAggregateAllocToLoad` migration (CR-2 Plan-1 review R2): the three newer arms (`:23,290,331`) already use `isAggregateAllocToLoad(rhs, elemTyp)`; these three were left on the loose predicate.
- **Regression origin**: introduced by the whole-aggregate-assign fix (2026-06-06, `gen_control.bn`) — that fix ADDED the `isStructOrArrayAlloc(rhs)`→`EmitLoad` load-before-store so `arr = {...}` / `x = other` copy correctly; it over-fires on `&aggregateGlobal` because the IsGlobalRef pseudo also presents as an `OP_ALLOC` of a struct/array.
- **Proper fix**: migrate `gen_control.bn:144,174,186` from `isStructOrArrayAlloc(rhs)` to the **dest-type-aware** `isAggregateAllocToLoad(rhs, <destTyp>)` (`varTyp` / `derefTyp` / `fieldTyp` respectively) — load only when the destination is itself a struct/array; a pointer dest stores the address as-is. **Do NOT** use the naive `if instr.IsGlobalRef { return false }` guard inside `isStructOrArrayAlloc`: that would skip the load for `x = VS` (whole-struct copy from a global, which currently works and is pinned green), re-breaking whole-aggregate-assign for global sources. Also audit `gen_call.bn:67` and `gen_method.bn:174` (same loose predicate; likely unreachable for well-typed args since a `*Node` arg can't bind a by-value struct param, but migrate for consistency).
- **Severity**: CRITICAL — silent wrong-code (null pointer) on idiomatic code (`var head *Node = &sentinel`, global lookup tables, linked structures), all targets.
- **Test**: `conformance/regressions/addr-of-aggregate-global` (xfailed on all 10 CI modes + the x64-darwin proxy; covers both the initializer and assignment forms, struct + array).
- **Discovery**: 2026-06-09, building Plan-C C8 (`cross_pkg_extern_field_write`) — the raw-ptr facet needed `var RP *Node = &VS` to set up an imported raw-ptr var, which surfaced this.
...
### [CR-2 Plan-1 review] `readonly`-wrapped >16-byte aggregate parameter: by-value signature vs by-pointer call site → garbage / SIGSEGV — LLVM+IR halves RESOLVED 2026-06-09 (binate `79ebfa98`); NATIVE half remains (Plan 3)
...
### [CR-2 Plan-1 review] plain (no-managed-field) struct/array composite literal passed BY VALUE → caller passes the alloca POINTER, not the value — CONFIRMED 2026-06-08, PRE-EXISTING
- **Symptom**: `dist(Pt{a:3,b:4})` / `asum([2]int{5,6})` compile clean and print garbage. Probe: `dist(Pt{a:3,b:4})` → `12252862232` (expected 7). IR: callee `define i64 @...(%Pt %v0)` (by value) vs call `call ...(i8* %v0)` (pointer). Controls: a named-var arg works (genExpr loads it); a managed-field struct works (hits `needsStructCopy`); `box(T{...})` is a keyword path.
- **Root cause**: `coerceArg` (`pkg/binate/ir/gen_call.bn:66-77`) only loads the OP_ALLOC aggregate inside the `if needsStructCopy(paramTyp)` branch; for a plain aggregate (no managed fields) `needsStructCopy` is false → the un-loaded alloca pointer is passed.
- **Severity**: CRITICAL — silent miscompile (garbage args) on the most basic "pass a composite literal by value" pattern. **Owner: Plan-1 (`pkg/binate/ir/gen_call.bn`).** Fix: hoist the OP_ALLOC-aggregate-to-value load out of the `needsStructCopy` branch (load unconditionally for a by-value struct/array OP_ALLOC arg; RefInc/copy only additionally when `needsStructCopy`). Add conformance: plain struct AND plain array composite literal passed by value.
- **Discovery**: 2026-06-08 adversarial review of Plan-1 (probe-confirmed).
...
### Iface upcast lowerings use `IfaceParentSlotOffset`'s −1 sentinel directly as a vtable GEP/byte offset — silent vtable corruption — FILED 2026-06-08 (plan-cr2-1 Defect 8 follow-up; NOT fixed, user deferred the decision)
...
### e2e/repl.sh build broken — generated test runner depends on `std/errors` (uses `same`), which BUILDER `bnc-0.0.7` predates — FILED 2026-06-09 (fixed by a `bnc-0.0.8` release + BUILDER bump)
- **Symptom**: `e2e/repl.sh` fails at its BUILDER-stage build: `impls/stdlib/common/pkg/std/errors/errors.bn:104:6: undefined: same` (and `cannot call non-function` / `non-bool condition`). The REPL e2e never runs because the toolchain build aborts.
- **Root cause**: `cmd/bnc/gen_test_runner.bn` now makes the GENERATED test runner depend on `pkg/std/errors` (binate `36e979df`, the `[B]` placeholder); `std/errors` uses the `same` builtin (binate `1f87b905`, `errors.Is`); but `BUILDER_VERSION` is pinned to `bnc-0.0.7`, which PREDATES the `same` builtin. So any path where BUILDER compiles the test-runner + stdlib hits an unknown `same`. NOT a current-bnc bug — `same` works in gen1 (conformance `661_same_ref` is green; gen1 type-checks it). The conformance/unit `builder-comp` modes are unaffected because gen1 (current bnc, supports `same`) compiles the test runner there, not BUILDER.
- **Fix**: cut `bnc-0.0.8` (current source has `same`) and bump `BUILDER_VERSION` → `bnc-0.0.8`. The release bundle build itself is NOT blocked (it builds the four binaries — which don't import `std/errors` — and packages the lib as source). VERSION is already `bnc-0.0.8-pre`.
- **Discovery**: 2026-06-09, building the CR-2 Plan-B B3 e2e value test (the REPL parked-member iota-repeat case). B3's e2e case (`tier3-pending-const-group-bare-iota-repeat`) is added with a verified transcript and will run once this is fixed.

### Float-component multi-return mis-packed on the native backends — packed into INTEGER regs, not D0/XMM0 — native↔LLVM ABI divergence — float64 ✅ RESOLVED 2026-06-09 (binate `b5911fbe`); x64 float32 follow-up below
