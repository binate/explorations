...
### Transitively re-exported interface degrades to a managed pointer (`i8*`) → SIGSEGV at dispatch — CONFIRMED 2026-06-08 (plan-cr2-1 Defect 8, now confirmed; was unconfirmed)
- **Symptom**: a consumer that imports package A — which re-exports `interface I = B.I` from package B — and uses the MANAGED interface value `@A.I` crashes (SIGSEGV, exit 139) when it dispatches a method on it.  The single-hop cross-package alias (`*A.I` constructed via the canonical `*B.I`, conformance 384) works; the TRANSITIVE managed case does not.
- **Repro (CONFIRMED, `conformance/665_transitive_iface_reexport`, xfailed all modes)**: `pkg/inner.bni` declares `interface Greeter { greet() int }` + `func Make() @Greeter`; `pkg/middle.bni` does `import "pkg/inner"; interface Greeter = inner.Greeter` + `func Get() @Greeter`; `main` imports ONLY `pkg/middle` and runs `var g @middle.Greeter = middle.Get(); println(g.greet())` → SIGSEGV (no output). Single-file / single-hop equivalents print `42`.
- **Root cause (direction, not yet pinned to a one-line fix)**: `main` imports `middle` but NOT `inner`, so `inner.Greeter` (the alias's canonical target) is never registered in `main`'s interface registry.  When `main` resolves `@middle.Greeter`, the alias chain reaches `inner.Greeter`, the bare-name/qualified interface lookup misses it (the resolver follows a single `currentImportAlias` hop — `gen_iface.bn` `isInterfaceTypeExpr` / `ifaceTypeForName`, `gen_util.bn` `resolveTypeExpr` TEXPR_NAMED arm), and the `@X` arm falls back to `MakeManagedPtrType` → the iv is laid out as a 1-word `i8*` instead of a 2-word interface value.  The dispatch then reads the vtable through a bad pointer → SIGSEGV.  Fix direction: re-exporting (aliasing) a cross-package interface must transitively register / resolve the canonical target interface in the consumer, so `@A.I` resolves to B.I's interface-value layout regardless of how many re-export hops separate them.
- **Severity**: CRITICAL — silent ABI mismatch → memory-unsafe crash. Narrow (transitive interface re-export, managed `@I`), but a real safety hole. The fix is a cross-package import/alias-resolution change (Plan-1 / `pkg/binate/ir` territory), non-trivial — NOT yet attempted.
- **Discovery**: 2026-06-08, plan-cr2-1 Defect 8 — built the 3-package repro per the plan ("confirm before any code change") and it reproduced.

### Interface-method dispatch of a multi-return method mis-packs the result tuple on two backends — SILENT wrong values — CONFIRMED 2026-06-08 (exposed by the Defect-3 SEAM fix)
- **Context**: plan-cr2-1 Defect 3 (binate landing pending) fixed the FRONT-END — the iface registry now stores every method's result types (`MethodResultsFlat`/`MethodResultCounts`) and `genInterfaceMethodCall` builds the correct dispatch result type (void / single / tuple struct). That unblocked the `abi/iface-multi-return` cells, which now exercise the BACKEND tuple lowering — and two backends mis-handle it (the direct multi-return call is correct for the same shapes, so this is the iface-dispatch lowering specifically).
- **Facet A — LLVM, >16-byte result (codegen, Plan 2)**: `iface-multi-return/int/{3,4,5}` (a method returning 3/4/5 `int`s = 24/32/40-byte struct) prints GARBAGE on `builder-comp{,-comp,-comp-comp}` (e.g. `8300882272 1073741824 …` instead of `10 20 30`). `int/2` (16 bytes, register-returned) is correct; a DIRECT `(int,int,int)` multi-return is correct. So the LLVM iface-call emission doesn't set up the sret (return-by-hidden-pointer) path for a >16-byte struct returned through the vtable function pointer. Codegen — `pkg/binate/codegen` indirect-call signatures.
- **Facet B — native aa64, sub-word result (native, Plan 3)**: `iface-multi-return/u16/{2,3,4,5}` (2..5 `uint16`s) prints wrong values on `builder-comp_native_aa64`; `int/*` is correct on native. The native iface-return tuple packer mis-packs sub-word components — almost certainly the same family as the native `five-u8` / non-8-multiple funcval-return packing CRITICAL.
- **Tests**: `conformance/matrix/abi/iface-multi-return/{int,u16}/{2,3,4,5}` — after the SEAM fix, unxfailed on the (cell,mode) pairs that pass (LLVM int/2 + u16/*; VM int/* + u16/*; native int/*); remaining xfails pin exactly these two facets (LLVM int/3,4,5; native u16/*) plus unverified x64/arm32.
- **Severity**: CRITICAL — silent wrong values, but narrow (multi-return iface dispatch at specific result shapes per backend). The front-end SEAM is correct; these are backend tuple-lowering gaps.
...
- **Backend follow-on (CONFIRMED 2026-06-08, plan-cr2-3 Defect 3)**: this front-end fix is necessary but NOT sufficient for iface multi-return on the native backends — there's a second backend layer. `pkg/binate/native/common/common.bn` `IsMultiReturnCall` gates on `OP_CALL`/`OP_CALL_FUNC_VALUE` only, so even with valid IR an `OP_CALL_IFACE_METHOD` multi-return is NOT recognized as multi-return: the x64 (and aa64) collect falls into the aggregate-single-return path (stores a pointer into the alloc region) while `OP_EXTRACT` reads it as a packed multi-return spill — a layout mismatch. When this front-end fix lands, `IsMultiReturnCall` must also gain an `OP_CALL_IFACE_METHOD` arm (and the matching `SpillHoldsAggregatePointer` logic). The x64 iface collect block itself is already routed through the shared `collectMultiReturnTuple` (binate `cb27f510`) so it is correct the moment it becomes reachable; it is dead until both this fix and the classifier arm land. (Same applies to a hypothetical `OP_CALL_INDIRECT` multi-return, though none exists today.)
...
### VM mis-unpacks a SUB-WORD (uint16) multi-return returned through interface dispatch — SILENT wrong values — CONFIRMED on the bytecode VM
- **Symptom**: `iface-multi-return/u16/2` (a dispatched `mr() (uint16, uint16)` returning 100, 200) prints `13107300, 1` on the VM instead of `100, 200`. `13107300 = (200<<16)|100` — the VM packs BOTH u16 results into the FIRST word and reads garbage for the second. The `int` variant (`iface-multi-return/int/*`) is CORRECT on the VM, so this is sub-word-specific (distinct from the iface-dispatch-multi-return entry above, which is the LLVM/native side). Likely related to the existing x64 sub-word multi-return packing family, but here on the VM and through dispatch.
- **Severity**: CRITICAL — silent wrong-code.
- **Test**: `conformance/matrix/abi/iface-multi-return/u16/{2,3,4,5}` (xfail the VM modes: builder-comp-int, builder-comp-int-int, builder-comp-comp-int).
- **Discovery**: 2026-06-07, abi result-side matrix sweep.
...
- **The question**: should a named-distinct type (`type X <underlying>`) inherit field access / non-mutating method dispatch from its underlying type? Reference points: Go ALLOWS selectors on a named *struct* type (`type T struct{...}` — its fields are accessible) but DISALLOWS selectors on a named *pointer* type (`type P *T` — no automatic field access through `P`). So a defensible Binate rule is: named-distinct over a struct VALUE → field access allowed (peel named for the field lookup); named-distinct over a POINTER (`@T`/`*T`) → field access NOT allowed (must convert to the underlying pointer first). Today Binate rejects BOTH, which is at least over-strict for the struct-value case.
...
### Global `var` of an interface-value / func-value (or readonly-wrapped aggregate) type emits invalid LLVM (`global %BnIfaceValue 0`) — FIX COMMITTED to worktree (binate `5dddef7d`), pending landing approval
...
- **FIX (landed `fce07ccd`)**: `genUnary` MINUS arm now types OP_NEG at the operand's exact integer width (any concrete `TYP_INT`, or the checker-resolved type for an untyped literal), not just float/Width==64.  The native/VM sub-word re-narrow (`68616b20`) was already landed, so facet B is correct on every backend and facet A compiles.  Pinned by `conformance/regressions/unary-minus-subword` + a gen_expr OP_NEG sub-word width unit test.  (Follow-up: re-add the exhaustive `scalar-diff/neg` generator family.)
...
### `==` / `!=` (and relational) on aggregates: checker now rejects — no more invalid LLVM. DECIDED + IMPLEMENTED at the checker (binate `e0f40c06`, in work-3, pending landing); struct/array impl + generic path remain OPEN
...
- **DECIDED (user, 2026-06-07)** and **implemented** in `pkg/binate/types` (binate `e0f40c06`):
...
### Cyclic non-struct named-type definitions (`type A B; type B A`, `type A A`) accepted with no diagnostic → every `Underlying`-walking helper hangs/crashes the compiler (CONFIRMED 2026-06-07)
...
### `present(...)` is interface-value-only today — investigate / specify / implement it for all sensible types
...
- **Refinement, NOT a blocker — ergonomic detection**: `err == io.EOF` doesn't work yet (the `==`-on-aggregates spec gap + codegen bug). The sentinel exists and is usable regardless; how a `Reader` consumer best tests for it (`==` as identity once that's decided, an `io.IsEOF(e) bool` helper, etc.) is a separate design question for when the first real `Reader` lands.
