...
- **Two fix options** (pick one):
  - **(A) Narrow**: in `gen_return`, RefInc the returned iface only for *borrowed loads* (`rv.Op == OP_LOAD`), not owned producers.  ~1 line; fixes only bug #1; verified for all 8 return shapes.  Leaves #2 and #3 unfixed; fragile for untested borrowed producers (e.g. slice-element `arr[i]`).
  - **(B) Principled / uniform (recommended)**: extend the refcount machinery to handle `@Iface` everywhere it handles `@T` — add the iface case to `emitTempCleanupBody`/`emitTempCleanupSince` (fixes #2), to the gen_assign overwrite path (fixes #3), and make `gen_return` RefInc unconditionally (owned temps then balance via temp-cleanup; borrowed loads balance via scope-RefDec).  Correct-by-construction (mirrors `@T`), fixes the whole family.  Blast radius: must audit iface *move-sites* to `consumeTemp` (so a moved iface temp isn't double-RefDec'd), exactly as `@T` does.
...
- **Tests**: conformance 546 (return-iface, method-value) exists but only catches UAF; add rt.Refcount *balance* assertions (catch leaks) for return / discard / reassign shapes.
- **Status**: IN PROGRESS — root-caused; **awaiting user decision on fix option (A vs B)**.  Part 0 (`present`) already landed.  See `plan-std-errors.md`.
