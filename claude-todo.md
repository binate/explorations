...
- **STATUS 2026-06-11 — FIXED & LANDED (binate `69eaf662`).** lang carries its own float formatter; the integer formatting is unified around the buffer-writing primitives `formatUint64Into` / `formatInt64Into`, shared by both the integer Stringers and the float formatter — a single 64-bit integer→decimal path (no narrow/wide `formatInt`/`formatInt64` pair).  Output byte-for-byte unchanged; `conformance/664` extended to pin the mantissa*2^exponent fallback.  `import "pkg/bootstrap"` is gone.  Follow-up (separate task, in progress): switch that fallback to approximate decimal e-notation for readability.
