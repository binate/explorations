...
### Big MULTI-RETURN function values are miscompiled on the x86-64 (SysV) native backend — data pointer collides with the sret pointer (both want RDI) — CONFIRMED 2026-06-10
- **Symptom**: a CAPTURING closure returning a big multi-return tuple (`> 16 B`, e.g. `var f @func() (int, @[]int) = func() (int, @[]int) { return n, make_slice(int, 3) }`; `a, s = f()`) **crashes** (SIGBUS) on `--backend native --target x86_64-{linux,darwin}` — the captures are read from garbage.  A NON-capturing big multi-return *with user args* (`@func(int) (int, @[]int)`) is also wrong; non-capturing with NO user args happens to survive.  aa64 native is CORRECT (sret uses a SEPARATE register, X8); LLVM + VM are CORRECT.
- **Root cause**: SysV-AMD64 returns a `> 16 B` aggregate via an sret pointer in **RDI** (the hidden first arg).  `emitCallFuncValue` (`pkg/binate/native/x64/x64_call_indirect.bn`) treats a multi-return as the SCALAR funcval convention (prefixSlots=1: data → RDI), then the `bigMultiRet` block **overwrites RDI with the sret pointer** (`LEA RDI, [RSP+sretOff]`).  So (a) a capturing closure's data pointer is destroyed before the shim can load captures; (b) the non-capturing scalar shim's user-arg shift (`RDI=RSI, ...`) clobbers RDI=sret when there are user args.  The single-aggregate path already uses the right retbuf-in-RDI / data-in-RSI convention (binate `646e1638` / `48e30320`); big multi-return does not.
- **Where**: x64 `emitCallFuncValue` (the prefixSlots / bigMultiRet RDI handling) + `emitFuncValueShims` (non-capturing multi-return shim) + `emitClosureShim_x64` (closure multi-return path).  The fix routes big multi-return through the same retbuf-in-RDI + data-in-RSI + user-args-RDX+ convention single-aggregate uses, coordinated across the call site and both shims; the closure side then reuses `emitClosureShimAggregate_x64`'s sret shape (capBaseSlot 1).  Small multi-return (`≤ 16 B`, packed RAX:RDX) is unaffected and stays on the scalar path.
- **Severity**: CRITICAL — silent crash / wrong-code for a natural shape (a closure returning `(value, @Error)`), x86-64 only.
- **Test**: `conformance/regressions/capturing-closure-multi-return` case (b) (`{int, @[]int}`) fails on the x64 lanes; it has been red there since binate `3540118c` (the comprehensive rewrite added the big-multi-return case without an x64 xfail — the x64 lane was wrongly assumed to be a non-functional stub).  Small multi-return cases (a)/(c) pass on x64.
- **Discovery**: 2026-06-10, validating the x64 single-aggregate closure-shim fix (`48e30320`) on the Rosetta lane — `capturing-closure-multi-return` crashed at case (b) while `capturing-closure-aggregate-return` passed.
- **Plan (per the user, 2026-06-10)**: land the x64 single-aggregate fix first (done — `48e30320`), then fix big-multi-return-x64 next (in progress).
