...
### arm32-baremetal runtime files (`crt0.s`/`semihost.s`) resolve against the target-iface OVERLAY, not the repo root → link fails → arm32 unit + conformance red — REGRESSION (in-window) — 🔴 OPEN (bnc-0.0.8 blocker)
...
- **Severity**: MAJOR — breaks all arm32-baremetal linking (primary target class) on a previously-green gate mode. **Still blocks cutting bnc-0.0.8** (arm32 unit/conformance not yet back to 0.0.7-green).
- **Fix (needs a decision — touches `ac738936` author's area / the overlay design)**: `root`/`primaryRoot` must yield the REPO ROOT for runtime-file (and linker-script) resolution, not the first iface path. Candidates: (a) `primaryRoot` returns the first `BniPath` not under `ifaces/` (the repo root is `BniPaths[1]` here, an ancestor of the overlay); (b) the runner passes the repo root to bnc explicitly (a `--root`/`--base` flag) instead of bnc inferring it from `-I[0]`; (c) `appendTargetRuntime` searches the `-L` impl roots (which already include `<root>/runtime/baremetal_arm32` via `targetImplPathSuffixes`) for the files rather than joining one `root`. (a) is the smallest.
...
- **Follow-ups (open)**: (a) split `pkg/binate/types/check_expr.bn` — now 506 lines, 6 over the 500 soft limit from this change; (b) comprehensive shift type-pair MATRIX conformance tests — every integer (value-type, count-type) combo is permitted and the result type is the value's; (c) count-wider OVERSHIFT corner — when the count's TYPE is wider than the value AND its VALUE ≡ a small residue mod 2^valueBitWidth (e.g. `byteVal << 256`), the count truncates to the value width and overshift is mis-detected (silent wrong value). Reachable only with an absurd count (≥ 2^width); proper fix = guard at the wider width (VM-safe, not sub-word) for the wider-count case.
