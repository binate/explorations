...
## MAJOR (self-compile / nondeterministic wrong-code) — `033_named_transparency` (named-managed-slice `len`/slice) miscompiles NONDETERMINISTICALLY on the gen2/gen3 self-compiled compiler (`builder-comp-comp` / `builder-comp-comp-comp`) (2026-06-23) — 🔴 OPEN — REPRODUCED (CI)

**Symptom.** `conformance/spec/07-types/033_named_transparency` (named managed-slice
`type Buf @[]int`, exercising index/`len`/slice transparency) prints a wrong value on one
of its later output lines (expected `30 / cmp ok / 12 / 3 / 2 / 99`; the divergence is in
the `len(buf)`=3 / `len(sub)`=2 / final `99` group — the runner truncates the diff to the
first 3 lines, which match) on the gen2 (`builder-comp-comp`) AND gen3
(`builder-comp-comp-comp`) LLVM self-compile modes. PASSES on gen1 (`builder-comp`), the
bytecode VM, and `native_aa64`.

**Layout-sensitive / nondeterministic.** It passed on `3345027c` CI and FAILED on
`05f3f047` CI — two commits differing ONLY by the `VERSION`/`version.bn` string
(`bnc-0.0.10-pre`→`bnc-0.0.10`), which shortens a compiled-in string constant and shifts
the self-compiled compiler's memory layout. It also PASSES locally on macOS/arm64 (gen1 +
gen2 ×2); the CI failure is ubuntu/x64. So the version bump did not introduce the bug — it
perturbed the layout and exposed a latent nondeterministic miscompile in the NEW compiler's
codegen for named-managed-slice `len`/slice when that compiler is itself built by the new
compiler (gen2 = BUILDER→gen1→gen2). This **falsifies the "correct on gen2" claim** in the
native-aa64 named-managed-slice entry (the embeddable-interp Inc-2 item) — gen2 is
nondeterministically wrong, not correct.

**Discovery / release impact.** Surfaced gating the `bnc-0.0.10` release: tag commit
`05f3f047` shows `builder-comp-comp` + `-comp-comp-comp` red on this single test
(`2410 passed, 1 failed`). The SHIPPED bnc-0.0.10 bundle compiler compiles 033 CORRECTLY
(verified: `30/cmp ok/12/3/2/99` through the extracted bundle), so that artifact is fine —
but because the defect is nondeterministic and lives in the self-compile path, a
release-pipeline build could in principle bake a subtly-miscompiled named-managed-slice
codegen into the BUILDER. Pre-existing (latent in bnc-0.0.9 too), NOT a 0.0.10 regression.

**Root cause.** Unknown — needs investigation. Likely an uninitialized read or
layout/address-dependent UB in IR-gen/codegen for named-managed-slice `len`/slice peeling
(the `@[]T` 4-word managed-slice header) that only manifests in the self-compiled compiler.
Candidate areas: managed-slice len/slice lowering, named-type peel-through, refcount/cleanup
ordering. Do NOT xfail it on the comp-comp modes — that would HIDE a nondeterministic
miscompile; root-cause it. Repro vehicle: the x64 gen2/gen3 CI lanes (does not repro on
arm64 locally).

## MAJOR (native codegen / native↔LLVM divergence) — a readonly string-literal managed-slice gets `backing_len = 0` on the native backends vs `len` on LLVM (2026-06-22) — 🔴 OPEN — REPRODUCED
- **Symptom**: `var s @[]readonly char = "hello"` (an `OP_RODATA_MSLICE`) materializes the 4-word managed-slice header `{ data, len, backing_refptr, backing_len }` with **`backing_len = 0`** on BOTH native backends, but `backing_len = len` on LLVM. Reproduced: a probe that `bit_cast`s `&s` and prints word 1 (len) + word 3 (backing_len) prints `5 / 5` on `builder-comp` (LLVM) but `5 / 0` on `builder-comp_native_aa64-comp_native_aa64`.
- **Root cause**: native `emitRodataSliceHeader` (`pkg/binate/native/x64/x64_rodata.bn:61-66`, `pkg/binate/native/aarch64/aarch64_rodata.bn:144-149`) hard-zeros BOTH the refptr (word 2) AND backing_len (word 3) for managed-slice targets. Zeroing the refptr is correct (immortal — RefInc/RefDec no-op on null). Zeroing backing_len is WRONG: the runtime's own convention (`rt.MakeManagedSlice` sets `BackingLen = length`) and the LLVM `.ms` header (`pkg/binate/codegen/emit.bn:443-446` writes word 3 = `len`) both make `backing_len == len`. So word 3 should be `len(bytes)`, not 0.
- **Impact**: latent — `backing_len` is the backing-allocation length preserved across subslicing (see `conformance/129_subslice_backing_len.bn`, whose own comment documents the 4-word layout). A subslice/reslice of a readonly string-derived managed-slice reads word 3 to bound the backing; native's 0 diverges from LLVM's `len`. No current conformance test reslices a *string-literal-derived* managed-slice and reads word 3 (129 uses `make_slice`, the heap path, which sets backing_len correctly via `MakeManagedSlice` on both backends), so it's gone unobserved.
- **How discovered**: planning DataGlobal Inc 4d (unify the `.ms` managed-slice header onto one `ir.BuildStringMSHeader`) — comparing the LLVM static `.ms` header against the native stack-built header surfaced the word-3 mismatch.
- **Proper fix**: native `emitRodataSliceHeader` must write `backing_len = len(bytes)` in word 3 (store `lenReg` there instead of zero). Add a conformance test that reslices a readonly string-literal managed-slice and reads word 3 (xfail the native modes until fixed). **DataGlobal Inc 4d fixes this for free** — routing native through the shared `ir.BuildStringMSHeader` (word 3 = `len`) replaces the hand-zeroed stack header. So the choice is: land a small standalone native-only fix now, or let 4d carry it (with the conformance test either way).
