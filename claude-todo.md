...
- **builder-comp swept (2026-06-13)**: only ONE stale xfail — `const-group-bare-inherited-overflow` — REMOVED (binate `680a4eca`, all 11 markers; it's a `.error` type-check test so stale in every mode). Host-mode xfails are otherwise healthy. **Remaining modes to sweep**: `builder-comp-comp`/`-comp-comp` (gen2), `builder-comp-int`/`-int-int`/`-comp-int` (VM), and the native/arm32 cross modes — for *backend-specific* stale xfails (run `run.sh --check-xpass <mode>`; flag MUST precede the mode; mind the hang risk on VM/arm32).
