...
- **Landed:** `strings.Builder`; `pkg/binate/stringutils` (binate `04c67dd3`) supplying the Builder-method gap as free functions over `*strings.Builder` (`WriteInt`/`WriteInt64`/`WriteHexByte`); the `.bni`-impl-registration fix (`3d147369`) that unblocks Builder-through-`io.Writer`; `cmd/bnlint` migrated (`ab076c5d`, first caller). `stringutils.Freeze` was dropped (`7350bdd1`) — it equals `buf.CopyStr(b.String())`.
