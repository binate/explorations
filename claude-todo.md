...
### Sweep for STALE xfails — the runner skips xfailed tests, so now-passing ones sit marked-failing forever (2026-06-13) — 🟡 OPEN
...
- **builder-comp swept (2026-06-13)**: only ONE stale xfail — `const-group-bare-inherited-overflow` — REMOVED (binate `680a4eca`, all 11 markers; it's a `.error` type-check test so stale in every mode). Host-mode xfails are otherwise healthy. `builder-comp-comp` (gen2) ALSO swept 2026-06-13 → 0 XPASS. So both default LLVM modes are clean; `const-group-bare` was the only stale xfail. **Remaining modes to sweep** (for *backend-specific* stale xfails): VM (`builder-comp-int`/`-int-int`/`-comp-int`), and the native/arm32 cross modes (`builder-comp_native_{aa64,x64_darwin}-…`, `builder-comp_arm32_{linux,baremetal}`). Run `run.sh --check-xpass <mode>` (flag MUST precede the mode; mind the hang risk on VM/arm32 — bound with `timeout`).
- **Systematic**: a script that, per mode, temporarily ignores the xfail and runs each xfailed test — anything that PASSES is a stale xfail to remove (and possibly a todo entry to close). Or: teach `conformance/run.sh` to report `XPASS` (run xfailed tests; flag unexpected passes) so this can't silently recur.
