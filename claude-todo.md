...
### REGRESSION on main: `b0402d04` (codegen: discover struct types reachable only through globals) makes gen1 emit an invalid GEP for loader's `Package` struct → the compiled-interpreter (`bni`) build fails → all `-int` conformance modes broken — REVERTED 2026-06-08 (`1ae18289`); proper mangler fix + Defect-2 re-land IN PROGRESS
- **STATUS 2026-06-08**: `b0402d04` was REVERTED on main (`1ae18289`) — main is green again (`-int` build verified). Defect 1 (`f2ebaca1`, the global static-zero NAMED-peel) was kept (independent, correct). Remaining work: (1) fix the underlying cross-package struct-name mangler collision in `addStructDef` (see Refined root cause below), (2) re-land Defect 2 (the `m.Globals` scan + `TYP_NAMED`/`TYP_ARRAY` discovery arms) + `conformance/657_cross_pkg_struct_global` + the `named-struct` globals cell, with a `builder-comp-int` smoke (full `cmd/bni` build) in the landing checks so a self-host break can't slip through again.
...
### Native (aa64/x64) mis-packs a SUB-WORD struct-return (`five-u8`) returned through a FUNCTION-VALUE call — SILENT wrong values — CONFIRMED aa64 + x64
- **Symptom**: `funcval-return/five-u8` (a `@func() S` returning a 5×uint8 struct {1,2,3,4,5}) prints `16,146,211,…` on native aa64/x64 instead of `1,2,3,…`. The *interface*-dispatch variant (`iface-return/five-u8`) PASSES and the DIRECT struct-return passes — so it's the function-value-call result path specifically that mis-packs the sub-word struct. Almost certainly a facet of the known native `five-u8` / non-8-multiple packing family (see the native-packing entries lower in this file), now seen through a function-value call.
- **Severity**: CRITICAL — silent wrong-code (narrow: sub-word struct return through a funcval call).
- **Test**: `conformance/matrix/abi/funcval-return/five-u8` (xfail native aa64/x64, likely arm32).
- **Discovery**: 2026-06-07, abi result-side matrix sweep.

### `readonly` / `const` type modifier is broken for managed values — Symptom 1 (silent field read) FIXED (binate `27c1ee8b`); Symptom 2 iface dispatch FIXED (binate `d3761004`); the `readonly @Box` method-receiver rejection remains, reframed below
...
- **Reframe of the remaining rejection (user, 2026-06-08) — likely a real bug, not a policy choice**: distinguish `readonly @Box` (a const *pointer* to a *mutable* object — the pointer can't be reassigned, but the pointee Box CAN be mutated, so ANY method call on it should be allowed) from `@(readonly Box)` (a mutable pointer to a const *object* — mutation of the object disallowed). The current rejection conflates the two: it treats `readonly @Box` as if the *object* were const. Const-METHOD annotations are explicitly NOT wanted ("always kind of a lie"). So the fix direction is: a `readonly @Box` receiver should dispatch to `*Box`/`@Box` methods freely (the readonly is on the handle, not the pointee); the mutation restriction belongs to `@(readonly Box)` (if/once that spelling is supported — needs checking). NEEDS: confirm whether `@(readonly Box)` is accepted today, then make the method-receiver rule key off object-constness, not handle-constness.
