...
### Named-distinct type transparency — ✅ RATIFIED + CORE LANDED 2026-06-11 (increments 1+2: slices/pointers/arrays + field access + assignability + present/same); only the named fixed-array PARSER bug + matrix cells remain
...
- **REMAINING**: (a) named fixed-array PARSER bug — ✅ PARSER FIXED (binate `5a9b08be`; grammar D11 two-token lookahead, `type Row [3]int` now parses) — **BUT this surfaced a separate, pre-existing 🔴 MAJOR IR-gen bug** (raised 2026-06-11): `pkg/binate/ir/gen_access.bn` checks `Typ.Kind == TYP_ARRAY` peeling only `peelReadonly`, never `peelNamed`, so a `TYP_NAMED`-over-array mis-compiles — `r[0]` emits invalid LLVM (`bitcast i64→ptr`) and `len(r)` returns **0** (SILENT wrong result). The CHECKER peels named for arrays (increment 1), but IR-gen's array path does not — named SLICES work in IR-gen, named ARRAYS don't. **Fix**: peel `TYP_NAMED` (via `peelNamed`, exists in `gen_typedecl.bn`) at every index/slice/len/bounds-check site in `gen_access.bn` (`genIndex`, `genIndexPtr`, `getIndexElemType`, `isNestedArrayBase`, the index-store path, the `len` lowering). Pinned by `conformance/723_named_array_type` (xfail builder-comp{,-int}). (b) the `conformance/matrix/globals` named-slice/named-array cells.
