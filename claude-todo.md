...
- **Now visible on native_aa64 (2026-06-10)**: `TestExternFloat{,32}ArgViaRegistry` are the SOLE remaining `pkg/binate/vm` unit failures on `builder-comp_native_aa64` after the `_Package` native-emit fix (binate `2d8e1f67`) unmasked them (the package previously link-failed before any test ran). So this float-arg-shim native gap is now the one thing keeping native_aa64 `pkg/binate/vm` unit red.
