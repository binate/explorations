...
- **🔴 MAJOR — named COMPOSITE LITERALS silently drop their initializers (NEW, raised 2026-06-12)**: `type Row [3]int; Row{10,20,30}` and `type NS S; NS{10,20}` (struct) COMPILE with no error but read **0** for every element/field (the `len` is right — it comes from the static type — but the values are dropped). As a global `var G Row = Row{10,20,30}` reads `0,30`; as a local, garbage. **Root cause**: `genCompositeLit` (`pkg/binate/ir/gen_composite.bn`) dispatches on `e.TypeRef.Kind`; a named type is `TEXPR_NAMED`, which matches none of the `TEXPR_ARRAY`/`TEXPR_MANAGED_SLICE`/`TEXPR_SLICE` arms, so it falls into the struct path, `lookupStructIdx(typeName)` misses the wrapper (`< 0`), and it returns `EmitConstInt(0)` (gen_composite.bn:34-36) — silently discarding the literal. It never peels `TYP_NAMED`. **Fix**: peel `TYP_NAMED` at the top of `genCompositeLit` (resolve to the underlying array/slice/struct before dispatching) — the literal analogue of increment-1's index/len/assignability peel. **Discovered** 2026-06-12 probing the init form for the matrix `init/named-array` cell. Workaround in use: the matrix + user code construct via the unnamed literal form (`[3]int{...}` → named, which works via assignability). NOT yet xfail-tested — add `conformance/regressions/named-composite-literal` (xfail all modes) when filing the fix.
