...
- **Cap now 1109**, bounded by `types.bni` (1109), then `vm.bni` (1103); `ir.bni` is 902. **`types.bni` reduction: IN PROGRESS (claimed 2026-09-19, work-3/session).** Next: reduce `types.bni` (1109) and `vm.bni` (1103) toward 1000 — over-export trims + any cohesive concern that can peel into an acyclic sub-package (as done for ir/vm satellites). These are the remaining blockers to a 1000 cap.
