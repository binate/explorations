...
### Unit tests RED on compiled-native modes — `pkg/binate/vm` / `repl` / `cmd/bni` test binaries don't link the std `_Package` objects — FILED 2026-06-10 (Lane C task-5 confirm) — MAJOR
...
### e2e/repl.sh + print-args.sh build broken — gen1 build resolved stdlib current-first, so the BUILDER hit current `std/errors` (`same`) — ✅ FIXED binate `e29aaec0` (pending landing) 2026-06-10 (Lane C)
