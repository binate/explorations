...
### Global address (`&G`) as an rvalue STILL dropped at `OP_CAST` (all 3 non-VM backends) + iface-method ARG (aa64 + LLVM) — `emitValOperand` work was incomplete, per-op whack-a-mole keeps missing sites — LLVM halves RESOLVED 2026-06-09 (binate `d086ccac`); NATIVE halves remain (Plan 3)
- **STATUS 2026-06-09**: the LLVM halves of BOTH facets are FIXED (CR-2 Plan-2 Round-2, binate `d086ccac`). `emit_ops.bn` `emitCast` now precomputes the source via `emitValRef(buf.New(), Args[0])` (mirroring `emitBitCast`) and writes it at every arm; `emit_iface_call.bn:156` switched `emitRef`→`emitValRef`. Tests added: conformance `669_cast_global_addr` + `670_iface_method_global_addr` — green on all LLVM modes + VM. **REMAINING (Plan 3, native):** Facet 1 native OP_CAST (`x64_dispatch.bn:388`, `aarch64_dispatch.bn:411` — bare `getOperand`) and Facet 2 native aa64 iface-arg (`aarch64` `emitCallIfaceMethod`). `669` xfails all 3 native modes; `670` xfails aa64 only (native **x64** iface-arg was already fixed in `0c707e1f`, confirmed: 670 passes on x64-darwin). The architectural root (per-op whack-a-mole vs the VM's op-agnostic materialization pass) below still stands as the durable-fix recommendation.
