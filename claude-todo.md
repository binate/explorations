...
- **named-float / named distinct scalar type mis-lowering** — root cause is DEEPER than "emitCast float-float cast": IR-gen does not represent named DISTINCT scalar types (`type F float32`, IsAlias=false).  resolveTypeExpr (gen_util.bn) only finds aliases (`type F = float32`) and structs; a named distinct non-struct type isn't registered (the alias-registration passes in gen_module.bn RegisterStructTypes, gen_import.bn RegisterImports, and gen_module's current-package pass all gate on `d.IsAlias`), so it falls through to the `TypInt()` fallback.  Named-INT works by luck (fallback is int); named-FLOAT becomes int → `var x F32 = 1.5; cast(int,x)` yields the raw double bits (4609434218613702656).  Probed fix (drop the `IsAlias` gate so named distinct non-struct types register like aliases for layout) was attempted and reverted — it didn't take effect for the main package (there's another current-package resolution path), confirming a broad change is needed across all the registration sites + possibly genDecl's var-type path, with care around method dispatch on named scalar types.  NOT a minor follow-up; needs a proper named-distinct-scalar-type representation pass in IR-gen.  STILL OPEN.
- ~~negative / div-by-zero array dims have no clean diagnostic~~ — ✅ FIXED (binate work-2 `87528314`): evalConstInt now reports a negative length and a fully-known div/mod-by-zero dimension.  Conformance 643 / 644 error tests.
- ~~bare iota-repeat member type uses the GROUP (first-member) type~~ — ✅ FIXED (binate work-2 `e5c82085`): genConstGroup tracks prevTyp alongside prevExpr, so a bare member inherits the PRECEDING member's type.  Conformance 645.
- ~~stale comments~~ — ✅ DONE (binate work-2 `bc88b434`): iota-repeat.bn comment updated to the fixed runtime (1,2,4,8).  The aarch64 "D-regs at offset 100" comment is already gone from the tree (recent float work removed it).
