...
- **Cap now 1109**, bounded by `types.bni` (1109), then `vm.bni` (1103); `ir.bni` is 902. **`types.bni` over-export trim DONE (`7014ccaab`)**: 1109 -> 1033 (removed 19 internal-only symbols). Cap now 1103 (bounded by vm.bni). Checker split (types -> types + check) planned in `explorations/plan-checker-split.md` (the path to types < 1000; deferred). **`vm.bni` reduction: IN PROGRESS (claimed 2026-09-19, work-3).** Then return to checker split — over-export trims + any cohesive concern that can peel into an acyclic sub-package (as done for ir/vm satellites). These are the remaining blockers to a 1000 cap.
