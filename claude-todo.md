...
### Passing a plain (no-managed-field) struct/array COMPOSITE LITERAL by value passes the alloca POINTER, not the value → SILENT garbage — LLVM + every native backend (VM correct) — PRE-EXISTING — re-confirmed + pinned 2026-06-10
...
- **Severity**: CRITICAL — silent miscompile (garbage args) on the most basic "pass a composite literal by value" pattern, on LLVM and EVERY native backend; the VM is correct (it carries aggregates by address). **Owner: Plan-1 (`pkg/binate/ir/gen_call.bn`).** Fix: hoist the OP_ALLOC-aggregate-to-value load out of the `needsStructCopy` branch (load unconditionally for a by-value struct/array OP_ALLOC arg; RefInc/copy only additionally when `needsStructCopy`).
- **Test**: `conformance/regressions/composite-arg-by-value` (struct + array; xfailed the LLVM-exec + native modes, PASSES on the VM modes — binate `a77591e0`).
...
### Likely-stale `native_x64` (ELF) iface-multi-return xfails — confirm + remove — 2026-06-10 (Lane B)
- **What**: the 16 markers `conformance/matrix/abi/iface-multi-return{,-assign}/{int,u16}/{2,3,4,5}.xfail.builder-comp_native_x64-comp_native_x64` blame "iface dispatch multi-return: native tuple-packing not yet implemented". That packing **IS implemented** (`pkg/binate/native/x64/x64_iface.bn` routes `OP_CALL_IFACE_METHOD` multi-returns through `collectMultiReturnTuple`), and the **identical-codegen** `builder-comp_native_x64_darwin` (Mach-O; same `pkg/binate/native/x64` backend, only object format differs) **PASSES all of these cells** (Lane B run 2026-06-10, and already noted in `03b80566`). ELF also passes the un-xfailed `multi-return` / `funcval-multi-return` cells, so there's no general ELF runtime gap — only iface int/u16 carry the marker, and even iface `f64` is un-xfailed and passes.
- **Why not removed here**: the ELF mode (`builder-comp_native_x64-comp_native_x64`) isn't locally runnable on macOS/arm64 (no `qemu-x86_64`), so XPASS can't be confirmed from here. Removing on inference risks a CI red if there were some ELF-specific wrinkle (unlikely — it's pure register-packing codegen verified on the Mach-O sibling).
- **Confirm + remove**: on an x86-64-linux host or in CI, `conformance/run.sh --check-xpass builder-comp_native_x64-comp_native_x64 matrix/abi/iface-multi-return` → expect XPASS on all 16 → delete the markers. (Leave the arm32 iface-multi-return xfails — different, less-complete backend, not covered by this evidence.) Note CI runs ELF natively on the ubuntu x86-64 runner, but only surfaces these once Lane A's `-comp*` link break clears.
