...
### float32 COMPARISON is computed in double precision on the f32 bit pattern — CONFIRMED silent wrong-code, VM + both native backends (LLVM correct) — OPEN (arithmetic + negate FIXED 2026-06-06)
- **FIXED 2026-06-06 (binate df7a5ec1, 12a24e74)**: float32 *arithmetic* (`+ - * /`) and *negate* now compute at single precision — native single-precision ops (aa64 FADD/FSUB/FMUL/FDIV/FNEG `_s` ty=00 encoders, x64 ADDSS/SUBSS/MULSS/DIVSS; x64 negate XORs the f32 sign bit) and VM `BC_F32ADD/SUB/MUL/DIV/NEG` opcodes. `conformance/635_float32_arith` now covers all four binops + negate + a bit-exact `1.0f/3.0f = 0x3EAAAAAB` single-precision-rounding check; green on every lane (the `builder-comp_native_x64` x86_64-linux 635 marker is retained — unverifiable without qemu-user, same x64 codegen as the passing x64_darwin lane).
- **STILL OPEN — float32 comparison**: `==/!=/</<=/>/>=` on float32 still compute in double on the raw f32 bits → garbage. VM `lowerCmpOp` emits `BC_FEQ/FLT/...` (float64); native `emitFloatCompare` (`aarch64_float.bn` / `x64_float.bn`) uses double `Fcmp` / `Ucomisd`, no width check. `var a float32 = 1.5; var b float32 = 1.5; if a == b {…}` is wrong on the VM + both natives (correct on LLVM).  NOT yet covered by a conformance test.
- **Fix shape (comparison)**: aa64 FCMP single (ty=00 variant of Fcmp, base 0x1E202000) + an emitFloatCompare width branch (`Fmov_w_to_s` then FCMP.S); x64 UCOMISS (`0F 2E`, no `66` prefix) + branch (`Movd` then UCOMISS); VM `BC_F32EQ/NE/LT/LE/GT/GE` opcodes (`bit_cast(float32, …)` the low 4 bytes) + a `lowerCmpOp` width branch.  Add a float32-comparison conformance test, xfailed on the 6 non-LLVM lanes until fixed.  Discovered by the float32-arithmetic adversarial review.
- **NOT broken**: float32 CONST materialization (539), float32 RETURN bits (636), and float32 CASTS (`BC_F32TOSI` / `emitFloatCast` carry width) are all correct.
