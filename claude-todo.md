...
- **Approach — ratchet**: set the cap to the current largest `.bni`, shrink that file, reset the cap to the new max, repeat until the cap can be 1000. Each step is independently landable and the tree can never regress. Two levers shrink a `.bni`: (1) remove over-exports — decls in the `.bni` that no other package uses become package-private; (2) genuine package decomposition — move a cohesive concern into its own package whose dependency direction is acyclic (e.g. a generator package that imports the data model, not vice-versa). `expose` (facade re-export) only re-exports an *independent* package, so it cannot break a tightly-coupled core like `ir` apart — a peeled-off sub-package that still depended on `ir` would cycle. Multi-`.bni`-per-package is a non-starter: spreading a bloated interface across files defeats the cap (a big `.bni` = too-big public interface).
- **Step 1 (done, `c748ad51b`)**: cap set to the current max, **1454** (`ir.bni`), as a single hard limit (the warn zone was removed earlier in the file-length rework). A live `TODO` in `scripts/hygiene/file-length.sh` also tracks the target.
- **Step 2 (done, `a47be1e2e`)**: removed 9 over-exported funcs from `ir.bni` (6 internal helpers + the 3 test-only verifier entry points) and moved the verifier's `IRViolation`/`VERR_*` const block into `verify.bn`. `ir.bni` 1454 → **1377**; cap follows.
- **Step 3 (done, `ff5f6ef2f`)**: split `pkg/binate/ir` into three packages — `ir` (IR data model + the construction/emit/query API), `irgen` (the `gen_*` generation subsystem; imports `ir`), and `irutil` (pure leaf helpers: type predicates, name manglers; depends only on types/mangle, so both import it acyclically). Generation-fixture tests (`genFromSource`-based) moved to `irgen` integration tests; pure data-model/opt/verifier tests stay in `ir`. All 838 IR test functions preserved. `ir.bni` 1377 → **1343**; cap follows. (A big change — 276 files; conformance 3040/0, adversarial-reviewed.)
- **Empirical floor for `ir.bni` (~1050, so <1000 is at the edge)**: after the split, `ir.bni` is the IR **data-model types** (~605 lines: Module/Func/Block/Instr/…) + the **emit-method API** (~333 lines: `EmitReturn`/`EmitCall`/… — pinned to `ir` by method-locality) + the construction/query/emit helpers that `irgen` AND the in-package optimizer/verifier both need. That core is ~1050+; reaching <1000 for `ir` means extracting satellite *types* (`FuncSig`+hashing, `TypeInfo`/RTTI, `StringConst`) to sub-packages and/or trimming the still-shared helper API — a further grind with diminishing returns.
- **Step 4 (done, `3015a7d66`)**: moved the satellite IR-metadata types out of `ir.bni` into `pkg/binate/irdata` (backend-neutral, ir-free): `FuncSig`+`FuncSigHasher`/`FuncSigEq`+`funcSigHash`, `ImplInfo`, `TypeInfoEntry`, `TypeDef`. `Global` stays in `ir` (holds an `@Instr`). `ir.bni` 1343 → **1231**.
- **Step 5 (done, `8ece527b4`)**: moved the VM's peripheral runtime types out of `vm.bni` into new `pkg/binate/vmdata` (leaf deps; vm imports it): `VMFuncVtable`/`VMFuncHandle`/`VMClosureRec`, `ExternBinding`, `ArgIfaceLayout`/`ArgSlotIfaceLayout`, `IfaceVtable`, `VmNameHasher`/`VmNameEq` (+ `hashName`/`nameEq`). `vm.bni` 1274 → **1103**.
- **Step 6 (done, `64186dd73`)**: privatized 5 `ir`-internal methods no external package calls (`AddTypeDef`, `CollectStrings`, `EmitDeref`, `EmitRodataMSliceCopy`, `EmitStructLit`) — removed from `ir.bni`, bodies stay in `ir/*.bn`. `ir.bni` 1231 → **1214**.
- **Cap now 1214.** Max `.bni` is `ir.bni` (1214); then `types.bni` (1109), `vm.bni` (1103). The cap is now gated *entirely* by `ir` — reducing `types`/`vm` won't lower it while `ir` dominates.
- **`ir.bni`'s real floor is ~1100–1200, NOT ~1050** (earlier estimate too low). Audits: (a) funcs used *only* by `irgen` and movable out = 2, both pinned by private verify internals; (b) `Module` fields movable to `GenCtx` = 5 (~5 lines) — the rest (`Checker` 108 external uses, `OptLevel` 52, `Interfaces` 45, …) are read by backends; (c) emit methods cleanly movable = 9 of 54, and a partial move yields a schizophrenic API (`b.EmitReturn(x)` vs `EmitMainEntry(b,x)`) = gaming the check. `ir.bni` = data-model types (493: Module 178/Func 140/Instr 103/…) + emit/builder methods (~330, pinned by method-locality; 22 called by ir's own opt/verifier) + shared helpers (~200, pinned by ir-self). Splitting `irgen` *out* is what forced the emit API to become *public*.
- **IRBuilder re-arch — Phase 1 DONE (`1acae8ad6`)**: extracted the IR optimizer (RunOptPasses + mem2reg/SROA/inline/load-forward/bce/phi-elim + dom/loops) out of `ir` into new `pkg/binate/iropt`. Conformance builder-comp 3040/0/9; adversarial review clean (zero logic changes). `ir.bni` 1214 → 1206; cap now **1206**. Plan: `explorations/plan-ir-builder-rearch.md`.
- **IRBuilder re-arch — Phase 2 DONE (`b590efa7c`)**: moved the emit/construction API (79 Emit* methods → free functions) + verifier out of `ir` into new `pkg/binate/irbuild`; re-pointed ~1030 call sites. Conformance builder-comp 3040/0/9; adversarial review clean. **`ir.bni` 1206 → 902 — the re-arch goal (ir < 1000) is met.** `ir` is now a pure data-model package (`irbuild` = construction+verify, `iropt` = optimizer). Full plan archived in `explorations/done/plan-ir-builder-rearch.md`.
- **Cap now 1109**, bounded by `types.bni` (1109), then `vm.bni` (1103); `ir.bni` is 902. **`types.bni` over-export trim DONE (`7014ccaab`)**: 1109 -> 1033 (removed 19 internal-only symbols). Cap now 1103 (bounded by vm.bni). Checker split (types -> types + check) planned in `explorations/plan-checker-split.md` (the path to types < 1000; deferred). **`vm.bni` reduction DONE (`342701b29`)**: moved the 470-line BC_* bytecode opcode enum out of vm.bni into package-private `pkg/binate/vm/bytecode_ops.bn` (no external code consumer) + removed 2 dead-from-birth opcodes (BC_CALL_EXTERN, BC_NUM_OPS). vm.bni 1103 -> 632. Validated: builder-comp-int 3028/0/21. **Cap now 1033, bounded solely by types.bni (1033).** ONLY remaining blocker to a 1000 cap: the checker split (`explorations/plan-checker-split.md`), which needs explicit user go (14k-line re-arch). Then return to checker split — over-export trims + any cohesive concern that can peel into an acyclic sub-package (as done for ir/vm satellites). These are the remaining blockers to a 1000 cap.
- **(orig) Next — IRBuilder re-architecture (user-greenlit 2026-09-19)**: the only path below ir's ~1100 floor. Make `ir` a pure data-model package; route *all* IR construction — `irgen`'s calls AND the in-`ir` opt passes — through an explicit builder so the emit API leaves `ir`'s public surface; relocate the opt passes out of `ir`. Large, compiler-wide. Plan: `explorations/plan-ir-builder-rearch.md`.
