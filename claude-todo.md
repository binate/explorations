...
**Spun off — two ultra-contrived `2^63`-magnitude const-shift-count signed-overflow edges (a shift count of ~2^63 is absurd; tracked, not fixed):**
- **IR-gen (bare literal):** `x << 0x8000000000000000` now compiles (correct, a positive count) but runs as `1` not the spec'd overshift `0` — `emitConstOvershiftOrNil` keys on the *signed* `IntVal`, negative for 2^63, missing the overshift.
- **Checker (computed expr), found by the brief review:** `x << (1 << 63)` / `x << (0x4000000000000000 + 0x4000000000000000)` — a COMPUTED (non-bare-literal) count in `[2^63, 2^64)` is spuriously rejected with `negative shift count`. The `46204267` gate skips only a bare `EXPR_INT_LIT`; a computed count flows through `evalConstIntValue`, which folds in a SIGNED host int and overflows negative. Inconsistent with the bare-literal allow-case. A spurious REJECTION (not a miscompile).
- **Common root-cause fix:** make the const-shift-count sign/overshift determination bignum/unsigned-magnitude-aware (the language folder `foldIntArith`/`foldIntBitwise` already computes the exact value + `LitSign`) in BOTH the checker gate and `emitConstOvershiftOrNil`, instead of re-folding through the overflow-prone signed `evalConstIntValue`/`IntVal`. (The clean checker side could instead fix `checkUnaryExpr`'s TILDE branch to complement `LitSign`, then gate on `LitSign` — handling `~0`, computed, and bare cases uniformly.) Awaiting a user decision.

The brief review also added the `>>=` wide-negative compound test (`conformance/796`) and fixed a stale comment — binate `51a9a2e6`.
