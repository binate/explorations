...
### Named-distinct type transparency — ✅ COMPLETE & LANDED 2026-06-11 (slices/pointers/arrays + field access + assignability + present/same/operators + named arrays); only the matrix-globals coverage cells remain
...
- **REMAINING (minor)**: the `conformance/matrix/globals` named-slice / named-array coverage-matrix cells (the point behaviors are all pinned by 719–723; these would round out the systematic matrix).
