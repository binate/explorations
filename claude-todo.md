...
### Inferred-type func-value local call mis-lowers to a direct symbol — `var f = <func value>; f(x)` → undefined `main.f` on ALL backends — PRE-EXISTING — RUNTIME-CONFIRMED 2026-06-11
...
- **Root cause**: unknown — needs investigation. The mangled symbol `main.f` / `@bn_main__f` is the *variable's own name* lowered as if `f` were a package-level function: a call whose callee is an inferred-type func-value local is lowered as a DIRECT call to a global named after the variable, instead of an indirect func-value call through the variable's {vtable,data} value. With an explicit `@func`/`*func` annotation the local is correctly typed as a func value and the call is indirect. Likely the func-value flavour/type isn't recorded for the inferred local at the point the call is lowered (front-end / IR call-lowering), so the callee resolver falls back to "callee is a function name". Plausibly in the same family as the named-func-value-type hint bug below (`checkExprWithFVHint` not peeling), but distinct (this is *inference*, no annotation at all).
...
- **Fix**: TBD pending root-cause. If inference of a func-value local is intended-unsupported, it must be a CLEAN front-end rejection ("cannot infer func-value type; annotate @func/*func"), not a cryptic undefined-symbol link error. Otherwise, record the inferred func-value type so the call lowers indirectly.
- **Test**: NOT yet added (fails identically on all modes → either an all-modes-xfailed conformance test or, better, a front-end/IR unit test once the location is known). Repro above is exact. Add per the Bug Discovery Protocol when the fix is scoped.
