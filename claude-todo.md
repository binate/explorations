...
- **(a) char-literal arithmetic — MAJOR (narrow).** `const N int = cast(int, 'A') - cast(int, 'Z')` (== -25); `x << N` → silent **0**. `evalConstIntValue` has no `EXPR_CHAR_LIT` case (evalConstExpr does, `gen_const_fold.bn:43`); the new cast case recurses into the subtraction whose leaves are char lits → ok=false → gate doesn't reject. The structurally-identical int-literal form (`cast(int, 65) - cast(int, 90)`) IS rejected — proving the missing char case is the cause. (Bare `'A' - 'Z'` is BENIGN — uint8 wraps to 231, a legit overshift.) Fix: add an `EXPR_CHAR_LIT` arm to `evalConstIntValue` (needs a `parseCharLit` in the types pkg; review verified this closes it). Cheap/mechanical.
- **(b) selector-qualified cross-package const — MAJOR (broader).** imported `const N int = -3` (or cast-hidden); `x << pkg.N` → silent **0**. `evalConstIntValue` has no `EXPR_SELECTOR` case (evalConstExpr does, `gen_const_fold.bn:67`). ALSO breaks the positive direction: a cross-package const array dim `[pkg.SZ]int` fails with "array length must be a constant integer". Fix: add an `EXPR_SELECTOR` arm resolving the qualified `SYM_CONST` via `resolveQualifiedSym` → `sym.ConstVal` (mirrors the `EXPR_IDENT` arm); fixes both the shift gate AND cross-package array dims. Cheap/mechanical.
