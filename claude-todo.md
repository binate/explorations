...
### Shift count: negative → panic (Go semantics) + unchecked-shift intrinsic — 🟡 OPEN (in flight, 2026-06-15)
Decided 2026-06-15 (after the runtime-overshift fix above): a shift count is non-negative; a NEGATIVE count is a programmer error, matching Go. Scope agreed with the user:
- **(2a) Runtime negative-count panic via a dedicated check op** (for consistency with the bounds/divide-fault convention, NOT a one-off branch-to-panic): new `OP_SHIFT_CHECK` + `rt.ShiftFail` parallel to `OP_BOUNDS_CHECK`/`rt.BoundsFail` and `OP_DIV_CHECK`/`rt.DivFail`. Lowering needed in: `pkg/binate/ir` (op + `EmitShiftCheck`), `pkg/binate/codegen` (`emit_instr.bn`/`emit.bn`, LLVM), `pkg/binate/native/{x64,aarch64}` dispatch (+ arm32 if separate), `pkg/binate/vm` (`lower_instr.bn`/`vm_exec_helpers.bn`), and the runtime (`ifaces/core/pkg/builtins/rt.bni`, `impls/.../rt.bn`, `rt_baremetal.bn`) — ~10 files + tests. Emitted only for a SIGNED count type (unsigned can't be negative) — gate to avoid a needless check.
- **(3) Constant negative shift count → COMPILE error** (like any out-of-range constant shift). Checker change; relates to the just-landed `042d2fe6` "reject out-of-range constant casts". A const negative count should never reach IR-gen.
- **Unchecked-shift intrinsic (OPEN QUESTION, user raised)**: analogue of `unsafe_div`/`unsafe_rem` — an `unsafe_shl`/`unsafe_shr` (or similar) that skips BOTH the overshift guard and the negative-count check, lowering to a bare hardware shift (target-defined for an out-of-range count; the contract is "count in [0, width)"). Gives hot paths an escape hatch from the per-shift guard/check. Decide whether to include in this body of work.
