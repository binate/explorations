...
- **Release gate:** out-of-cone callers are all migrated (above). The REMAINING `buf.CharBuf` usage is entirely **in-cone** (`cmd/bnc`, `pkg/binate/{codegen,ir,types,asm/*,native/*,lexer,parser,loader,mangle,token,...}`) and can't migrate until the next BUILDER release — importing Builder pulls `strings` into bnc's cone, which the current BUILDER (`bnc-0.0.8`) can't yet carry. Next step there: a trial cone build against the next BUILDER.
