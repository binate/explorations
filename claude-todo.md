...
### Multi-value assignment `a, n = f()` mishandled managed targets — FIXED 2026-06-03 (binate, pending cherry-pick)
...
- **Remaining gap (tracked, NOT fixed — rare, pre-existing)**: a managed *struct/array AGGREGATE* field/element targeted by a multi-assign SELECTOR/INDEX (`s.structField, n = f()` / `arr[i], n = f()` where the element is a managed struct) is still a plain store — no save-copy-destroy — so its managed fields are under-retained / the old aggregate's fields leak.  The IDENT struct case IS handled (save-copy-destroy).  Out of scope for the four managed VALUE types; mirror single-assign's `needsStructCopy` arm for SELECTOR/INDEX if a real caller appears.
...
- **Proposed fix**: in `collectInterfaceFromDecl`, register the interface's identity in `moduleInterfaces` (Pkg+Name, enough for `isInterfaceTypeExpr`/`ifaceTypeForName` to resolve a self-ref) **before** resolving its method result types — e.g. append a preliminary entry and back-fill `MethodNames`/`MethodResults`/parents by index, or split into a name-registration pre-pass over all `DECL_INTERFACE` followed by a method-signature pass.  Mind the parent-interface and alias arms (they also `resolveTypeExpr` and may self/forward-reference).  Add a conformance test: an interface method returning its own type, dispatched in-package (`x.M().…`), rt-balanced.
...
### Cross-package function returning `@Iface` resolves the return type to a managed pointer (`i8*`) in the consumer → ABI mismatch / would-be silent miscompile — BLOCKS `plan-std-errors.md` Part 1
...
- **Proposed fix**: in `isInterfaceTypeExpr` and `ifaceTypeForName` (`gen_iface.bn`), for a bare name (`te.Pkg` empty) that misses under `currentModulePkgPath`, also try `lookupModuleInterfaceIndex(resolveImportPkg(currentImportAlias), te.Name)` when `currentImportAlias` is non-empty, and (in `ifaceTypeForName`) build the `TYP_INTERFACE` under that resolved package so the type identity matches the registry.  Mirrors the existing `currentImportAlias` retry in `gen_util.bn`'s `TEXPR_NAMED` arm.  Add a cross-package conformance test (consumer calls a `@Iface`-returning imported function) — the whole `plan-std-errors.md` cross-package suite exercises it.
...
### Multi-return of a `@func` component is miscompiled — capture lost (LLVM) + invalid closure-data kind (VM)
- **Symptom**: a function returning a tuple with a function-value component — `func two(...) (int, @func(int) int)` — is wrong-coded for the `@func` slot.  Probe (2026-06-03): `two(false)` returns `(0, adder(10))` where `adder` returns a capturing closure `func(x) { return x + n }` with `n=10`; calling the returned `f(5)`:
  - **LLVM**: prints `5`, not `15` — the captured `n` reads as 0 (the closure's capture is lost / not carried through the multi-return aggregate).
  - **VM (`-int`)**: crashes `vm: unsupported function-value data kind: 0` (the packed `@func` component's data slot isn't a valid closure record).
- **Contrast**: the IFACE analogue (a `@Iface` multi-return component) IS fixed (see the VM multi-return iface entry / `571`); `@func` is a *separate, deeper* break (it fails in LLVM too, so it's not just the VM packing), so the VM multi-return fix (`isVMInterfaceValue`) deliberately does NOT cover `@func` — giving the broken `@func` path multi-word treatment only swaps one failure mode for another.
- **Root cause (unknown — needs investigation)**: likely the multi-return aggregate layout / copy doesn't preserve a func value's closure-record pointer (LLVM), and the VM packing leaves a non-closure data word.  Single-return `@func` works (534/542/555), so it's specific to the multi-return tuple path.
- **Severity**: MAJOR — silent wrong result in LLVM (capture loss), crash in VM.  Latent: no code returns a `@func` in a tuple yet.
- **Discovery**: 2026-06-03, while fixing the `@Iface` multi-return VM bug for `plan-std-errors.md` (the `(T, @Error)` error-return pattern).  Pre-existing.
