...
- **Symptom**: when a struct is declared in `pkg/foo.bni` AND in
  ...
  the program and silently emits machine code that mis-resolves the
  mismatched field(s).  Discovered via Stage 4 of plan-c-call.md
  (binate `0d0f35b7`): `common.bni` declared
  `CallConv.VariadicStackOnly bool` as the 8th field, but
  `common_callconv.bn`'s impl-side `struct` only listed 7 fields
  (missing VariadicStackOnly).  The type checker accepted
  `cc.VariadicStackOnly = true`, but pkg/codegen lowered the field
  access to GEP index 0 (the first field, NumGpArgRegs) and emitted
  an `i64` store of the zext'd `i1` — overwriting NumGpArgRegs with
  1 instead of setting the trailing bool.  Took a full debug session
  to track down; classic silent-miscompile shape.
- **Reliable repro**:
    ```
    # /tmp/bug3/pkg/inconsistent.bni
    type S struct { a int; b int; c bool }
    func MakeS() S
    # /tmp/bug3/pkg/inconsistent/inconsistent.bn
    type S struct { a int; b int }    # MISSING `c bool`
    func MakeS() S { var s S; s.a = 1; s.b = 2; return s }
    # /tmp/bug3/main.bn — `s.c = true` is silently accepted +
    # mis-resolved against the impl's 2-field layout.
    ```
- **Why it matters now**: any package whose bni/bn pair drifts (a
  field is added to one without the other, or fields are reordered)
  silently miscompiles, with the symptom landing far from the cause.
  Easy to hit during refactoring; impossible to diagnose without
  reading the emitted LLVM IR.
- **Underlying language-design issue**: structs are CURRENTLY
  DUPLICATED in both `.bni` and `.bn`.  This shouldn't be the case:
  a struct should be defined in ONE place.  Proposed model (user
  preference, 2026-05-29):
    * `type S struct { ... }` in `.bni` only → fully exported
      (callers see all fields and can read/write them directly).
    * `type S` (forward decl, no body) in `.bni` + full
      `type S struct { ... }` in `.bn` → opaque export: callers can
      use `*S` / `@S` as handles, but can't access fields (must go
      through accessor functions).
    * No declaration in `.bni`, full def in `.bn` → fully private.
  Removes the duplication entirely, makes the bug structurally
  impossible.
- **Fix paths** (orthogonal — could do both or pick one):
    1. **Stop-gap**: type checker validates that `.bni` and `.bn`
       struct declarations match (same fields in same order with same
       types).  Catches the silent miscompile but keeps the
       duplication smell.
    2. **Proper fix**: implement the single-definition model above.
       Bigger language change; touches the parser, type-checker
       (cross-file struct lookup), IR-gen (no more "impl struct
       overrides bni struct"), and pkg-layout-spec.md.  Likely
       coordinated with pkg-layout-plan.md follow-on work.
- **Severity rationale**: CRITICAL because it's a silent miscompile
  on a class of refactoring (add a field to bni and forget to mirror
  it in bn) that's near-certain to recur.  Stage 4 was a real
  bug-find; without the universal-sret debugging it would have
  shipped as "Stage 4 mysteriously doesn't work on darwin-arm64."
...
- **Root cause (unconfirmed)**: looks like the native-aa64 backend emits a definition for a struct's destructor-vtable symbol from every translation unit that *uses* the struct (not just the one that defines the type).  Cross-package consumers (`pkg/asm/x64` linking against `pkg/asm.Assembler`) re-emit the same `bn_pkg__asm____dtor_Assembler__vt` and the macOS linker refuses to dedup (Mach-O is strict about duplicate strong symbols).  Needs `pkg/native/aarch64` (or whatever native-aa64 emits the dtor-vt) to mark the symbol weak / linkonce, or only emit from the defining package.
- **Why MAJOR**: blocks the entire macOS aa64 self-host lane.  Doesn't affect LLVM-mode amd64 / arm32 (the LLVM emitter doesn't have this issue; the bug is specific to native-aa64 codegen + Mach-O strictness).
- **Tests covering it**: every package in the cascade list above counts as a regression test once the symbol-emission is fixed.  A focused unit test on `pkg/native/aarch64` would also help (assert the dtor-vt symbol is only emitted from the package that defines the struct, OR is emitted as a coalescing/weak symbol).
- **Proposed fix**: in the native-aa64 backend's dtor-vt emission, gate on "is this struct's defining package the one currently being compiled?"  Same gate already exists for regular function definitions vs. external declarations; extend it to dtor-vts.  Alternatively, emit as `.weak_definition` / `__attribute__((weak))` so the Mach-O linker coalesces duplicates.
