...
### Recursive closure via self-reassignment captures the STALE binding → silent wrong value — all backends — PRE-EXISTING — 🔴 OPEN 2026-06-11
- **Symptom**: a closure that calls a func-value variable which is then (re)assigned to that very closure sees the OLD value, not the closure. `var g @func(int) int = func(x int) int { return 0 }; g = func(x int) int { if x <= 1 { return 1 }; return x * g(x-1) }; println(g(5))` → **0** (expected `120`).  Pure explicit `@func`, no inference, no borrow involved — reproduces with a package-level `g` too.  The capture snapshots `g`'s value at closure-creation (the `return 0` stub), so the recursive `g(x-1)` calls the stub → 0.
- **Severity**: MAJOR — silent wrong value (not a loud failure) for a natural-looking recursive-closure idiom.  Held at major (not critical) because the idiomatic recursion path — a NAMED top-level function — works, and the broken form is a self-referential closure.
- **Root cause** (likely): closures capture by VALUE (snapshot at creation); a self-reference captures the pre-reassignment value.  The fix is either capture-the-binding semantics for the self-reference, or a front-end rejection of the pattern (don't silently miscompile).  Needs a language-semantics decision (capture-by-binding is a real change).
- **Discovery**: 2026-06-11, adversarial review of the `@func`-inferred-default work (claude-todo #123).  Confirmed pre-existing + orthogonal to that work (pure explicit `@func`, reproduces without any inference).
- **Test**: NOT yet added (fails identically on all backends → all-modes xfail, or better a front-end/IR unit test once the semantics decision is made).  Repro above is exact.
