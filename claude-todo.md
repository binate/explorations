...
of bounds and does NOT fault. Silent memory-unsafety / wrong-code, not a crash or a
loud compile error.

Repro (host, LLVM backend):
```
func g(j int) int {
	var a [3]int
...
func main() { testing.Println(g(5)) }
```
`bnc -O0` → `runtime error: index out of bounds: 5 (len 3)` (exit 134, correct).
`bnc -O1` → prints garbage, exit 0 (check gone). Same with a *dynamic* write
(`a[k]=7; return a[j]`), and inside a loop — so it is **any** local-array
dynamic-index access, not const-store-promotion specific. **Raw slices `*[]T` /
managed-slices `@[]T` are UNAFFECTED** (`s[j]` faults correctly at both `-O0` and
`-O1`).

Root cause — NOT yet pinned, needs investigation. The two `OP_BOUNDS_CHECK` removers
do NOT apply: `bceConstIndex` requires BOTH operands const (the index `j` is not);
`bceLoop` requires a loop-header-phi index (`j` is a param). Neither `mem2reg` nor
`load_forward` references `OP_BOUNDS_CHECK` at all. Yet at `-O1` the check AND its
length operand both vanish from the IR (the length is DCE'd once the check is gone).
So it is a **side effect of local-array promotion / aggregate-materialization** at
`-O1` (the array is materialized as an `insertvalue [N x iK]` value while the alloca
is kept for the dynamic GEP) — some pass drops the check while rewriting the array
access. Find that pass and stop it; add an IR-level unit test (RunOptPasses at level 1
preserves the `OP_BOUNDS_CHECK` for a local-array dynamic index).

Discovered while validating the (2a) inline-bounds-check lowering at `-O1`
(commit-in-flight on the LLVM inline form; that work is backend-lowering-only and does
NOT cause this — the removal is at the IR level, before codegen). Confined to bnc
`-O1+` on LLVM, which is **not a shipping config** (shipping = clang `--cflag -O2` on
`-O0` IR; native uses `-O0`, and native `-O2` self-host is a separate MAJOR item), so
non-blocking for releases — but a real correctness hole in the `-O1` path, and the
FIRST *silent* one found (the [24 `-O1` LLVM failures](#) are loud clang type-mismatch
rejections; the native `-O2` SIGSEGV is a crash).

Not catchable in any current CI lane: `-O1` is not a conformance mode (see the
"optimization-level dimension" todo), so there is no mode to `.xfail`; the unit-test
xfail is per-package (too coarse for one case). The opt-level matrix would surface it
automatically. Tracked here with the repro until then.
