...
### Remove `findRuntime` auto-resolution; require an explicit `--runtime` — FILED 2026-06-10
...
- **Why**: this implicit, CWD-dependent resolution caused the Lane A CI conformance break — deeply-nested conformance cells, compiled from CI's workspace-root CWD (checkout one dir deeper, under `binate/`), resolved empty → runtime dropped → every deep `-comp*` cell failed to link. The immediate release-blocker fix made the conformance runners pass explicit `--runtime` (binate `7b35997a`). With that, **no caller relies on auto-resolution** — `scripts/build-*.sh`, `e2e/*.sh`, `scripts/lib/build-compilers.sh` (gen1), and the `release-process.md` smoke tests all already pass `--runtime`.
