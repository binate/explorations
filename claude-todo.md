...
- **Track 1 — const int div/mod → magic-number multiply (+ `madd` fusion, `mul ×1` elim) — 🟡 IN PROGRESS (work-1/session).** aarch64 landed `838ffd40e` (magic SMULH/UMULH + pow2 + ×0/1/-1; shared `common.ComputeMagic*`; conformance `1273` cross-checks magic-vs-hardware incl. sub-word). Measured: does NOT move fasta on aarch64 (~noise) — fasta's `% im` is a local var so magic doesn't fire, M isn't hoisted, and the per-iter `DivCheck` call + FP dominate (see `plan-track1-const-div-magic.md`). x64 landed `225755b19` (one-op IMUL/MUL magic; full native-x64 conformance 3041/0). arm32 landed `1e34fc581` (SMULL/UMULL magic; full native-arm32-baremetal 2995/0; also fixed 1273's INT64 edge literals to int64 so it compiles on 32-bit-int arm32). **Magic division now landed on all three native arches**, each with an adversarial review. **B expansion, Part 1 LANDED `f42bf7131`** — `iropt/simplify.bn`: `foldCastConst` folds an `OP_CAST(OP_CONST_INT)` to `OP_CONST_INT` (the real enabler — mem2reg grounds a promoted var-load into an identity cast of the stored constant, `var d=139968;x%d` → `srem x,cast(int,139968)`; NOT `add X,0` as first thought), so magic fires on NAMED constants; plus algebraic-identity fold (`add X,0`, `mul X,1`, …) + dead-leaf-const DCE (native↔LLVM gap-closer). Reviewed clean; conformance green in all 6 modes (LLVM 3041, VM 3030, gen2 3042, native aa64 3042 / x64 3042 / arm32 2996, all /0); unit tests + conformance `1274`. **Measured: fasta native user-CPU 0.67s→0.55s, ratio 2.16→1.96.** Part 2 IN PROGRESS (work-1) — building a GENERAL LICM pass (user-chosen 2026-09-20): hoist loop-invariant computation (incl. the magic multiplier M and other const materialization) out of loops to a preheader. A native↔LLVM gap-closer the native backends lack; modest fasta gain (Part 1 got the big win). Builds on iropt dom/loops infrastructure. `madd` fusion still deferred.
...
- **Track 4 — IR load-forwarding / promotion of managed-pointer field loads (STRUCTURAL) — 🟡 IN PROGRESS (claimed 2026-09-20, work-4/session).**
  ...
  - **Piece 2 — redundant FIELD-load elimination — LANDED `e88737398`, MEASURED WIN.**
    ...
    Barriers: any call (kills all); a STORE not provably disjoint (a store to a different field of the
    same param, or rooted at a local alloca, is disjoint — else kills). OP_REFDEC is NOT a barrier —
    spec confirms no user destructors (dtors only dec child refcounts + free, never write data
    fields), and the base being a managed-ptr PARAM (single-store ⇒ never reassigned ⇒ pointee live
    for the whole body) means its field is never freed. Barrier set verified complete: at the IR
