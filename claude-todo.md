...
- **CROSS MODES SWEPT via the CI workflow (2026-06-14) — 99 stale conformance xfails removed.** The on-demand `.github/workflows/conformance-xpass.yml` (Actions → "Conformance XPASS (stale-xfail sweep)" → Run workflow; blank `mode` = all 10 modes, or pass one) re-runs each mode's xfailed tests under `--check-xpass`; a red job lists XPASS = stale markers. Full-matrix run results:
  ...
  - **Unittest sweep now possible** — `scripts/unittest/run.sh` gained `--check-xpass` (committed, pending landing approval; same XPASS-on-stale semantics, per-package): run `scripts/unittest/run.sh --check-xpass <mode>`. First finding: `pkg/builtins/rt`'s `builder-comp-int` xfail is STALE (rt runs 21 passing tests — it's injected as native in the VM); `pkg/bootstrap` / `pkg/std/os` are likely the same "native-only in VM" markers that now pass. Whether to drop them is a judgment call (the tests pass, but against injected-native code) — NOT yet removed. The `native_aa64` unittest xfails (11, the weak-`buf.Builder`-dtor dup-symbol MAJOR bug) correctly stay XFAIL.
