...
- **Decision (2026-06-02): Phase B (composite-typed consts) is CANCELED.**  `const` stays **scalar-only** (per `claude-notes.md:267-283`); immutable composite data is expressed with `var readonly` (the in-flight `plan-const-readonly.md` work), not `const`.  The follow-up — **owned by someone else** — is to **reject** `const X CompositeType = …` at the type checker (today it is silently accepted, then dropped at IR-gen → the `EmitConstInt(0)` bug).
  - **Where the rejection goes**: `checkConstDecl` (`pkg/binate/types/check_decl.bn:355-388`) type-checks the const Value via plain `checkExpr` + `AssignableTo` with **no** scalar restriction; add a check that the declared/inferred const type is scalar (int-family / bool / char / float / string), else a clear diagnostic.  Add negative conformance tests (`…_err_const_composite*`).
...
- **Remaining**: `plan-const-nonint.md` — **Phase C** (pointer consts — const-pointer-to-static-global + nil) is still open.
