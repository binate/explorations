...
- **🔴 MAJOR SIBLING — `return <named composite literal>` still mis-compiles (found 2026-06-12 self-review; NOT fixed)**: `func f() Row { return Row{100,200,300} }` returns GARBAGE (e.g. `var a Row = f(); a[0]+a[2]` printed `14420308617`, not 400). Narrowed: `return rowVar` (a named-array VAR) is CORRECT; `return [3]int{...}` (UNNAMED literal) is CORRECT; only `return <NAMED composite literal>` is wrong (both into a named slot AND into an unnamed slot). So the return path has its own composite-literal materialization that doesn't peel `TYP_NAMED` — the RETURN analogue of the var-decl alloca-reuse fast path that `2eeb71c1` fixed. Site: `genReturnStmt` in `pkg/binate/ir/gen_return.bn` (the single-value return path; a named-array return is likely not recognized as `TYP_ARRAY` so the literal's value isn't copied — it returns the alloca pointer / a stale slot). Note the call-arg position (`f(Row{...})`), nested-named (`type B Row; B{...}`), and named raw-slice literal all WORK (verified). **Fix**: peel `TYP_NAMED` (`peelTransparent`) in the return-of-composite-literal path, mirroring the var-decl fast-path fix. Add the return + call-arg positions to `728` (728 currently only covers var-decl + global). 728 should be re-run; this is a real silent-wrong-code hole in named-array/struct returns.
