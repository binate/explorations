...
- **REMAINING — qemu-gated cross modes (can't run on a darwin/arm64 host; runner hard-errors without qemu)**: `builder-comp_native_x64-comp_native_x64` (needs qemu-x86_64), `builder-comp_arm32_linux` (qemu-arm), `builder-comp_arm32_baremetal` (qemu-system-arm) — ~115 conformance xfails (mostly `abi/` matrix) + 17 unittest, UNSWEPT. **Tool now exists**: the on-demand CI workflow `.github/workflows/conformance-xpass.yml` (binate `4414c19d`) — Actions → "Conformance XPASS (stale-xfail sweep)" → Run workflow (blank `mode` = all 10 modes incl. the qemu cross ones; or pass a single mode). It re-runs each mode's xfailed tests under `--check-xpass`; a red job lists the XPASS (stale) markers to delete. CONFORMANCE only — the unittest runner still lacks `--check-xpass`, so the 17 cross-mode unittest xfails stay manual until that lands. Prime suspects already flagged elsewhere in this file: the line-~5077 `native_x64` `abi/iface-multi-return{,-assign}` cells ("almost certainly STALE / now-XPASS") and `635_float32_arith` (xfailed ONLY for the missing host).
