...
- **Next**: pick the biggest remaining `.bni`. After `ir` (1343), the order is `vm.bni` (1265) then `types.bni` (1109). `vm` is NOT in `cmd/bnc`'s BUILDER tree (built by `bnc` first), so it has more freedom; `types` is foundational data. Continue the ratchet: shrink the current max, reset the cap, repeat.
