...
### [CR-2 Plan-1 review] `readonly`-wrapped >16-byte aggregate parameter: by-value signature vs by-pointer call site → garbage / SIGSEGV on BOTH backends — CONFIRMED 2026-06-08, PRE-EXISTING
...
### [CR-2 Plan-1 review] `[N][M]Struct` (value struct) field write `a[i][j].field = …` stores NOWHERE (silent data loss) + read → 0 — CONFIRMED 2026-06-08; BROADER than the filed managed-ptr xfail
...
### [CR-2 Plan-1 review] whole-inner-array composite-literal store via slice/pointer element or struct field-init stores the alloca POINTER (managed-inner variant CORRUPTS) — CONFIRMED 2026-06-08, PRE-EXISTING
...
### [CR-2 Plan-1 review] concrete value cannot be iface-wrapped into a `readonly @Iface` / `readonly *Iface` target (reject-only) — CONFIRMED 2026-06-08
...
### [CR-2 Plan-1 review] method call through an alias-typed receiver (`type AB = @Box`) rejected "cannot call non-function" (reject-only) — CONFIRMED 2026-06-08, PRE-EXISTING (predates Defect 2b)
...
### [CR-2 Plan-1 review] cyclic named type + the new `==`/`<` operand checks → infinite hang (`==`) / SIGSEGV (`<`) — CONFIRMED 2026-06-08; the new checks WORSEN a filed weakness
...
### [CR-2 Plan-1 review] unary minus on a NAMED sub-word/non-host-width int → invalid IR (`sub i64 0, %i8`) — Defect-9 fix incomplete for `TYP_NAMED` — CONFIRMED 2026-06-08
...
### Global address (`&G`) as an rvalue STILL dropped at `OP_CAST` (all 3 non-VM backends) + iface-method ARG (aa64 + LLVM) — `emitValOperand` work was incomplete, per-op whack-a-mole keeps missing sites — CONFIRMED (reproduced) 2026-06-08
