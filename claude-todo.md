...
deleted (`d92726b7`)**.  Only the eventual post-release `buf.Builder` → stdlib
`strings.Builder` swap remains (mechanical import-and-rename; see below).  Detail
retained here as a reference for that swap and the lifetime/UAF gotcha; move to
[claude-todo-done.md](claude-todo-done.md) once the swap lands.
...
- **REMAINING — qemu-gated cross modes (can't run on a darwin/arm64 host; runner hard-errors without qemu)**: `builder-comp_native_x64-comp_native_x64` (needs qemu-x86_64), `builder-comp_arm32_linux` (qemu-arm), `builder-comp_arm32_baremetal` (qemu-system-arm) — ~115 conformance xfails (mostly `abi/` matrix) + 17 unittest, UNSWEPT. Run on CI's x86_64/qemu runner: `run.sh --check-xpass <mode> ...`. Prime suspects already flagged elsewhere in this file: the line-~5077 `native_x64` `abi/iface-multi-return{,-assign}` cells ("almost certainly STALE / now-XPASS") and `635_float32_arith` (xfailed ONLY for the missing host).
