...
- **Residual gap (M3-residual, binate `0fdc9295`)**: only handles the const-BEFORE-struct order. When the struct is declared first (`type S struct { arr [N]int }; const N int = 3`), pass-1 source-ordered collection resolves `[N]int` before N is collected → dim sticks at [0]int → valid use rejected. Same dependency-ordered-resolution root cause as M6; the proper lazy/dependency-ordered const-resolution fix covers both. False-positive rejection, not a miscompile. **Test**: `TestStructBeforeConstDimKnownGap` (checker unit, expectError; flip when fixed).
...
#### M7 — &f()[i] / &a[i][j] wild-pointer — ⏳ PARTIAL FIX (genIndexPtr handles call-base + nested-slice; array-nested blocked on M8)
...
- **Fix (in work-2, gen_access.bn genIndexPtr)**: added a general arm — (1) nested-index base recurses genIndexPtr for an in-place pointer to the inner element, then indexes it (array inner → GEP the pointer; slice/raw-ptr inner → load then index); (2) r-value base (call result) is genExpr'd and its slice/raw-pointer backing is GEP'd. Verified: `&get()[i]` (call→managed-slice, conformance 623) and `&a[i][j]` on a slice-of-slices both work; no regressions (array/slice/index/addr sweeps green).
- **Blocked sub-case**: `&a[i][j]` where `a` is a 2-D ARRAY can't be exercised because plain nested ARRAY indexing is itself broken — see **M8**. The genIndexPtr recursion for the array path is in place and correct in principle; it becomes reachable/testable once M8 lands. **Test**: conformance 623 (unxfail) + nested-slice addr.

#### M8 — nested ARRAY indexing `a[i][j]` emits invalid LLVM (MAJOR, pre-existing, found while doing M7)
...
- **Root cause**: same non-IDENT/SELECTOR index-base limitation just fixed in genIndexPtr, but still present in genIndex (read) and the lvalue/assignment path: for a nested base `a[i]` (EXPR_INSTANTIATE_OR_INDEX) they fall through to loading the inner array as an r-value and then GEP/SliceGet it. Nested SLICE indexing works (the loaded inner slice value still carries its backing pointer); nested ARRAY indexing does not (an array r-value has no backing pointer).
- **Fix**: teach genIndex + the assignment lvalue lowering to recurse for an in-place pointer to a nested-array base (mirror the genIndexPtr fix), then load/store through it. Landing M8 also unblocks M7's `&a[i][j]`-array sub-case. **Test**: conformance xfail (plain 2-D array read+write); add `&a[i][j]`-array once green.
...
### float32 arithmetic is computed in double precision on the f32 bit pattern — CONFIRMED silent wrong-code, VM + both native backends (LLVM correct) — IN PROGRESS (2026-06-06)
- **Symptom**: any float32 arithmetic produces garbage. `var a float32 = 1.5; var b float32 = a + a; println(cast(int, b))` yields a saturated INT64 (`9223372036854775807` / `-9223372036854775808`) instead of `3` on the VM modes (`builder-comp-int`, `-int-int`, `-comp-int`) and on native aa64 / x64_darwin. Correct only on the LLVM modes (`builder-comp` etc.).
- **Confirmed**: `conformance/635_float32_arith` — expects `3\n3`; xfailed on the 3 VM modes + 3 native lanes, passes on the LLVM modes. Discovered during the Plan-4 adversarial review while probing float32 RETURNS (which are CORRECT — see `conformance/636_native_float32_return`).
- **Root cause**: float32 values are stored as their 32-bit IEEE pattern in an 8-byte slot (e.g. `emitConstFloat` → `F64BitsToF32Bits`), but the float-arithmetic emitters treat every float operand as a float64: native `emitFloatBinop` (`aarch64_float.bn` / `x64_float.bn`) FMOVs the 64-bit slot into a D/XMM reg and runs DOUBLE-precision FADD/FMUL/etc., and the VM's float-arith path (`vm_exec_pure.bn` execFloatArithOp) likewise computes in float64. None widen the f32 operand (`Fcvt_s_to_d`) before the op nor narrow the result (`Fcvt_d_to_s`), so the f32 bit pattern is reinterpreted as a garbage double. LLVM is correct because it emits true single-precision ops.
