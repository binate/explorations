...
- **In-cone migration progress:** DONE — `lexer` `97b3af36`, `parser` `80bf8687`, `loader` `1bc59c7b`, `buildcfg`+`asm/parse` (`9e5a6a3d`), `asm/macho`+`asm/elf` (`9d500c1d`), `cmd/bnc` `69679440`, **`mangle` `880efb22`** (dependency root — see below), **`native/{x64,aarch64}` `69fcbca0`**, **`types` `667317aa`**, **`ir` `41e3f949`** (all 2026-06-12). (`native/common`, `debug` already CharBuf-free.) **REMAINING in-cone:** `codegen` — plus stray test-file builds in `token`/`stringutils`.
  - **Remaining shape / order:** `codegen` (2819 sites, 53 files) is the hard one — 103 funcs take a `buf.CharBuf` param + 65 return one: the pervasive value→reference rewrite (`out = emitX(out,...)` threading). Split codegen into several commits (by emit-file cluster), don't land as one blob. Patterns (validated on native+types+ir): local builder `buf.New()`→`buf.NewBuilder()`, `x = x.WriteStr(s)`→`x.Write(s)`, `x = x.WriteInt(n)`→`buf.WriteInt(x, n)`, `x = x.WriteByte(c)`→`x.WriteByte(c)`, `return x.Bytes()`→`return buf.CopyStr(x.String())`; threaded `func f(out buf.CharBuf, …) buf.CharBuf` (returns the threaded buf) → `func f(out *buf.Builder, …)` void, body `out = out.WriteStr`→`out.Write` and `return out`/`return g(out,…)`→bare return, callers `out = f(out,…)`→`f(out,…)` (ir's gen_dtor/gen_iface_vtable did this for 4 funcs). For codegen, threaded funcs that *return a fresh sub-fragment* (not the passed buf) stay value-returning `@[]char` via buf.CopyStr.
