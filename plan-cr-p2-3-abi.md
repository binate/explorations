...
- **Tests**: ✅ LANDED — `conformance/regressions/funcval/return-as-arg` (test on main `dc0f968d`; xfails dropped on the worktree `cfe9f567`, pending cherry-pick). `use(mk())` with a non-capturing `@func(int)int`, asserts `42`. Now passes on comp + the 3 VM-final modes + x64-native (the fix `e337e413` un-broke the VM). The 3 stale VM xfails were removed. NOTE: it currently fails on **aa64-native** only — but that's the unrelated broad aa64-native LINK regression (`bootstrap.Write` undefined, see the CRITICAL todo entry), not a funcval issue, so no per-test xfail was added.
