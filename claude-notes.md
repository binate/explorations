...
**No `string` type.** `string` does NOT exist as a type in Binate. String literals are untyped constants with natural type `[N]const char` and default type `@[]const char`. They may only be assigned to const-qualified char types: `[]const char` (raw slice borrowing static data), `@[]const char` (managed-slice with null backing_refptr — static, never freed), or `[N]const char` (array copy). Non-const variants (`[]char`, `@[]char`) are NOT valid targets — mutation of literal data is unsound, and implicit copying violates no-hidden-behavior. Use `buf.CopyStr("hello")` for mutable chars. (The bootstrap uses `[]char` as a stand-in since it lacks const types.) Language targets small systems where full UTF-8 support is too heavy to justify a separate type.
