...
**`__c_global` — C global variable access — DECIDED 2026-07-05 (pending)**: the variable counterpart to `__c_call`, filling the gap the C-function escape hatch left for C globals (e.g. POSIX `environ`). `__c_global("symbol", T)` yields the **address** of the C global named by the string literal (emitted **verbatim, no name mangling**, like `__c_call`) as a **raw `*T`**, where `T` is the variable's C type. Read via `*p`, write via `*p = …`. `T` must be **C-ABI-passable** (scalar or pointer, same constraint as `__c_call`'s args); the result is always **raw** (`*T`, never `@T` — the storage is the C side's, no refcount header). **Compiled-mode only** (the VM does no FFI). Example — `environ` (C `char**` = Binate `**char`): `__c_global("environ", **char)` is a `***char`, and `*it` is the current `**char` (a null-terminated array of `*char` C strings; bridge each to a Binate slice with a strlen-scan helper). The borrowed `*T` follows raw-borrow discipline — an `environ` entry can dangle after `setenv`/`putenv` reallocates, so copy out before mutating the environment. **NOT** `decl.var.extern` (that is the Binate `.bni`/`.bn` interface/impl split, unrelated to C symbols). Spec: §16.9 `pkg.cglobal`, `binate.ebnf` `BuiltinCall`. Long-term, `__c_global` + `__c_call` may be subsumed by annotation-based `extern` FFI declarations (the stated FFI end-state).

**FFI export (`#[c_export]` + `pkg/builtins/startup`) — DECIDED / RATIFIED 2026-07-11 (spec'd pending, not implemented)**: the *outbound* counterpart to `__c_call`/`__c_global` — expose Binate functions **to** C, and write the program's entry/startup glue in Binate (retiring `runtime/binate_runtime.c`). Adversarially reviewed (ratification-readiness: no blockers). Core pieces: (1) **`#[c_export("name")]`** — an **unqualified, compiler-recognized** annotation (language-standard, joining `build`; NOT ignorable `tool.*` metadata) on a `.bni`-public top-level func; emits an **additional, unmangled** C symbol aliasing it (mangled Binate name unchanged; multiple names allowed; **no grammar change** — fits `AnnotationEntry`). (2) **`bn_init` / `bn_entry`** — two hardcoded well-known glue symbols (a **linkage-ABI contract referenceable by literal name**, `bn_`-family, like today's real `bn_entry`): `bn_init` runs every package's `__init` in dependency order over the **build root's** transitive deps (root = `main` for a program, the facade package for a library — generalizing today's `main`-rooted `main.__init_all`), **idempotent** (run-once guard); `bn_entry` = `bn_init(); main.main()`. (3) **`pkg/builtins/startup`** — a **fifth** tier-0 (path-special) package holding **build-conditional** (`#[build]`) entry functions: hosted C-`main`+argv / baremetal `_start` (placed via a linker-placement annotation `#[section]`/`#[link_at]`) / library `_init` — the entry is **pluggable package code, not compiler-hardcoded**, and the **set of wired-up entry symbols IS the build "mode"** (no mode flag). (4) **Signature rule**: **C-ABI-replicable** (not scalar/pointer-only like `__c_call`) — every Binate type maps to a declarable C type; managed-refcount discipline + the func-value `{vtable,data}` **reversed** field order (vs the interface value's `{data,vtable}`, §7.13.9) are **documentation, not ABI gates**. **Build-tooling** (a `bnc --library`/merge build mode, a header generator, the trivial-forward→symbol-alias optimization) is a **driver concern, NOT core spec**. Design: `design-ffi-export.md` (Appendix A records the rejected alternatives: magic package names, package-clause mode annotations, a `raw` package mode, a designated-dispatcher extern, a `--init-name` build flag). Spec (all pending/Draft): `pkg.cexport`/`.eligible`/`.signature`/`.managed` (§16.9), `prog.entry.glue`/`prog.entry.pluggable`/`prog.init.idempotent` (§17), `pkg0.platform-init` (§20 — amends the tier-0 "exactly four" count), `pkg.link-placement` (§16.9, a Draft note pending the spelling/linker-script decision); plus registering `c_export`/`section`/`link_at` as unqualified names in `pkg.annotation.namespace` (§16.7). Naming is illustrative except: the entry/startup package is now decided — **`startup`** (renamed from `platform_init` 2026-07-13; the §20 spec anchor `pkg0.platform-init` still carries the old spelling until the spec is updated), the annotations are unqualified/compiler-recognized, and `bn_init`/`bn_entry` are literal-name linkage symbols. **Partly implemented (2026-07-13):** bump 0 introduced anodyne `startup` (force-included) and bump 1a moved the command-line-args seam (`Args`/`SetArgs`) into it; see `done/plan-ffi-export-detailed.md` Phase 6.
...
**`print`/`println` — eventual `...*any` library form — DECIDED direction 2026-07-03**: once type switches (§11.12) land, `print`/`println` should stop being compiler builtins and become an ordinary **`...*any`** library variadic. Precise element type is `*any` (a **raw** interface value — args are formatted then discarded, so no retain; the callee sees `*[]*any`), NOT bare `any` (bare `any` is not a value type, §7.10). Per-argument formatting dispatches at **run time in layers**: (1) a fast **type switch** over the built-in scalars / char-slices, written directly to the output with **no allocation**; (2) an **interface assertion** `arg.(*Formattable)` for user types that opt into custom formatting; (3) **reflection** for a struct/default rendering (a later phase, §20.3). It is **NOT** a `...Formattable` parameter: Binate forbids adding methods to imported types (same-package-only rule), so a `Formattable` *constraint* could not print a foreign type that hadn't already impl'd it — `...*any` + runtime dispatch is the only form that prints an arbitrary value (exactly Go's `fmt` rationale, and why `fmt.Print` takes `...any` with `Stringer` an opt-in runtime check). A nice consequence of **exact-identity** type-switch matching (§11.12): a `type Money int` with its own `Format` is **not** caught by `case int:` (Money ≠ int), so the primitive fast-path never shadows custom formatting — put the concrete cases first for speed without losing correctness. The current compile-time per-type lowering is transitional **zero-overhead** scaffolding (no boxing) that exists only because none of {type switches, a `Format` interface + primitive impls, reflection} is implemented yet. Spec: §15.7 `builtin.print` note.
...
Full design lives in `done/plan-interface-syntax-revision.md` (RATIFIED 2026-05-01). Implemented end-to-end: `interface X { ... }` declarations, bare-interface-name as a type expression for `*Iface` / `@Iface`, `impl T : Iface`, vtable dispatch, and generics atop the same machinery.
...
**Lone forward declarations are legal**: a `type T` forward decl with NO full definition anywhere in the package is a *pure opaque type* whose layout is defined outside Binate (C, assembly, or the runtime). Callers hold `*T`/`@T` only (as above). There is deliberately **no "dangling forward" check** requiring a forward decl to be paired with a full definition — it would be unsound: Binate compiles a package at a time and sees a *dependency* only through its `.bni`, so a dependency's forward decl always looks unpaired; and pure opaque types are a legitimate pattern. See `done/plan-type-redecl.md`.

**Whole-package re-export (`expose`) — DECIDED 2026-07-10 (ratified; spec + impl pending)**: a core `.bni` declaration `expose "pkg/std/foo"` adds another package B's **entire exported surface** to this package A's surface — for refactors/renames (promote `pkg/stdx/foo` → `pkg/std/foo`, leaving a forwarder `.bni` with **no `.bn`**) and internal package structuring (an aggregator). **Surface-only (Model 2)**: it affects only A's *exported* surface, **not** A's local scope (it is **not** a dot-import — Binate has none), so A's private names never collide with B's; if A's `.bn` wants B, it imports B separately. **Identity-preserving**: each re-exported `A.X` **is** `B.X` (same type identity / storage / symbol / value — never a copy), so old code (`stdx/foo.Bar`) and new (`std/foo.Bar`) interoperate. **Flat** (`A.X`, not `A.B.X`), **transitive**, **collisions are errors**. A pure forwarder emits no code/storage/`__init`; exposing B adds a dependency edge to B. **NOT an annotation** — it is load-bearing surface semantics, so a declaration. **Whole-package only** for now (per-symbol curation deferred); **vars included**. Impl crux: types/impls already follow identity, but func/var/const qualified-reference mangling is *spelling-driven*, so `expose` makes it follow the **resolved entity's home** (new IR plumbing — stamp the home on injected symbols + a reference-keyed lookup — swept across the ~75 `resolveImportPkg`/`buildQualName` sites, byte-identical for existing code). Design: `done/design-expose.md`; plan: `done/plan-expose.md`. Spec (pending): proposed `pkg.expose.*` rules + `ExposeDecl` grammar.
...
Grammar: `ReceiverType` gains a binding form (`ReceiverBase = QualifiedName [ "[" identifier-list "]" ]`), shared by `MethodDecl` receivers and `ImplDecl`. Spec: §12.1 `gen.no-generic-methods` (narrowed) + new `gen.method.generic-recv` / `gen.impl.generic-recv` (Draft); §11.3 `iface.impl.form`; §10.1/§10.4. Plan: `done/plan-generic-type-methods.md`.

**No conditional impls, and (DECIDED 2026-07-06) no specific-instantiation impls, for v1.** An `impl` on a generic type uses only the **parameterized** form (`impl *Cursor[T] : I`, binding the type's params for *all* T; the old "only specific instantiations can have `impl`" is superseded). Both a **conditional** impl (extra constraint) and a **specific-instantiation** impl (concrete receiver args, `impl Cursor[int] : I`) are forbidden — so the parameterized form is the single mechanism and no impl-overlap can arise. Specific-instantiation impls were verified used by ZERO code repo-wide + referenced by no other rule (vestigial: a generic type had no methods pre-feature), so forbidding is zero-breakage. (A plain type implementing a generic *interface* instantiation, `impl IntBox : Container[int]`, is unrelated and allowed.) Spec: §12.4 `gen.no-conditional-impls`, §12.1 `gen.impl.generic-recv`.
...
**Managed-slice backing ownership — DECIDED 2026-06-27 (clarifies/weakens "no backing ⟺ empty")**: the `backing_refptr` word records whether the slice OWNS a reference-counted allocation. **Owned** (its RefDec frees at count 0 and runs the element dtor over `backingLen`) iff `backing != null` AND its refcount is a normal POSITIVE value. **Unowned / immortal** (RefInc/RefDec no-op, never freed) in EITHER of two forms: (1) `backing == null` — no header; the slice is empty `{null,0,null,0}`, OR a view of immortal static read-only data with a non-null `data` (a `@[]readonly char` literal lowers to `{data, len, null, len}` — `BuildStringMSHeader`, `data_strings.bn`); (2) `backing != null` carrying the `STATIC_REFCOUNT` sentinel — a static-managed allocation whose 2-word `{refcount, free_fn}` header holds the deeply-negative count, so refcount ops short-circuit on it (the SAME immortal mechanism as immortal `@T` objects / package descriptors). Form (2) was verified to work across every refcount path (rt `if h[0] < 0 { return }` in RefInc/RefDec, LLVM-inline `icmp slt %old, 0`, VM + native-x64 via the rt call, native-aarch64 inline). So a **null backing does NOT imply an empty slice** (a non-empty static view is also unowned), and "unowned" is not null-only. An unowned backing's elements are never released (the dtor never runs at count 0) → any MANAGED elements held by such a backing must themselves be immortal. Spec: §7.13.6 `type.layout.slice-managed.backing` (cross-ref §7.7, §18.2 `mem.destructor`/`mem.immortal`).
