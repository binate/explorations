...
| pkg/native/arm64 | `TestEmitCallAggregateReturnSpillsAllWords` (post 337 fix) | The 337 split-passing fix unblocked TestNextRegAdvancesPool; now stuck on a different test. Boot and boot-comp pass; only native crashes. lldb: crash inside auto-generated dtor reading a managed-slice's refptr field as 1 (a bogus pointer). The dtor is invoked at end of an emitCall execution, on a local at sp+0x1638 of emitCall's frame. RefCount(refptr=1) → headerPtr(1) = 0xff..fff1 → ldr fault. Either a frame-layout/aliasing bug, or refptr=1 was written by some prior op (off-by-one in a slice address computation?). Boot/boot-comp pass, so the *logic* is fine — codegen-only bug. |
